...
- **A dynamic dock claims nothing.** Preferences → Appearance → Desktop layout → Dock behavior writes `os-dock-<behavior>` on `<body>` (PHP for the first paint, the apply pass live); `dock.css` parks the rail off its edge by INSET (not transform — the tooltip is `position: fixed`, and a transformed ancestor would pin it to the rail), `src/dock-behavior.ts` writes the rail's extent and summons it from a full-width edge zone, and `measureWorkArea` skips every rail under `os-dock-dynamic`. The apply pass calls `refreshWorkArea()` right after flipping the class.
