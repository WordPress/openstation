...
- **Views escape with `OpenStation\App\Html\esc()` / `attr()` / `json()` / `tag()`, not `esc_html()`**, so they stay host-agnostic; `phpcs.xml.dist` registers them as escaping functions. Short echo tags are rewritten to `<?php echo … ?>` by PHPCBF — write them that way from the start. The `.osx.php` suffix is exempt from the file-name sniff for the same reason `.blade.php` would be.
