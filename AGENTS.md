...
- **A setting that gates a server-side registration must spend a menu refresh when it saves** — the `$os->refresh_menu()` effect on an app action (the Extended Options action in `apps/os-settings/os-settings.os.php`), or `spendMenuRefresh()` (`src/settings/spend-menu-refresh.ts`, and the app's `parts/store.ts`) after a settings save has persisted (the developer-mode toggle). The refresh has to be its own request and that is the whole point: the `games` option is read on `plugins_loaded`, so the request that *writes* it already decided, near its own start, that games were off, and cannot report the window, icon and game list it would now register. Skipping this is why enabling Games saved correctly and showed nothing until an F5. When adding a toggle, ask whether the server registers anything differently because of it; if so, wire the refresh and pin it with a test.
...
Canonical examples in-tree: `src/desktop-files/recycle-bin-icon-state.ts` (state-driven tile art) and `docs/examples/dock-badge.md` (badge counts). Full doc: `docs/event-driven-framework.md`.
...
Two things the declaration cannot answer for you: a tab that is not a page you can ask for cold (the Plugins file editor, opened on a file you picked) is left out of the list, and a menu row wp-admin owns that the window has no tab for means the window is not really replacing that menu yet. `src/open-targets/posts-tabs.ts` is the separate, smaller job of catching those wp-admin URLs when they are reached from somewhere other than the dock — a link in another window, the admin bar's "+ New", a bookmark.
...
| `docs/workspaces.md` | The workspace profile shape, the template list, the token-matching rules, the visibility overlay, the per-desk widget column or appearance patch, or the `columns` / `focus` arrangements change. **Two lists must stay equal on both sides:** the shipped template ids (`openstation_workspace_presets()` ↔ `builtInPresets()` in `src/workspaces/presets.ts`) — neither generates the other, and a drifting id silently turns "drop the Commerce desk" into a filter that removes nothing; and the appearance allowlist (`OPENSTATION_WORKSPACE_APPEARANCE_KEYS` ↔ `WORKSPACE_APPEARANCE_KEYS`) — a key on one side only is either a setting a workspace cannot actually paint, or one the server will not persist. **A workspace never writes:** rails, widget column and appearance are all views over the user's own state, restored on exit. Anything that persists a workspace's value into `navPlacement`, the enabled-widget list or the settings is a bug, not a feature. |
...
| `docs/multisite.md` | Anything cross-site changes: the network admin, the Network Admin dock tile, session scoping. **Nothing that leaves the current site can be a window** — WordPress sends `X-Frame-Options: SAMEORIGIN` on every admin response, and a sibling site or the network admin is a different origin on two of the three network shapes. Read that section before proposing one. |
