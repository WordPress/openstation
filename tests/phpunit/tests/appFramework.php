...
class Tests_OpenStation_AppFramework extends WP_UnitTestCase {
	...
	public function test_effects_are_normalised_for_the_wire() {
		$os = Os::standalone();
		$os->begin();
		$os->badge( -3 )->icon( 'data:image/svg+xml;base64,FULL' )->announce( 'post', 'updated', '7' )->open_url( 'post.php?post=7&action=edit', 'Edit' )->send( 'ping', array( 1 ) )
			->menu(
				array(
		...
		$this->assertSame( 'open_url', $effects[3]['type'] );
