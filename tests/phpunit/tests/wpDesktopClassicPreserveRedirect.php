<?php
/**
 * Tests for desktop_mode_classic_preserve_redirect() — the wp_redirect
...
class Tests_WPDesktopClassicPreserveRedirect extends WP_UnitTestCase {
	...
	/**
	 * @covers ::desktop_mode_classic_preserve_redirect
	 */
	public function test_appends_flag_to_admin_redirect_in_classic_tab() {
		$this->enter_classic();

		$filtered = desktop_mode_classic_preserve_redirect( admin_url( 'edit.php' ) );

		$this->assertStringContainsString( DESKTOP_MODE_CLASSIC_FLAG . '=1', $filtered );
	}
	...
	public function test_leaves_admin_redirect_alone_when_not_classic_tab() {
		...
		$this->assertSame( $location, desktop_mode_classic_preserve_redirect( $location ) );
	}
	...
	public function test_does_not_double_append_when_flag_already_present() {
		...
		$location = admin_url( 'edit.php?' . DESKTOP_MODE_CLASSIC_FLAG . '=1' );
		$filtered = desktop_mode_classic_preserve_redirect( $location );

		$this->assertSame( $location, $filtered );
		$this->assertSame( 1, substr_count( $filtered, DESKTOP_MODE_CLASSIC_FLAG . '=' ) );
	}
	...
	public function test_leaves_empty_location_alone() {
		...
		$this->assertSame( '', desktop_mode_classic_preserve_redirect( '' ) );
	}
	...
	public function test_preserves_existing_query_args() {
		...
		$filtered = desktop_mode_classic_preserve_redirect(
	...
	public function test_filter_is_registered_on_wp_redirect() {
		$this->assertSame(
			999,
			has_filter( 'wp_redirect', 'desktop_mode_classic_preserve_redirect' )
		);
	}
}
