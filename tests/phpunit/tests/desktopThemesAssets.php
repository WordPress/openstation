...
class Tests_OpenStation_DesktopThemesAssets extends WP_UnitTestCase {
	...
	public function tear_down() {
		wp_dequeue_style( OPEN_STATION_DESKTOP_THEME_STYLE_HANDLE );
		wp_deregister_style( OPEN_STATION_DESKTOP_THEME_STYLE_HANDLE );
	...
	private function seed_upload( $slug = 'house-style' ) {
		...
		open_station_desktop_themes_put_index( $index );
	}

	private function select( $slug ) {
		open_station_save_os_settings( self::$admin_id, array( 'desktopTheme' => $slug ) );
	}
	...
	/**
	 * @covers ::open_station_active_desktop_theme_slug
	 */
	public function test_no_selection_resolves_to_empty() {
		$this->assertSame( '', open_station_active_desktop_theme_slug( self::$admin_id ) );
	}
	...
	public function test_installed_selection_resolves() {
		...
		$this->assertSame( 'house-style', open_station_active_desktop_theme_slug( self::$admin_id ) );
	}
	...
	public function test_code_registered_selection_resolves() {
		open_station_register_desktop_theme( 'acme/neon', array( 'name' => 'Neon' ) );
		$this->select( 'acme-neon' );
		$this->assertSame( 'acme-neon', open_station_active_desktop_theme_slug( self::$admin_id ) );
	}
	...
	public function test_orphaned_selection_degrades_to_empty() {
		...
		$stored = open_station_get_os_settings( self::$admin_id );
		$this->assertSame(
			'was-deleted',
			$stored['desktopTheme'],
	...
	/**
	 * @covers ::open_station_enqueue_desktop_theme_style
	 */
	public function test_nothing_is_enqueued_without_a_theme() {
		open_station_enqueue_desktop_theme_style();
		$this->assertFalse( wp_style_is( OPEN_STATION_DESKTOP_THEME_STYLE_HANDLE, 'enqueued' ) );
		$this->assertFalse( wp_style_is( OPEN_STATION_DESKTOP_THEME_STYLE_HANDLE, 'registered' ) );
	}
	...
	public function test_uploaded_theme_enqueues_the_compiled_file() {
		...
		$this->assertTrue( wp_style_is( OPEN_STATION_DESKTOP_THEME_STYLE_HANDLE, 'enqueued' ) );

		$style = wp_styles()->registered[ OPEN_STATION_DESKTOP_THEME_STYLE_HANDLE ];
	 ...
	 * @covers ::open_station_enqueue_desktop_theme_style
	 */
	public function test_style_depends_on_open_station_variables() {
		$this->seed_upload();
		$this->select( 'house-style' );
		open_station_enqueue_desktop_theme_style();
	...
	public function test_code_theme_inlines_its_compiled_css() {
		open_station_register_desktop_theme( 'acme/neon', array(
			'name'   => 'Neon',
	 ...
	 * @covers ::open_station_enqueue_desktop_theme_style
	 */
	public function test_nothing_is_enqueued_when_open_station_is_off() {
		$this->seed_upload();
		$this->select( 'house-style' );
		delete_user_meta( self::$admin_id, 'desktop_mode_mode' );
	...
	/**
	 * @covers ::open_station_desktop_theme_body_class
	 */
	public function test_body_class_is_appended_as_a_string() {
		$this->seed_upload();
		$this->select( 'house-style' );

		$classes = open_station_desktop_theme_body_class( 'existing-class' );
	...
	public function test_body_class_is_untouched_without_a_theme() {
		$this->assertSame( 'existing-class', open_station_desktop_theme_body_class( 'existing-class' ) );
	}
	...
	/**
	 * @covers ::open_station_desktop_theme_inject_shell_config
	 */
	public function test_shell_config_carries_capability_and_url() {
		$config = open_station_desktop_theme_inject_shell_config( array() );
	...
	public function test_shell_config_carries_the_theme_library() {
		...
		open_station_unregister_desktop_theme( 'acme-neon' );
	}
