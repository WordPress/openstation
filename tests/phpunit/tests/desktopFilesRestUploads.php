...
class Tests_OpenStation_RestUploads extends WP_UnitTestCase {
	...
	public function set_up() {
		...
		add_filter( 'open_station_stored_files_upload_overrides', array( $this, 'use_sideload_action' ) );
	}
	...
	public function tear_down() {
		...
		remove_filter( 'open_station_stored_files_upload_overrides', array( $this, 'use_sideload_action' ) );
		remove_all_filters( 'open_station_stored_files_user_quota_bytes' );
		remove_all_filters( 'open_station_stored_files_max_upload_bytes' );
	...
	public function test_happy_path_stores_bytes_and_creates_placement() {
		...
		$res = open_station_files_rest_upload( $req );
	...
	public function test_php_and_double_extension_and_dotfiles_rejected() {
		foreach ( array( 'shell.php', 'shell.php.gif', 'shell.phtml', '.htaccess', '.user.ini', 'web.config' ) as $bad ) {
			...
			$this->assertSame( 'open_station_stored_files_forbidden_type', $res->get_error_code(), "wrong code for $bad" );
		}
	}
	...
	public function test_disallowed_mime_rejected_by_wp_policy() {
		...
		$this->assertSame( 'open_station_stored_files_upload_failed', $res->get_error_code() );
	}

	public function test_oversize_rejected_with_413() {
		add_filter( 'open_station_stored_files_max_upload_bytes', function () {
			return 4;
		} );
		$req = $this->upload_request( 'big.txt', 'way more than four bytes' );
		$res = open_station_files_rest_upload( $req );
		$this->assertWPError( $res );
		$this->assertSame( 'open_station_stored_files_too_large', $res->get_error_code() );
	...
	public function test_quota_exceeded_rejected() {
		add_filter( 'open_station_stored_files_user_quota_bytes', function () {
			return 10;
		} );
		$req = $this->upload_request( 'a.txt', 'well over ten bytes of content' );
		$res = open_station_files_rest_upload( $req );
		$this->assertWPError( $res );
		$this->assertSame( 'open_station_stored_files_quota_exceeded', $res->get_error_code() );
	}
	...
	public function test_subscriber_fails_permission_gate() {
		...
		$result = open_station_files_rest_uploads_permission();
		$this->assertWPError( $result );
		$this->assertSame( 'open_station_stored_files_cannot_upload', $result->get_error_code() );
	}

	public function test_upload_capability_is_filterable() {
		update_user_meta( self::$subscriber_id, 'desktop_mode_mode', '1' );
		wp_set_current_user( self::$subscriber_id );
		add_filter( 'open_station_stored_files_upload_capability', function () {
			return 'read';
		} );
		$this->assertTrue( open_station_files_rest_uploads_permission() );
		remove_all_filters( 'open_station_stored_files_upload_capability' );
	}

	public function test_read_only_recipient_cannot_upload_into_shared_folder() {
		$folder_id = open_station_files_create_folder( self::$admin_id, array( 'name' => 'Team' ) );
		open_station_files_place( self::$admin_id, 0, 'folder', (string) $folder_id );
		$share_id = open_station_folder_share_invite( (int) $folder_id, self::$admin_id, 'user', (string) self::$editor_id, 'read' );
		...
		$res = open_station_files_rest_upload( $req );
		$this->assertWPError( $res );
		$this->assertSame( 'open_station_files_no_write_in_shared_folder', $res->get_error_code() );
	}
	...
	public function test_placement_failure_rollback_balances_created_deleted_actions() {
		...
		$created = 0;
		$deleted = 0;
		add_action( 'open_station_stored_file_created', function () use ( &$created ) {
			$created++;
		} );
		add_action( 'open_station_stored_file_deleted', function () use ( &$deleted ) {
			$deleted++;
		} );

		// Read-only recipient uploading into a shared folder: receive
		// succeeds, place() fails, rollback runs.
		$folder_id = open_station_files_create_folder( self::$admin_id, array( 'name' => 'RO' ) );
		open_station_files_place( self::$admin_id, 0, 'folder', (string) $folder_id );
		$share_id = open_station_folder_share_invite( (int) $folder_id, self::$admin_id, 'user', (string) self::$editor_id, 'read' );
		open_station_folder_share_accept( $share_id, self::$editor_id );

		wp_set_current_user( self::$editor_id );
		$res = open_station_files_rest_upload(
	...
	public function test_relative_path_rejects_dot_segments() {
		$result = open_station_files_resolve_relative_path( self::$admin_id, 0, '../../etc/passwd' );
		$this->assertWPError( $result );
		$this->assertSame( 'open_station_stored_files_bad_path', $result->get_error_code() );
	}

	public function test_directory_only_relative_path_creates_empty_folder() {
		$folder_id = open_station_files_resolve_relative_path( self::$admin_id, 0, 'empty-dir/' );
	...
	public function test_duplicate_display_names_are_distinct_rows() {
		$res1 = open_station_files_rest_upload( $this->upload_request( 'same.txt', 'one' ) );
		$res2 = open_station_files_rest_upload( $this->upload_request( 'same.txt', 'two' ) );
		$this->assertNotWPError( $res1 );
		$this->assertNotWPError( $res2 );
	...
	public function test_rename_route_owner_only() {
		...
		$res = open_station_files_rest_upload( $this->upload_request( 'old.txt', 'rename me please' ) );
		$this->assertNotWPError( $res );
		...
		$out = open_station_files_rest_rename_upload( $req );
		...
		$out2 = open_station_files_rest_rename_upload( $req2 );
