 ...
 * ({@see open_station_validate_default_window_url}), portal integration
...
class Tests_OpenStation_DefaultWindow extends WP_UnitTestCase {
	...
	public function tear_down() {
		delete_user_meta( self::$admin_id, OPEN_STATION_DEFAULT_WINDOW_META );
	...
	/**
	 * @covers ::open_station_get_default_window
	 */
	public function test_get_returns_sane_defaults_for_unconfigured_user() {
		$pref = open_station_get_default_window( self::$admin_id );
	...
	public function test_get_falls_back_when_meta_value_is_not_an_array() {
		update_user_meta( self::$admin_id, OPEN_STATION_DEFAULT_WINDOW_META, 'garbage' );
	...
	/**
	 * @covers ::open_station_set_default_window
	 */
	public function test_set_stores_an_enabled_preference_with_url() {
		$ok = open_station_set_default_window( self::$admin_id, admin_url( 'edit.php' ) );
	...
	public function test_set_null_disables_the_preference() {
		open_station_set_default_window( self::$admin_id, admin_url( 'edit.php' ) );
		open_station_set_default_window( self::$admin_id, null );
	...
	public function test_set_rejects_invalid_user_id() {
		$this->assertFalse( open_station_set_default_window( 0, admin_url( 'edit.php' ) ) );
		$this->assertFalse( open_station_set_default_window( -1, admin_url( 'edit.php' ) ) );
	}

	/**
	 * @covers ::open_station_validate_default_window_url
	 */
	public function test_validate_accepts_same_origin_admin_url() {
		$clean = open_station_validate_default_window_url( admin_url( 'edit.php?post_type=page' ) );
	...
	public function test_validate_rejects_cross_origin_url() {
		$this->assertSame(
			'',
			open_station_validate_default_window_url( 'https://evil.example.com/wp-admin/edit.php' )
		);
	}
	...
	public function test_validate_rejects_non_admin_paths() {
		$this->assertSame(
			'',
			open_station_validate_default_window_url( home_url( '/some-front-end-page/' ) )
		);
	}
	...
	public function test_validate_rejects_non_http_schemes() {
		$this->assertSame(
			'',
			open_station_validate_default_window_url( 'javascript:alert(1)' )
		);
		$this->assertSame(
			'',
			open_station_validate_default_window_url( 'file:///etc/passwd' )
		);
	}
	...
	public function test_validate_preserves_query_string() {
		$clean = open_station_validate_default_window_url( admin_url( 'edit.php?post_type=page&orderby=date' ) );
	...
	public function test_portal_entry_url_honors_disabled_preference() {
		...
		$url = open_station_portal_entry_url( self::$admin_id );
	...
	public function test_portal_entry_url_uses_configured_preference() {
		open_station_set_default_window( self::$admin_id, admin_url( 'plugins.php' ) );
	...
	public function test_validate_accepts_native_marker() {
		$clean = open_station_validate_default_window_url( 'native:os-settings' );
	...
	public function test_validate_rejects_native_marker_with_unsafe_slug() {
		...
		$this->assertSame( '', open_station_validate_default_window_url( 'native:' ) );
		$this->assertSame( '', open_station_validate_default_window_url( 'native:foo/bar' ) );
		$this->assertSame( '', open_station_validate_default_window_url( 'native:foo bar' ) );
		$this->assertSame( '', open_station_validate_default_window_url( 'native:../etc/passwd' ) );
	}
	...
	public function test_portal_entry_url_falls_back_for_native_marker() {
		open_station_set_default_window( self::$admin_id, 'native:os-settings' );
	 ...
	 *
	 * @covers ::open_station_rest_set_default_window
	 */
