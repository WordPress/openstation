 ...
 */
class Tests_DesktopMode_Files_Favicon extends WP_UnitTestCase {
	...
	public function test_resolves_apple_touch_icon_when_no_plain_icon() {
		...
		$result = desktop_mode_resolve_favicon( 'https://example.com/' );
		$this->assertSame( $this->expected_png_data_uri(), $result );
	}
	...
	public function test_falls_back_to_root_favicon_ico() {
		...
		$result = desktop_mode_resolve_favicon( 'https://example.com/some/page' );
		$this->assertSame( $this->expected_png_data_uri(), $result );
	}
	...
	public function test_resolves_relative_href_against_page_url() {
		...
		$result = desktop_mode_resolve_favicon( 'https://example.com/sub/page' );
		$this->assertSame( $this->expected_png_data_uri(), $result );
	}
	...
	public function test_rejects_oversize_body() {
		...
		$big_bytes  = str_repeat( 'x', DESKTOP_MODE_FAVICON_MAX_BYTES + 1 );
		$this->stub_http( array(
	...
	public function test_returns_null_for_non_http_url() {
		$this->assertNull( desktop_mode_resolve_favicon( 'ftp://example.com/' ) );
		$this->assertNull( desktop_mode_resolve_favicon( 'javascript:alert(1)' ) );
		$this->assertNull( desktop_mode_resolve_favicon( '' ) );
	}
	...
	public function test_filter_can_override_with_synthetic_uri() {
		...
		$this->assertSame(
			...
			desktop_mode_resolve_favicon( 'https://example.com/' )
		);
	}
