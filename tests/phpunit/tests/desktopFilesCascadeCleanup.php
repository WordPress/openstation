 ...
 */
class Tests_DesktopMode_FilesCascadeCleanup extends WP_UnitTestCase {
	 ...
	 * `desktop_mode_files_get_placement()` normalizer strips
	...
	/**
	 * @covers ::desktop_mode_files_cascade_trash_placements_for_entity
	 */
	public function test_cascade_soft_trashes_matching_placements() {
		...
		$p1 = desktop_mode_files_place( self::$admin_id, 0, 'post', (string) $post_id, array( 'x' => 0, 'y' => 0 ) );
		$p2 = desktop_mode_files_place( self::$other_id, 0, 'post', (string) $post_id, array( 'x' => 0, 'y' => 1 ) );

		$count = desktop_mode_files_cascade_trash_placements_for_entity( 'post', (string) $post_id );
	...
	public function test_cascade_is_idempotent() {
		$post_id = self::factory()->post->create( array( 'post_status' => 'publish' ) );
		desktop_mode_files_place( self::$admin_id, 0, 'post', (string) $post_id, array( 'x' => 0, 'y' => 0 ) );

		$first  = desktop_mode_files_cascade_trash_placements_for_entity( 'post', (string) $post_id );
		$second = desktop_mode_files_cascade_trash_placements_for_entity( 'post', (string) $post_id );
	...
	public function test_cascade_ignores_unrelated_file_types() {
		...
		$post_placement = desktop_mode_files_place( self::$admin_id, 0, 'post', (string) $post_id, array( 'x' => 0, 'y' => 0 ) );
		$user_placement = desktop_mode_files_place( self::$admin_id, 0, 'user', (string) $other_user, array( 'x' => 1, 'y' => 0 ) );
	...
	/**
	 * @covers ::desktop_mode_files_cascade_on_post_trash
	 */
	public function test_wp_trash_post_triggers_cascade() {
		$post_id = self::factory()->post->create( array( 'post_status' => 'publish' ) );
		$p_id    = desktop_mode_files_place( self::$admin_id, 0, 'post', (string) $post_id, array( 'x' => 0, 'y' => 0 ) );
	...
	public function test_pages_cascade_via_post_trash_hook() {
		...
		$p_id    = desktop_mode_files_place( self::$admin_id, 0, 'post', (string) $page_id, array( 'x' => 0, 'y' => 0 ) );
	...
	public function test_attachment_wp_trash_routes_to_attachment_filetype() {
		...
		$p_id   = desktop_mode_files_place( self::$admin_id, 0, 'attachment', (string) $att_id, array( 'x' => 0, 'y' => 0 ) );
	...
	/**
	 * @covers ::desktop_mode_files_cascade_on_attachment_delete
	 */
	...
	/**
	 * @covers ::desktop_mode_files_cascade_on_user_delete
	 */
	public function test_deleted_user_triggers_cascade() {
		require_once ABSPATH . 'wp-admin/includes/user.php';
		$victim_id = self::factory()->user->create( array( 'role' => 'subscriber' ) );
		$p_id      = desktop_mode_files_place( self::$admin_id, 0, 'user', (string) $victim_id, array( 'x' => 0, 'y' => 0 ) );
	...
	public function test_cascade_fires_after_action_per_placement() {
		$post_id = self::factory()->post->create( array( 'post_status' => 'publish' ) );
		desktop_mode_files_place( self::$admin_id, 0, 'post', (string) $post_id, array( 'x' => 0, 'y' => 0 ) );
		desktop_mode_files_place( self::$other_id, 0, 'post', (string) $post_id, array( 'x' => 0, 'y' => 1 ) );

		$fired = 0;
		add_action( 'desktop_mode_files_after_cascade_trash_placement', function () use ( &$fired ) {
			++$fired;
		} );
