...
class Tests_OpenStation_ContentChanges extends WP_UnitTestCase {
	...
	public function set_up() {
		...
		open_station_content_changes_reset();
		delete_option( OPEN_STATION_CONTENT_CHANGES_LOG_OPTION );
		delete_transient( open_station_content_changes_buffer_key( self::$admin_id ) );
	}
	...
	public function tear_down() {
		...
		remove_all_filters( 'open_station_content_changes_should_record' );
		remove_all_filters( 'open_station_content_changes_broadcasts' );
	...
	/**
	 * @covers ::open_station_content_changes_record
	 */
	public function test_record_rejects_invalid_args() {
		$this->assertFalse( open_station_content_changes_record( '', 1, 'updated' ) );
		$this->assertFalse( open_station_content_changes_record( 'post', 0, 'updated' ) );
		$this->assertFalse( open_station_content_changes_record( 'post', 1, '' ) );
		$this->assertSame( array(), open_station_content_changes_log() );
	}
	...
	public function test_record_dedupes_first_writer_wins() {
		$this->assertTrue( open_station_content_changes_record( 'post', 9, 'trashed' ) );
		$this->assertFalse( open_station_content_changes_record( 'post', 9, 'updated' ) );

		$log = open_station_content_changes_log();
	...
	public function test_should_record_filter_vetoes() {
		add_filter( 'open_station_content_changes_should_record', '__return_false' );
		$this->assertFalse( open_station_content_changes_record( 'post', 3, 'updated' ) );
		$this->assertSame( array(), open_station_content_changes_log() );
	}
	...
	public function test_recorded_action_fires() {
		$seen = array();
		add_action(
			'open_station_content_change_recorded',
		...
		open_station_content_changes_record( 'shop_order', 12, 'created' );
	...
	/**
	 * @covers ::open_station_content_changes_on_after_insert_post
	 */
	...
	public function test_post_type_without_show_ui_is_skipped() {
		...
		$this->assertArrayNotHasKey( 'wpd_hidden_cpt', open_station_content_changes_log() );
	}
	 ...
	 *
	 * @covers ::open_station_content_changes_record
	 * @covers ::open_station_recycle_bin_record_change
	 */
	...
	/**
	 * @covers ::open_station_content_changes_on_comment_transition
	 */
	...
	/**
	 * @covers ::open_station_content_changes_register_wc_hooks
	 */
	public function test_wc_hooks_are_not_registered_without_woocommerce() {
		$this->assertFalse( class_exists( 'WooCommerce' ) );
		$this->assertFalse( open_station_content_changes_register_wc_hooks() );
	}
	...
	/**
	 * @covers ::open_station_content_changes_on_shutdown
	 */
	public function test_shutdown_buffers_an_unflushed_changelog() {
		open_station_content_changes_record( 'post', 5, 'updated' );

		open_station_content_changes_on_shutdown();

		$buffered = get_transient( open_station_content_changes_buffer_key( self::$admin_id ) );
		$this->assertSame( array( 5 ), $buffered['post']['updated'] );
	}

	/**
	 * @covers ::open_station_content_changes_emit_footer
	 */
	public function test_footer_emits_broadcasts_and_consumes_the_buffer() {
		...
		open_station_content_changes_record( 'shop_order', 12, 'updated' );
		open_station_content_changes_on_shutdown();

		// Redirect target: fresh request state, footer flushes the buffer.
		open_station_content_changes_reset();
		ob_start();
		open_station_content_changes_emit_footer();
		$html = ob_get_clean();
		...
		$this->assertFalse( get_transient( open_station_content_changes_buffer_key( self::$admin_id ) ) );
	...
	public function test_footer_emits_nothing_outside_chromeless() {
		open_station_content_changes_record( 'post', 7, 'updated' );
	...
	public function test_shutdown_does_not_rebuffer_a_flushed_changelog() {
		$this->enter_chromeless();
		open_station_content_changes_record( 'post', 21, 'updated' );
	...
	public function test_broadcasts_filter_can_suppress_the_emit() {
		$this->enter_chromeless();
		open_station_content_changes_record( 'post', 8, 'updated' );
		add_filter( 'open_station_content_changes_broadcasts', '__return_empty_array' );
	...
	/**
	 * @covers ::open_station_content_changes_heartbeat_received
	 */
	public function test_heartbeat_without_opt_in_key_is_untouched() {
		$response = open_station_content_changes_heartbeat_received( array(), array() );
		$this->assertArrayNotHasKey( 'open_station_content_changes', $response );
	}
	...
	public function test_heartbeat_returns_entries_newer_than_seen_ts() {
		open_station_content_changes_record( 'page', 33, 'updated' );
		open_station_content_changes_on_shutdown();

		$response = open_station_content_changes_heartbeat_received(
			array(),
			array( 'open_station_content_changes_seen_ts' => 0 )
		);

		$block = $response['open_station_content_changes'];
		...
		$caught_up = open_station_content_changes_heartbeat_received(
			array(),
			array( 'open_station_content_changes_seen_ts' => $block['ts'] )
		);
		$this->assertSame( array(), $caught_up['open_station_content_changes']['entries'] );
		$this->assertSame( $block['ts'], $caught_up['open_station_content_changes']['ts'] );
	}
}
