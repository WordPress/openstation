...
class Tests_OpenStation_GamesPlaytime extends WP_UnitTestCase {
	...
	public function tear_down() {
		delete_user_meta( self::$player_a, OPENSTATION_GAMES_PLAYTIME_META );
		delete_user_meta( self::$player_b, OPENSTATION_GAMES_PLAYTIME_META );
		delete_user_meta( self::$player_a, OPENSTATION_GAMES_PLAYTIME_DAYS_META );
		delete_user_meta( self::$player_b, OPENSTATION_GAMES_PLAYTIME_DAYS_META );
		openstation_unregister_game( 'test-game' );
		remove_all_filters( 'openstation_games' );
		remove_all_filters( 'openstation_game_playtime_pre_record' );
		remove_all_filters( 'openstation_games_playtime_max_increment' );
		remove_all_filters( 'openstation_games_playtime_history_days' );
		parent::tear_down();
	}
	...
	public function test_playtime_accumulates_per_game() {
		...
		$this->assertSame( 105, openstation_games_get_playtime( self::$player_a, 'test-game' ) );
	...
	public function test_playtime_rejects_unknown_game_and_bad_input() {
		...
		$this->assertSame( 'openstation_unknown_game', $unknown->get_error_code() );

		$zero = openstation_games_add_playtime( 'test-game', self::$player_a, 0 );
		$this->assertWPError( $zero );
		$this->assertSame( 'openstation_invalid_playtime', $zero->get_error_code() );

		$negative = openstation_games_add_playtime( 'test-game', self::$player_a, -10 );
		$this->assertWPError( $negative );

		$ghost = openstation_games_add_playtime( 'test-game', 0, 30 );
	...
	public function test_playtime_increment_is_clamped() {
		...
		add_filter(
			'openstation_games_playtime_max_increment',
			static function () {
				return 60;
			}
		);
	...
	public function test_pre_record_filter_vetoes() {
		...
		$result = openstation_games_add_playtime( 'test-game', self::$player_a, 30 );
	...
	public function test_get_playtime_survives_corrupt_meta() {
		...
		update_user_meta( self::$player_a, OPENSTATION_GAMES_PLAYTIME_META, array( 'test-game' => -50, '' => 10 ) );
	...
	public function test_playtime_buckets_by_day() {
		...
		openstation_games_add_playtime( 'test-game', self::$player_a, 15 );

		$days = openstation_games_get_playtime_daily( self::$player_a, 'test-game' );
		$this->assertSame( array( $today => 60 ), $days );
		$this->assertSame(
			array( 'test-game' => array( $today => 60 ) ),
			openstation_games_get_playtime_daily( self::$player_a )
		);
	}
	...
	public function test_playtime_daily_buckets_are_pruned() {
		...
		update_user_meta(
			self::$player_a,
			OPENSTATION_GAMES_PLAYTIME_DAYS_META,
	...
	public function test_get_playtime_daily_survives_corrupt_meta() {
		...
		$this->assertSame(
			array( '2026-01-02' => 0 ),
			openstation_games_get_playtime_daily( self::$player_a, 'test-game' )
		);
	}
	...
	public function test_rest_record_then_get() {
		...
		$resp = openstation_games_rest_record_playtime( $req );
	...
	public function test_rest_record_404s_unknown_game() {
		...
		$result = openstation_games_rest_record_playtime( $req );
		$this->assertWPError( $result );
