<?php
/**
 * Tests for `open_station_ai_normalize_tool_schema()` — the projection that keeps
...
class Tests_OpenStation_AiToolSchemaNormalization extends WP_UnitTestCase {
	 ...
	 *
	 * @covers ::open_station_ai_normalize_tool_schema
	 */
	public function test_type_union_becomes_object() {
		$out = open_station_ai_normalize_tool_schema( array(
	...
	public function test_nested_combinators_are_preserved() {
		...
		$out = open_station_ai_normalize_tool_schema( $nested );
	...
	public function test_empty_or_non_array_becomes_object_schema() {
		foreach ( array( array(), null, '', 0, false ) as $empty ) {
			$out = open_station_ai_normalize_tool_schema( $empty );
			$this->assertSame( 'object', $out['type'] );
			$this->assertSame(
	 ...
	 * also normalizes on the `open_station_ai_tools` filter).
	 *
	 * @covers ::open_station_ai_normalize_tool_schema
	 */
	public function test_idempotent() {
		$once  = open_station_ai_normalize_tool_schema( array(
			'type'       => array( 'object', 'null' ),
			'anyOf'      => array( array( 'required' => array( 'a' ) ) ),
			'properties' => array(),
		) );
		$twice = open_station_ai_normalize_tool_schema( $once );
	 ...
	 *
	 * @covers ::open_station_ai_strip_wp_schema_keys
	 */
