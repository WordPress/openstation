...
class Tests_OpenStation_AgentsDrag extends WP_UnitTestCase {
	 ...
	 *
	 * @covers Open_Station_User_File::serialize
	 */
	public function test_human_user_file_has_no_agent_fields() {
		$shape = ( new Open_Station_User_File( (string) self::$admin_id ) )->serialize();
	...
	public function test_agent_without_drag_trigger_serializes_null_kinds() {
		...
		open_station_agent_update( $agent->ID, array( 'description' => 'Drops backgrounds.' ) );
		$shape = ( new Open_Station_User_File( (string) $agent->ID ) )->serialize();
	...
	public function test_drag_trigger_kinds_ship_inline() {
		...
		$shape = ( new Open_Station_User_File( (string) $filtered->ID ) )->serialize();
		...
		$shape = ( new Open_Station_User_File( (string) $open->ID ) )->serialize();
	...
	public function test_invoke_source_reaches_completed_context() {
		...
		$response = open_station_agents_rest_invoke( $request );
