...
class Tests_OpenStation_NotesRecycleBin extends WP_UnitTestCase {
	...
	/**
	 * @covers ::open_station_recycle_bin_capture_post_types
	 */
	public function test_show_ui_custom_post_types_are_captured_by_default() {
		...
		$types = open_station_recycle_bin_capture_post_types();
		$this->assertContains( 'wpd_test_book', $types );
		// The notes CPT is headless but opted in by its own feature.
		$this->assertContains( OPEN_STATION_NOTES_POST_TYPE, $types );
	...
	/**
	 * @covers ::open_station_recycle_bin_shape_item
	 */
	public function test_custom_post_type_rows_use_their_menu_dashicon() {
		...
		$item = open_station_recycle_bin_shape_item( get_post( $book_id ) );
	...
	/**
	 * @covers ::open_station_notes_recycle_bin_gate
	 */
	public function test_owner_sees_their_trashed_note_in_the_bin() {
		...
		$result = open_station_recycle_bin_get_items();
	...
	/**
	 * @covers ::open_station_recycle_bin_plain_text
	 * @covers ::open_station_recycle_bin_shape_item
	 */
	public function test_titles_with_apostrophes_are_not_entity_encoded() {
		...
		$item = open_station_recycle_bin_shape_item( get_post( $note_id ) );
	...
	/**
	 * @covers ::open_station_notes_recycle_bin_item
	 */
	...
	public function test_admins_do_not_see_other_users_trashed_notes() {
		...
		$ids = wp_list_pluck( open_station_recycle_bin_get_items()['items'], 'id' );
		$this->assertNotContains( $note_id, $ids );

		// Nor can they restore or purge it through the bin.
		$restore = open_station_recycle_bin_restore( $note_id );
		$this->assertWPError( $restore );
		$purge = open_station_recycle_bin_purge( $note_id );
	...
	public function test_subscribers_manage_their_own_trashed_notes() {
		...
		$this->assertTrue( open_station_recycle_bin_restore( $note_id ) );
		$this->assertSame( 'private', get_post_status( $note_id ) );
	}

	/**
	 * @covers ::open_station_recycle_bin_restore
	 */
	...
	/**
	 * @covers ::open_station_notes_recycle_bin_count
	 */
