...
class Tests_OpenStation_GamesStore extends WP_UnitTestCase {
	...
	public function tear_down() {
		...
		remove_all_filters( 'openstation_game_score_pre_save' );
		parent::tear_down();
	}
	...
	public function test_score_meta_is_bounded_and_flat() {
		...
		$clean = openstation_games_sanitize_score_meta( $meta );
	...
	public function test_pre_save_filter_vetoes() {
		...
		$result = openstation_games_save_score( 'test-game', self::$player_a, 10 );
		$this->assertWPError( $result );
	...
	public function test_leaderboard_orders_and_pages() {
		...
		openstation_games_save_score( 'test-game', self::$player_b, 40 );

		$page1 = openstation_games_get_scores( 'test-game', array( 'per_page' => 2 ) );
		$this->assertSame( 4, $page1['total'] );
		$this->assertSame( array( 40, 30 ), wp_list_pluck( $page1['rows'], 'score' ) );

		$page2 = openstation_games_get_scores( 'test-game', array( 'per_page' => 2, 'page' => 2 ) );
		$this->assertSame( array( 20, 10 ), wp_list_pluck( $page2['rows'], 'score' ) );

		$mine = openstation_games_get_scores( 'test-game', array( 'user_id' => self::$player_b ) );
	...
	public function test_create_challenge_validates_parties() {
		...
		$this->assertSame( 'openstation_self_challenge', $self->get_error_code() );

		$ghost = openstation_games_create_challenge( 'test-game', self::$player_a, 999999, 10 );
	...
	public function test_challenge_state_machine() {
		...
		$updated = openstation_games_complete_challenge( $id, 120, array( 'wpm' => 60 ) );
	...
	public function test_matching_score_does_not_beat() {
		...
		$updated = openstation_games_complete_challenge( $id, 100 );
	...
	public function test_challenge_deltas_are_version_gated() {
		...
		$all = openstation_games_get_challenges_for_user( self::$player_b, 0 );
