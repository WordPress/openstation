...
class Tests_OpenStation_DesktopThemesPayload extends WP_UnitTestCase {
	...
	public function tear_down() {
		...
		remove_all_filters( 'open_station_desktop_themes_payload_cap' );
		remove_all_actions( 'open_station_desktop_theme_registered' );
		parent::tear_down();
	}

	private function register( $id, $overrides = array() ) {
		return open_station_register_desktop_theme( $id, array_merge(
			array(
				'name'    => 'Theme ' . $id,
				'version' => '1.0.0',
	...
	public function test_register_returns_wp_error_on_bad_input() {
		$this->assertWPError( open_station_register_desktop_theme( 'Bad Id!', array( 'name' => 'x' ) ) );
		$this->assertWPError( open_station_register_desktop_theme( 'acme/neon', array( 'name' => '' ) ) );
	}
	...
	public function test_registered_action_fires_only_on_success() {
		$fired = array();
		add_action(
			'open_station_desktop_theme_registered',
			static function ( $slug ) use ( &$fired ) {
				$fired[] = $slug;
			}
		);
		open_station_register_desktop_theme( 'Bad Id!', array( 'name' => 'x' ) );
	...
	/**
	 * @covers ::open_station_unregister_desktop_theme
	 */
	public function test_unregister_accepts_id_or_slug() {
		$this->register( 'acme/neon' );
		open_station_unregister_desktop_theme( 'acme/neon' );
		$this->assertNull( open_station_desktop_theme_registry( 'acme-neon' ) );
	}
	...
	public function test_code_themes_are_sanitized_like_uploads() {
		...
		$css = open_station_desktop_theme_registry( 'acme-neon' )['cssText'];
	...
	public function test_code_theme_assets_must_be_absolute_urls() {
		...
		$icons = open_station_desktop_theme_registry( 'acme-neon' )['manifest']['icons'];
	...
	public function test_payload_is_empty_by_default() {
		$this->assertSame( array(), open_station_build_desktop_themes_payload() );
	}
	...
	public function test_css_url_versus_css_text_by_source() {
		...
		$payload = array();
		foreach ( open_station_build_desktop_themes_payload() as $entry ) {
	...
	/**
	 * @covers ::open_station_desktop_themes_payload_cap
	 */
	public function test_payload_cap_is_enforced_and_filterable() {
		for ( $i = 0; $i < 5; $i++ ) {
			$this->register( "acme/theme-{$i}" );
		}
		add_filter( 'open_station_desktop_themes_payload_cap', static function () {
			return 2;
		} );
		$this->assertCount( 2, open_station_build_desktop_themes_payload() );
	}
