...
class Tests_OpenStation_AgentsIdentity extends WP_UnitTestCase {
	...
	private function create_agent_user( $name = 'Remove BG' ) {
		$user = openstation_agent_create_user(
			array(
				'name' => $name,
				'role' => 'author',
			)
	...
	public function test_users_column_labels_agents() {
		...
		$agent_cell = openstation_agent_users_custom_column( '', 'openstation_agent_type', $agent->ID );
		$this->assertStringContainsString( 'Agent', $agent_cell );

		$human_cell = openstation_agent_users_custom_column( '', 'openstation_agent_type', self::$admin_id );
		$this->assertStringContainsString( 'Person', $human_cell );

		$other = openstation_agent_users_custom_column( 'existing', 'posts', $agent->ID );
	...
	public function test_delete_refuses_non_agents() {
		...
		$this->assertWPError( $result );
		$this->assertSame( 'openstation_agent_not_an_agent', $result->get_error_code() );
