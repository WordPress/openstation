<?php
/**
 * Tests for `open_station_sanitize_os_settings()` — the gatekeeper
...
class Tests_OpenStation_OsSettings extends WP_UnitTestCase {
	...
	public function test_sanitize_keeps_known_admin_bar_mode() {
		foreach ( OPEN_STATION_OS_SETTINGS_ADMIN_BAR_MODES as $mode ) {
			$clean = open_station_sanitize_os_settings( array( 'adminBarMode' => $mode ) );
	...
	public function test_sanitize_falls_back_to_default_for_unknown_admin_bar_mode() {
		$clean = open_station_sanitize_os_settings( array( 'adminBarMode' => 'peekaboo' ) );
	...
	public function test_sanitize_falls_back_when_admin_bar_mode_missing() {
		$clean = open_station_sanitize_os_settings( array( 'wallpaper' => 'dark' ) );
		$this->assertSame( 'static', $clean['adminBarMode'] );
	}
	...
	public function test_sanitize_keeps_known_layout_value() {
		foreach ( array( 'classic', 'unified', 'spatial' ) as $layout ) {
			$clean = open_station_sanitize_os_settings( array( 'desktopLayout' => $layout ) );
	...
	public function test_sanitize_falls_back_to_default_for_unknown_layout() {
		$clean = open_station_sanitize_os_settings( array( 'desktopLayout' => 'invalid-mode' ) );
	...
	public function test_sanitize_keeps_developer_mode_enabled_true() {
		$clean = open_station_sanitize_os_settings( array( 'developerModeEnabled' => true ) );
	...
	public function test_window_reveal_round_trips_through_user_meta() {
		...
		update_user_meta(
			...
			open_station_sanitize_os_settings( array( 'windowReveal' => 'blinds' ) )
		);
		$loaded = open_station_get_os_settings( $user_id );
	...
	public function test_sanitize_drops_malformed_wallpaper_settings() {
		...
		$clean = open_station_sanitize_os_settings( array( 'wallpaperSettings' => 'bogus' ) );
	...
	public function test_sanitize_caps_and_trims_wallpaper_settings() {
		...
		$clean = open_station_sanitize_os_settings( array( 'wallpaperSettings' => $many ) );
