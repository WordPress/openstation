...
class Tests_OpenStation_FileShares extends WP_UnitTestCase {
	...
	private function make_stored_file( $owner_id, $name = 'doc.txt', $contents = 'bytes' ) {
		...
		open_station_files_place( $owner_id, 0, 'upload', (string) $id );
		return $id;
	}
	...
	public function test_invite_accept_creates_recipient_placement_and_read_access() {
		...
		$share_id = open_station_stored_file_share_invite( $file_id, self::$owner_id, self::$recipient_id );
		...
		$pending = open_station_files_get_pending_file_shares_for_user( self::$recipient_id );
		$this->assertCount( 1, $pending );
		$shape = open_station_files_shape_file_share( $pending[0] );
		$this->assertSame( 'file', $shape['targetType'] );
		$this->assertSame( 'doc.txt', $shape['fileName'] );

		$next = open_station_stored_file_share_accept( $share_id, self::$recipient_id );
		$this->assertSame( 'accepted', $next['state'] );

		// The recipient got a root placement of the upload.
		$rows = open_station_files_get_for_user_folder( self::$recipient_id, 0 );
		...
		// And read access.
		$this->assertTrue( open_station_stored_file_user_can_read( $file_id, self::$recipient_id ) );
		// The pending list is now empty.
		$this->assertCount( 0, open_station_files_get_pending_file_shares_for_user( self::$recipient_id ) );
	}
	...
	public function test_invite_validations() {
		...
		$err = open_station_stored_file_share_invite( $file_id, self::$recipient_id, self::$stranger_id );
		$this->assertWPError( $err );
		$this->assertSame( 'open_station_files_forbidden', $err->get_error_code() );

		// Owner cannot be invited.
		$err = open_station_stored_file_share_invite( $file_id, self::$owner_id, self::$owner_id );
		$this->assertWPError( $err );
		$this->assertSame( 'open_station_files_share_owner', $err->get_error_code() );

		// Subscribers are ineligible (no edit_posts).
		$err = open_station_stored_file_share_invite( $file_id, self::$owner_id, self::$stranger_id );
		$this->assertWPError( $err );
	...
	public function test_rest_create_rejects_write_capability() {
		...
		$res = open_station_files_rest_create_file_share( $req );
		$this->assertWPError( $res );
		$this->assertSame( 'open_station_files_invalid_capability', $res->get_error_code() );
	...
	public function test_deny_after_accept_scrubs_placement() {
		$file_id  = $this->make_stored_file( self::$owner_id );
		$share_id = open_station_stored_file_share_invite( $file_id, self::$owner_id, self::$recipient_id );
		open_station_stored_file_share_accept( $share_id, self::$recipient_id );
		$this->assertCount( 1, open_station_files_get_for_user_folder( self::$recipient_id, 0 ) );

		$next = open_station_stored_file_share_deny( $share_id, self::$recipient_id );
		$this->assertSame( 'denied', $next['state'] );
		$this->assertCount( 0, open_station_files_get_for_user_folder( self::$recipient_id, 0 ) );
		$this->assertFalse( open_station_stored_file_user_can_read( $file_id, self::$recipient_id ) );
	}
	...
	public function test_leave_flow() {
		...
		$this->assertTrue( open_station_stored_file_share_leave( $file_id, self::$recipient_id ) );
		$this->assertSame( 'denied', open_station_stored_file_share_state( $file_id, self::$recipient_id ) );
		$this->assertCount( 0, open_station_files_get_for_user_folder( self::$recipient_id, 0 ) );

		// Owner cannot leave; a non-member gets not_member.
		$err = open_station_stored_file_share_leave( $file_id, self::$owner_id );
		$this->assertSame( 'open_station_files_owner_cannot_leave', $err->get_error_code() );
		$err = open_station_stored_file_share_leave( $file_id, self::$stranger_id );
	...
	public function test_revoke_scrubs_recipient() {
		...
		$this->assertTrue( open_station_stored_file_share_revoke( $share_id, self::$owner_id ) );
		$this->assertSame( 'none', open_station_stored_file_share_state( $file_id, self::$recipient_id ) );
		$this->assertCount( 0, open_station_files_get_for_user_folder( self::$recipient_id, 0 ) );
		$this->assertFalse( open_station_stored_file_user_can_read( $file_id, self::$recipient_id ) );
	}
	...
	public function test_owner_purge_deletes_bytes_shares_and_recipient_placements() {
		...
		$path = open_station_stored_file_path( open_station_stored_files_get( $file_id ) );

		// Owner hard-removes their placement.
		$rows = open_station_files_get_for_user_folder( self::$owner_id, 0 );
		$this->assertTrue( open_station_files_remove( (int) $rows[0]['id'], self::$owner_id ) );

		$this->assertNull( open_station_stored_files_get( $file_id ) );
	...
	public function test_file_share_does_not_bleed_into_folder_queries_with_same_id() {
		...
		$folder_id = open_station_files_create_folder( self::$owner_id, array( 'name' => 'Collide' ) );
		$file_id   = $this->make_stored_file( self::$owner_id );
		...
		open_station_stored_file_share_invite( $file_id, self::$owner_id, self::$recipient_id );

		// The folder's share list must stay empty; the file's list has one.
		$this->assertCount( 0, open_station_files_get_folder_shares( $folder_id ) );
		$this->assertCount( 1, open_station_stored_files_get_file_shares( $file_id ) );

		// The FOLDER capability resolver must not grant anything from
		// the file share.
		$this->assertSame( 'none', open_station_folder_share_user_capability( $folder_id, self::$recipient_id ) );

		// Folder pending list stays empty for the recipient.
		$pending_folders = open_station_files_get_pending_shares_for_user( self::$recipient_id );
	...
	public function test_folder_purge_does_not_wipe_colliding_file_share() {
		...
		$this->assertTrue( open_station_files_trash_folder( self::$owner_id, $folder_id ) );
		$this->assertTrue( open_station_files_purge_folder( self::$owner_id, $folder_id ) );

		// The file share must survive; the recipient keeps access.
		$this->assertSame( 'accepted', open_station_stored_file_share_state( $file_id, self::$recipient_id ) );
		$this->assertTrue( open_station_stored_file_user_can_read( $file_id, self::$recipient_id ) );
		$this->assertNotNull( open_station_stored_files_get( $file_id ) );
	}
	...
	public function test_folder_hard_delete_does_not_wipe_colliding_file_share() {
		...
		$folder_id = open_station_files_create_folder( self::$owner_id, array( 'name' => 'Collide2' ) );
		open_station_files_place( self::$owner_id, 0, 'folder', (string) $folder_id );
		$file_id = $this->make_stored_file( self::$owner_id );
		$wpdb->update( $tables['stored_files'], array( 'id' => (int) $folder_id ), array( 'id' => $file_id ) );
		$wpdb->update(
		...
		$revoked = array();
		add_action( 'open_station_files_share_revoked', function ( $sid ) use ( &$revoked ) {
			$revoked[] = (int) $sid;
		} );

		$this->assertTrue( open_station_files_delete_folder( $folder_id, self::$owner_id ) );
	...
	public function test_folder_accept_rejects_file_share_row() {
		$file_id  = $this->make_stored_file( self::$owner_id );
		$share_id = open_station_stored_file_share_invite( $file_id, self::$owner_id, self::$recipient_id );
		$err      = open_station_folder_share_accept( $share_id, self::$recipient_id );
		$this->assertWPError( $err );
		$this->assertSame( 'open_station_files_share_not_found', $err->get_error_code() );
	}
	...
	public function test_heartbeat_delta_carries_pending_file_share() {
		...
		$delta = open_station_files_compute_heartbeat_delta( self::$recipient_id, array(), 0, 200, 0 );
	...
	public function test_reinvite_after_deny_goes_back_to_pending() {
		$file_id  = $this->make_stored_file( self::$owner_id );
		$share_id = open_station_stored_file_share_invite( $file_id, self::$owner_id, self::$recipient_id );
		open_station_stored_file_share_deny( $share_id, self::$recipient_id );
		$again = open_station_stored_file_share_invite( $file_id, self::$owner_id, self::$recipient_id );
		$this->assertSame( $share_id, $again );
		$this->assertSame( 'pending', open_station_files_get_share( $share_id )['state'] );
	}
}
