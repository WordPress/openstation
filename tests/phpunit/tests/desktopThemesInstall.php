...
class Tests_OpenStation_DesktopThemesInstall extends WP_UnitTestCase {
	...
	public function tear_down() {
		...
		delete_option( OPEN_STATION_DESKTOP_THEMES_OPTION );
		remove_all_filters( 'open_station_desktop_theme_zip_caps' );
		remove_all_actions( 'open_station_desktop_theme_installed' );
		remove_all_actions( 'open_station_desktop_theme_deleted' );
		parent::tear_down();
	}
	...
	/**
	 * @covers ::open_station_desktop_theme_install_from_zip
	 */
	public function test_installs_a_valid_theme() {
		...
		$dir = open_station_desktop_themes_dir( 'acme-neon' );
	 ...
	 *
	 * @covers ::open_station_desktop_theme_validate_zip
	 */
	...
	public function test_installed_action_fires() {
		$seen = array();
		add_action(
			'open_station_desktop_theme_installed',
		...
		open_station_desktop_theme_install_from_zip(
	...
	public function test_reupload_updates_and_prunes_stale_assets() {
		open_station_desktop_theme_install_from_zip( $this->make_zip( array(
			'theme.json'     => $this->manifest_json( array(
				'textures' => array(
	...
	public function test_reupload_busts_every_asset_url() {
		...
		$first  = open_station_desktop_theme_install_from_zip( $zip );
		$this->assertIsArray( $first, is_wp_error( $first ) ? $first->get_error_message() : '' );
		$before = open_station_shape_desktop_theme_payload_entry( $first, 'upload' );

		// `installedAt` is second-resolution; a real re-upload is
		// always at least a moment later.
		sleep( 1 );
		$second = open_station_desktop_theme_install_from_zip( $zip );
		$after  = open_station_shape_desktop_theme_payload_entry( $second, 'upload' );
		...
		$css = file_get_contents(
			open_station_desktop_themes_dir( 'acme-neon' ) . '/theme.css'
		);
	...
	public function test_unreferenced_assets_are_not_installed() {
		...
		$this->assertFileDoesNotExist(
			open_station_desktop_themes_dir( 'acme-neon' ) . '/stowaway.png'
		);
	}
	...
	public function test_traversal_entry_rejects_the_archive() {
		$error = open_station_desktop_theme_install_from_zip( $this->make_zip( array(
			'theme.json'          => $this->manifest_json(),
			'../../evil.png'      => $this->png_bytes(),
		) ) );
		$this->assertWPError( $error );
		$this->assertSame( 'open_station_desktop_theme_unsafe_entry', $error->get_error_code() );
	}
	...
	public function test_forbidden_extension_rejects_the_archive() {
		...
		$this->assertWPError( $error );
		$this->assertSame( 'open_station_desktop_theme_bad_extension', $error->get_error_code() );
	}
	...
	public function test_missing_manifest_rejects() {
		...
		$this->assertWPError( $error );
		$this->assertSame( 'open_station_desktop_theme_missing_manifest', $error->get_error_code() );
	}
	...
	public function test_invalid_json_rejects() {
		...
		$this->assertWPError( $error );
		$this->assertSame( 'open_station_desktop_theme_bad_json', $error->get_error_code() );
	}
	...
	public function test_entry_count_cap_is_enforced_and_filterable() {
		add_filter( 'open_station_desktop_theme_zip_caps', static function ( $caps ) {
			$caps['max_entries'] = 2;
			return $caps;
		} );
		$error = open_station_desktop_theme_install_from_zip( $this->make_zip( array(
		...
		$this->assertWPError( $error );
		$this->assertSame( 'open_station_desktop_theme_too_many_entries', $error->get_error_code() );
	}
	...
	public function test_total_size_cap_is_enforced() {
		...
		$this->assertWPError( $error );
		$this->assertSame( 'open_station_desktop_theme_archive_too_large', $error->get_error_code() );
	}
	 ...
	 *
	 * @covers ::open_station_desktop_theme_zip_entry_ignored
	 */
	public function test_macosx_and_dotfiles_are_ignored_not_rejected() {
		$entry = open_station_desktop_theme_install_from_zip( $this->make_zip( array(
			'theme.json'              => $this->manifest_json(),
	...
	public function test_svg_script_and_handlers_are_stripped() {
		...
		$clean = file_get_contents(
			open_station_desktop_themes_dir( 'acme-neon' ) . '/icons/x.svg'
		);
	...
	public function test_svg_with_entities_rejects_the_upload() {
		...
		$this->assertWPError( $error );
		$this->assertSame( 'open_station_desktop_theme_bad_svg', $error->get_error_code() );
	}
	...
	public function test_unparseable_svg_aborts_install_and_cleans_up() {
		...
		$this->assertSame( array(), open_station_desktop_themes_index() );
		$this->assertDirectoryDoesNotExist( open_station_desktop_themes_dir( 'acme-neon' ) );

		// No staging directories left behind on the failure path.
		$staging = glob( open_station_desktop_themes_dir() . '/.staging-*' );
	 ...
	 *
	 * @covers ::open_station_desktop_themes_ensure_dir
	 */
	public function test_protection_files_are_exec_off_not_deny_all() {
		open_station_desktop_themes_ensure_dir();
		$base = open_station_desktop_themes_dir();
	...
	public function test_delete_removes_directory_index_entry_and_fires_action() {
		...
		$fired = null;
		add_action(
			'open_station_desktop_theme_deleted',
			static function ( $slug ) use ( &$fired ) {
				$fired = $slug;
			}
		);

		$this->assertTrue( open_station_desktop_theme_delete( 'acme-neon' ) );
	...
	public function test_delete_unknown_slug_is_a_404() {
		$error = open_station_desktop_theme_delete( 'nope' );
		$this->assertWPError( $error );
	 ...
	 *
	 * @covers ::open_station_desktop_theme_rmdir
	 */
	public function test_rmdir_refuses_paths_outside_the_themes_base() {
		...
		$this->assertFalse( open_station_desktop_theme_rmdir( $outside ) );
		$this->assertDirectoryExists( $outside );
		$this->assertFalse(
			open_station_desktop_theme_rmdir( open_station_desktop_themes_dir() ),
