...
class Tests_OpenStation_DesktopThemesInstall extends WP_UnitTestCase {
	...
	public function tear_down() {
		...
		delete_option( OPENSTATION_DESKTOP_THEMES_OPTION );
		remove_all_filters( 'openstation_desktop_theme_zip_caps' );
		remove_all_actions( 'openstation_desktop_theme_installed' );
		remove_all_actions( 'openstation_desktop_theme_deleted' );
		parent::tear_down();
	}
	...
	public function test_installs_a_valid_theme() {
		...
		$dir = openstation_desktop_themes_dir( 'acme-neon' );
	...
	public function test_installed_action_fires() {
		$seen = array();
		add_action(
			'openstation_desktop_theme_installed',
	...
	public function test_reupload_busts_every_asset_url() {
		...
		$first  = openstation_desktop_theme_install_from_zip( $zip );
	...
	public function test_unreferenced_assets_are_not_installed() {
		...
		$this->assertFileDoesNotExist(
			openstation_desktop_themes_dir( 'acme-neon' ) . '/stowaway.png'
		);
	}
	...
	public function test_svg_script_and_handlers_are_stripped() {
		...
		$clean = file_get_contents(
			openstation_desktop_themes_dir( 'acme-neon' ) . '/icons/x.svg'
		);
	...
	public function test_delete_removes_directory_index_entry_and_fires_action() {
		...
		$fired = null;
		add_action(
			'openstation_desktop_theme_deleted',
			static function ( $slug ) use ( &$fired ) {
				$fired = $slug;
			}
		);

		$this->assertTrue( openstation_desktop_theme_delete( 'acme-neon' ) );
	...
	public function test_rmdir_refuses_paths_outside_the_themes_base() {
		...
		$this->assertFalse( openstation_desktop_theme_rmdir( $outside ) );
		$this->assertDirectoryExists( $outside );
		$this->assertFalse(
			openstation_desktop_theme_rmdir( openstation_desktop_themes_dir() ),
