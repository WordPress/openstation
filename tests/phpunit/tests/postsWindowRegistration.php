...
class Tests_OpenStation_PostsWindowRegistration extends WP_UnitTestCase {
	...
	public function tear_down() {
		...
		remove_all_filters( 'open_station_posts_window_user_can_use' );
		remove_all_filters( 'open_station_posts_window_user_can_register' );
		parent::tear_down();
	}
	...
	/**
	 * @covers ::open_station_posts_window_user_can_register
	 */
	public function test_register_gate_open_for_admin_without_opt_in() {
		...
		$this->assertTrue( open_station_posts_window_user_can_register() );
	}
	...
	public function test_register_gate_closed_for_subscriber() {
		...
		$this->assertFalse( open_station_posts_window_user_can_register() );
	}
	...
	public function test_register_filter_can_block_a_capable_user() {
		wp_set_current_user( $this->admin_id );
		add_filter( 'open_station_posts_window_user_can_register', '__return_false' );
		$this->assertFalse( open_station_posts_window_user_can_register() );
	}
	 ...
	 *
	 * @covers ::open_station_posts_window_user_can_use
	 */
	public function test_gate_closed_by_default_until_admin_opts_in() {
		wp_set_current_user( $this->admin_id );
		$this->assertFalse( open_station_posts_window_user_can_use() );

		open_station_save_os_settings(
			$this->admin_id,
			array( 'nativePostsEnabled' => true )
		);
		$this->assertTrue( open_station_posts_window_user_can_use() );
	}
	...
	public function test_filter_can_force_gate_open() {
		...
		add_filter( 'open_station_posts_window_user_can_use', '__return_true' );
		$this->assertTrue( open_station_posts_window_user_can_use() );
	}
	...
	public function test_filter_can_force_gate_closed() {
		...
		add_filter( 'open_station_posts_window_user_can_use', '__return_false' );
		$this->assertFalse( open_station_posts_window_user_can_use() );
	}
	...
	public function test_explicit_user_id_argument_is_honoured() {
		...
		wp_set_current_user( $this->subscriber_id );
		$this->assertTrue(
			open_station_posts_window_user_can_use( $this->editor_id )
		);
	}

	/**
	 * The `open_station_posts_window_query_args` filter is the upgrade
	 * path for v1.1 CPT support — a plugin overrides `post_type` here
	 * and the bundle picks it up without further changes.
	 *
	 * @covers ::open_station_posts_window_default_query_args
	 */
	public function test_query_args_filter_is_applied() {
		...
		$args = open_station_posts_window_default_query_args();
