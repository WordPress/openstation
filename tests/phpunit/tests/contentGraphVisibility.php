 ...
 * @group content-graph
 */
class Tests_DesktopMode_ContentGraphVisibility extends WP_UnitTestCase {
	...
	public function test_nodes_strips_contributor_ids_for_posts_user_cannot_edit() {
		...
		$data = rest_ensure_response( desktop_mode_content_graph_rest_nodes( $request ) )->get_data();
	...
	protected function detail_response( $post_id ) {
		...
		$response = desktop_mode_content_graph_rest_post_detail( $request );
	...
	protected function post_type_count( $slug ) {
		$data = rest_ensure_response( desktop_mode_content_graph_rest_post_types() )->get_data();
