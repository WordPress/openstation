...
class Tests_OpenStation_NonceRefresh extends WP_UnitTestCase {
	...
	public static function wpSetUpBeforeClass( WP_UnitTest_Factory $factory ) {
		...
		// (`open_station_is_enabled()`) lets the payload through.
	...
	/**
	 * @covers ::open_station_nonce_refresh_heartbeat_received
	 */
	public function test_skips_anonymous_users() {
		wp_set_current_user( 0 );

		$response = open_station_nonce_refresh_heartbeat_received( array(), array() );

		$this->assertArrayNotHasKey(
			OPEN_STATION_NONCE_REFRESH_FIELD,
			$response,
	 ...
	 * @covers ::open_station_nonce_refresh_heartbeat_received
	 */
	public function test_skips_users_without_open_station_enabled() {
	...
	public function test_logged_in_user_receives_fresh_nonces_for_default_actions() {
		...
		$this->assertArrayHasKey( OPEN_STATION_NONCE_REFRESH_FIELD, $response );
		$nonces = $response[ OPEN_STATION_NONCE_REFRESH_FIELD ];
	...
	public function test_preserves_pre_existing_response_keys() {
		...
		$response = open_station_nonce_refresh_heartbeat_received(
	...
	/**
	 * @covers ::open_station_nonce_refresh_build_payload
	 */
	public function test_filter_can_add_custom_actions() {
		...
		add_filter( 'open_station_nonce_refresh_actions', $callback );

		$payload = open_station_nonce_refresh_build_payload();

		remove_filter( 'open_station_nonce_refresh_actions', $callback );
	...
	public function test_filter_is_registered_on_heartbeat_received() {
		$this->assertNotFalse(
			has_filter(
				'heartbeat_received',
				'open_station_nonce_refresh_heartbeat_received'
			),
			'open_station_nonce_refresh_heartbeat_received should hook heartbeat_received.'
		);
	}
	...
	public function test_tick_carries_current_user_id() {
		...
		$this->assertArrayHasKey( OPEN_STATION_AUTH_FIELD, $response );
		$this->assertSame(
			self::$user_id,
			$response[ OPEN_STATION_AUTH_FIELD ]['uid']
		);
	}
	 ...
	 *
	 * @covers ::open_station_nonce_refresh_on_expired
	 */
	public function test_expired_path_carries_payload_and_uid() {
		wp_set_current_user( self::$user_id );

		$response = open_station_nonce_refresh_on_expired( array() );

		$this->assertArrayHasKey( OPEN_STATION_NONCE_REFRESH_FIELD, $response );
		$this->assertSame(
			1,
			wp_verify_nonce(
				$response[ OPEN_STATION_NONCE_REFRESH_FIELD ]['wp_rest'],
				'wp_rest'
			)
		);
		$this->assertSame(
	 ...
	 * @covers ::open_station_nonce_refresh_on_expired
	 */
	public function test_expired_path_skips_users_without_open_station() {
		$opted_out = self::factory()->user->create( array( 'role' => 'administrator' ) );
		wp_set_current_user( $opted_out );

		$response = open_station_nonce_refresh_on_expired( array( 'nonces_expired' => true ) );

		$this->assertArrayNotHasKey( OPEN_STATION_NONCE_REFRESH_FIELD, $response );
		$this->assertArrayNotHasKey( OPEN_STATION_AUTH_FIELD, $response );
	...
	public function test_expired_path_filter_is_registered() {
		$this->assertNotFalse(
			has_filter(
				'wp_refresh_nonces',
				'open_station_nonce_refresh_on_expired'
			),
			'open_station_nonce_refresh_on_expired should hook wp_refresh_nonces.'
		);
	}
}
