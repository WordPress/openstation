<?php
/**
 * Tests for the desktop files registry — `open_station_register_file_type()`,
 * `open_station_resolve_file()`, and the seven built-in `Open_Station_File`
 * subclasses.
 *
 * @package WordPress
...
class Tests_OpenStation_Files extends WP_UnitTestCase {
	...
	public function tear_down() {
		remove_all_filters( 'open_station_file_types' );
		remove_all_filters( 'open_station_file_serialize' );
		remove_all_actions( 'open_station_file_type_registered' );
		parent::tear_down();
	}

	/**
	 * @covers ::open_station_register_file_type
	 */
	public function test_built_in_types_are_registered() {
		$types = wp_list_pluck( open_station_get_file_types(), 'type' );
	...
	public function test_register_with_missing_id_returns_wp_error() {
		$result = open_station_register_file_type( '', array(
			'label' => 'X',
			'class' => 'Open_Station_Post_File',
		) );
	...
	public function test_register_with_missing_label_returns_wp_error() {
		$result = open_station_register_file_type( 'no-label', array(
			'class' => 'Open_Station_Post_File',
		) );
		$this->assertWPError( $result );
	...
	public function test_register_with_unknown_class_returns_wp_error() {
		$result = open_station_register_file_type( 'broken', array(
			'label' => 'Broken',
			'class' => 'Definitely_Not_A_Class',
		) );
		$this->assertWPError( $result );
		$this->assertSame( 'open_station_invalid_class', $result->get_error_code() );
	}
	...
	public function test_register_with_class_not_extending_base_returns_wp_error() {
		$result = open_station_register_file_type( 'wrong-base', array(
			'label' => 'Wrong base',
			'class' => 'WP_Post', // Real class, not a Open_Station_File.
		) );
		$this->assertWPError( $result );
		$this->assertSame( 'open_station_invalid_class', $result->get_error_code() );
	}
	...
	/**
	 * @covers ::open_station_resolve_file
	 */
	public function test_resolve_returns_instance_of_correct_class() {
		$post_id = self::factory()->post->create();
		$file    = open_station_resolve_file( 'post', $post_id );
		$this->assertInstanceOf( 'Open_Station_Post_File', $file );
	...
	public function test_resolve_unknown_type_returns_null() {
		$this->assertNull( open_station_resolve_file( 'never-registered', 1 ) );
	}

	/**
	 * @covers Open_Station_Post_File
	 */
	...
	public function test_term_file_uses_taxonomy_id_ref() {
		...
		$file    = open_station_resolve_file( 'term', "category:{$term_id}" );
		$shape   = $file->serialize();
	...
	/**
	 * @covers Open_Station_Bookmark_File
	 */
	public function test_bookmark_file_uses_url_as_ref() {
		$file  = open_station_resolve_file( 'bookmark', 'https://example.com/page' );
		$shape = $file->serialize();
	...
	public function test_bookmark_file_rejects_javascript_url() {
		$file = open_station_resolve_file( 'bookmark', 'javascript:alert(1)' );
		$this->assertFalse( $file->exists() );
	}

	/**
	 * @covers Open_Station_Link_File
	 */
	public function test_link_file_serializes_url_and_host_title() {
		$file  = open_station_resolve_file( 'link', 'https://example.com/path' );
		$shape = $file->serialize();
	...
	public function test_link_file_rejects_javascript_url() {
		$file = open_station_resolve_file( 'link', 'javascript:alert(1)' );
		$this->assertFalse( $file->exists() );
	}

	/**
	 * @covers Open_Station_Embed_File
	 */
	public function test_embed_file_serializes_url_and_host_title() {
		$file  = open_station_resolve_file( 'embed', 'https://example.com/dashboard' );
		$shape = $file->serialize();
	...
	public function test_embed_file_rejects_empty_ref() {
		$file = open_station_resolve_file( 'embed', '' );
		$this->assertFalse( $file->exists() );
	}
	...
	public function test_registered_action_fires_on_success() {
		$calls = array();
		add_action( 'open_station_file_type_registered', static function ( $type, $entry ) use ( &$calls ) {
			$calls[] = array( 'type' => $type, 'entry' => $entry );
		}, 10, 2 );

		// Use a one-off subclass so we don't pollute the built-in registry with a fake.
		eval( 'class Tests_Custom_File extends Open_Station_File { public static function type(): string { return "custom"; } public function title(): string { return "x"; } }' );

		$result = open_station_register_file_type( 'custom', array(
			'label' => 'Custom',
	...
	/**
	 * @covers ::open_station_get_file_types
	 */
	public function test_filter_can_remove_file_type() {
		add_filter( 'open_station_file_types', static function ( $registry ) {
			unset( $registry['comment'] );
			return $registry;
		} );

		$ids = wp_list_pluck( open_station_get_file_types(), 'type' );
		$this->assertNotContains( 'comment', $ids );
	}

	/**
	 * @covers Open_Station_File::serialize
	 */
	public function test_serialize_filter_can_attach_extra_fields() {
		add_filter( 'open_station_file_serialize', static function ( $shape, $file ) {
			$shape['badge'] = 'NEW';
			return $shape;
		}, 10, 2 );

		$post_id = self::factory()->post->create();
		$shape   = open_station_resolve_file( 'post', $post_id )->serialize();
		$this->assertSame( 'NEW', $shape['badge'] );
	}

	/**
	 * @covers ::open_station_build_file_types_payload
	 */
	public function test_payload_shape() {
		$payload = open_station_build_file_types_payload();
