<?php
/**
 * Tests for open_station_chromeless_preserve_redirect() — the wp_redirect
 * filter that re-appends `open_station_chromeless=1` to same-site admin redirects so
...
class Tests_OpenStationChromelessPreserveRedirect extends WP_UnitTestCase {
	...
	/**
	 * @covers ::open_station_chromeless_preserve_redirect
	 */
	public function test_appends_flag_to_admin_redirect_in_chromeless() {
		$this->enter_chromeless();

		$filtered = open_station_chromeless_preserve_redirect( admin_url( 'edit.php' ) );

		$this->assertStringContainsString( 'open_station_chromeless=1', $filtered );
	}
	...
	public function test_leaves_admin_redirect_alone_when_not_chromeless() {
		...
		$this->assertSame( $location, open_station_chromeless_preserve_redirect( $location ) );
	}
	...
	public function test_does_not_double_append_when_flag_already_present() {
		...
		$location = admin_url( 'edit.php?open_station_chromeless=1' );
		$filtered = open_station_chromeless_preserve_redirect( $location );

		$this->assertSame( $location, $filtered );
		$this->assertSame( 1, substr_count( $filtered, 'open_station_chromeless=' ) );
	}
	...
	public function test_leaves_empty_location_alone() {
		...
		$this->assertSame( '', open_station_chromeless_preserve_redirect( '' ) );
	}
	...
	public function test_preserves_existing_query_args() {
		...
		$filtered = open_station_chromeless_preserve_redirect(
	...
	public function test_filter_is_registered_on_wp_redirect() {
		$this->assertSame(
			999,
			has_filter( 'wp_redirect', 'open_station_chromeless_preserve_redirect' )
		);
	}
	 ...
	 * without `open_station_chromeless=1`, which painted the full desktop
	 * shell inside the window. The relative-target branch in
	 * `open_station_is_admin_redirect_target()` must catch it whenever
	 * `is_admin()` is true.
	 *
	 * @covers ::open_station_chromeless_preserve_redirect
	 * @covers ::open_station_is_admin_redirect_target
	 */
	public function test_appends_flag_to_relative_admin_redirect_in_chromeless() {
		set_current_screen( 'user-new' );
		$this->enter_chromeless();

		$filtered = open_station_chromeless_preserve_redirect( 'users.php?update=add&id=42' );
	...
	public function test_appends_flag_to_absolute_path_admin_redirect() {
		...
		$filtered = open_station_chromeless_preserve_redirect( '/wp-admin/users.php?update=add' );
