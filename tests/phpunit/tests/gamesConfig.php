 ...
 */
class Tests_DesktopMode_GamesConfig extends WP_UnitTestCase {
	...
	public function tear_down() {
		...
		remove_all_filters( 'desktop_mode_games_words_url' );
	...
	private function payload_entry( $id ) {
		foreach ( desktop_mode_build_desktop_games_payload() as $row ) {
			if ( $id === $row['id'] ) {
				return $row;
			}
		}
	...
	/**
	 * @covers ::desktop_mode_games_words_url
	 */
	public function test_words_url_points_at_the_shared_asset() {
		$url = desktop_mode_games_words_url();
	...
	public function test_words_url_is_filterable() {
		...
		$this->assertSame( 'https://example.test/custom-words.txt', desktop_mode_games_words_url() );
	}

	/**
	 * @covers ::desktop_mode_games_framework_config
	 */
	public function test_framework_config_is_injected_into_every_game() {
		desktop_mode_register_game( 'cfg-game', array(
			'title'  => 'Config Game',
			'script' => 'cfg-game-script',
		) );
		$entry = $this->payload_entry( 'cfg-game' );
		$this->assertNotNull( $entry );
		$this->assertSame( desktop_mode_games_words_url(), $entry['config']['wordsUrl'] );
	}
	...
	/**
	 * @covers ::desktop_mode_alphabet_soup_register
	 */
	public function test_alphabet_soup_registers_with_score_columns() {
		desktop_mode_alphabet_soup_register();
		$this->assertTrue( desktop_mode_games_is_registered( 'alphabet-soup' ) );
		$entry = desktop_mode_games_registry( 'alphabet-soup' );
		$this->assertSame( 'desktop-mode-game-alphabet-soup', $entry['script'] );
		$this->assertSame(
		...
		$this->assertSame( desktop_mode_games_words_url(), $payload['config']['wordsUrl'] );
	}
}
