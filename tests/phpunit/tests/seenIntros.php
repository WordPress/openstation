...
class Tests_OpenStation_SeenIntros extends WP_UnitTestCase {
	...
	public function tear_down() {
		delete_user_meta( self::$user_id, OPEN_STATION_SEEN_INTROS_META_KEY );
		delete_user_meta( self::$user_id, 'desktop_mode_mode' );
		parent::tear_down();
	}

	/**
	 * @covers ::open_station_get_seen_intros
	 */
	public function test_get_returns_empty_array_for_unconfigured_user() {
		$this->assertSame( array(), open_station_get_seen_intros( self::$user_id ) );
	}
	...
	public function test_get_returns_empty_array_for_invalid_meta() {
		update_user_meta( self::$user_id, OPEN_STATION_SEEN_INTROS_META_KEY, 'garbage' );
		$this->assertSame( array(), open_station_get_seen_intros( self::$user_id ) );
	}

	/**
	 * @covers ::open_station_mark_intro_seen
	 * @covers ::open_station_has_seen_intro
	 */
	public function test_mark_and_has_round_trip() {
		$this->assertFalse( open_station_has_seen_intro( self::$user_id, 'posts' ) );
		$this->assertTrue( open_station_mark_intro_seen( self::$user_id, 'posts' ) );
		$this->assertTrue( open_station_has_seen_intro( self::$user_id, 'posts' ) );
		$this->assertSame( array( 'posts' ), open_station_get_seen_intros( self::$user_id ) );
	}
	...
	public function test_mark_is_idempotent() {
		open_station_mark_intro_seen( self::$user_id, 'posts' );
		open_station_mark_intro_seen( self::$user_id, 'posts' );
		$this->assertSame( array( 'posts' ), open_station_get_seen_intros( self::$user_id ) );
	}
	...
	public function test_mark_appends_distinct_slugs() {
		open_station_mark_intro_seen( self::$user_id, 'posts' );
		open_station_mark_intro_seen( self::$user_id, 'pages' );
		$this->assertSame(
			array( 'posts', 'pages' ),
			open_station_get_seen_intros( self::$user_id )
		);
	}
	...
	public function test_mark_rejects_invalid_input() {
		$this->assertFalse( open_station_mark_intro_seen( 0, 'posts' ) );
		$this->assertFalse( open_station_mark_intro_seen( self::$user_id, '' ) );
		$this->assertFalse( open_station_mark_intro_seen( self::$user_id, '   ' ) );
	}

	/**
	 * @covers ::open_station_clear_seen_intros
	 */
	public function test_clear_wipes_the_list() {
		open_station_mark_intro_seen( self::$user_id, 'posts' );
		open_station_mark_intro_seen( self::$user_id, 'pages' );

		$this->assertTrue( open_station_clear_seen_intros( self::$user_id ) );
		$this->assertSame( array(), open_station_get_seen_intros( self::$user_id ) );
	}

	/**
	 * @covers ::open_station_sanitize_seen_intros
	 */
	public function test_sanitize_drops_garbage_and_dedupes() {
		$out = open_station_sanitize_seen_intros( array( 'posts', 42, 'posts', '', 'PAGES' ) );
	...
	public function test_sanitize_caps_at_max() {
		$big = array();
		for ( $i = 0; $i < OPEN_STATION_SEEN_INTROS_MAX + 10; $i++ ) {
			$big[] = 'slug-' . $i;
		}
		$out = open_station_sanitize_seen_intros( $big );
		$this->assertCount( OPEN_STATION_SEEN_INTROS_MAX, $out );
	}

	/**
	 * @covers ::open_station_rest_mark_intro_seen
	 */
	...
	/**
	 * @covers ::open_station_rest_clear_seen_intros
	 */
	...
	/**
	 * @covers ::open_station_rest_seen_intros_permission
	 */
	 ...
	 * shared `open_station_rest_require_enabled()` gate 403'd the POST, the
	 * slug was never recorded, and the dialog re-appeared on every
	 * classic-admin page load.
	 *
	 * @covers ::open_station_rest_seen_intros_permission
	 */
	public function test_rest_welcome_slug_persists_without_open_station_enabled() {
		...
		$request = new WP_REST_Request( 'POST', '/desktop-mode/v1/intros/seen' );
		$request->set_param( 'slug', OPEN_STATION_WELCOME_INTRO_SLUG );

		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
		$this->assertContains(
			OPEN_STATION_WELCOME_INTRO_SLUG,
			$response->get_data()['seenIntros']
		);
		$this->assertTrue(
			open_station_has_seen_intro( self::$user_id, OPEN_STATION_WELCOME_INTRO_SLUG )
		);
	}
