 ...
 */
class Tests_DesktopMode_StoredFiles extends WP_UnitTestCase {
	...
	public function tear_down() {
		...
		$this->rrmdir( desktop_mode_stored_files_dir() );
		parent::tear_down();
	}
	...
	public function test_ensure_dir_writes_protection_files() {
		desktop_mode_stored_files_ensure_dir( self::$owner_id );
		$base = desktop_mode_stored_files_dir();
		$this->assertFileExists( $base . '/.htaccess' );
		$this->assertFileExists( $base . '/index.php' );
		$this->assertFileExists( desktop_mode_stored_files_dir( self::$owner_id ) . '/index.php' );
	...
	public function test_create_get_delete_roundtrip() {
		...
		$this->assertTrue( desktop_mode_stored_files_delete( $id ) );
		$this->assertNull( desktop_mode_stored_files_get( $id ) );
		$this->assertFileDoesNotExist( $path );
	}

	public function test_create_rejects_bad_disk_name() {
		$result = desktop_mode_stored_files_create( self::$owner_id, array(
			'display_name' => 'x.txt',
			'disk_name'    => '../../evil',
			'size_bytes'   => 1,
			'mime'         => 'text/plain',
		...
		$this->assertSame( 'desktop_mode_stored_files_bad_disk_name', $result->get_error_code() );
	}
	...
	public function test_path_rejects_traversal_disk_name() {
		...
		$this->assertNull( desktop_mode_stored_file_path( $row ) );
	}
	...
	public function test_total_bytes_sums_per_owner() {
		...
		$this->assertSame( 10, desktop_mode_stored_files_total_bytes( self::$owner_id ) );
		$this->assertSame( 2, desktop_mode_stored_files_total_bytes( self::$other_id ) );
	}

	public function test_rename_updates_display_name_and_bumps_placements() {
		$id = $this->make_stored_file( self::$owner_id );
		$placement_id = desktop_mode_files_place( self::$owner_id, 0, 'upload', (string) $id );
		$this->assertIsInt( $placement_id );
		$before = desktop_mode_files_get_placement( $placement_id )['updated_at_ms'];

		// Force a measurable clock delta.
		usleep( 2000 );
		$this->assertTrue( desktop_mode_stored_files_rename( $id, 'renamed.pdf' ) );
		$this->assertSame( 'renamed.pdf', desktop_mode_stored_files_get( $id )['display_name'] );
		$after = desktop_mode_files_get_placement( $placement_id )['updated_at_ms'];
		$this->assertGreaterThan( $before, $after );
	}

	public function test_owner_can_read_stranger_cannot() {
		$id = $this->make_stored_file( self::$owner_id );
		$this->assertTrue( desktop_mode_stored_file_user_can_read( $id, self::$owner_id ) );
		$this->assertFalse( desktop_mode_stored_file_user_can_read( $id, self::$other_id ) );
	}

	public function test_folder_share_reader_can_read_contained_upload() {
		$id        = $this->make_stored_file( self::$owner_id );
		$folder_id = desktop_mode_files_create_folder( self::$owner_id, array( 'name' => 'Shared' ) );
		...
		$share_id = desktop_mode_folder_share_invite( (int) $folder_id, self::$owner_id, 'user', (string) self::$other_id, 'read' );
		$this->assertIsInt( $share_id );
		desktop_mode_folder_share_accept( $share_id, self::$other_id );

		$this->assertTrue( desktop_mode_stored_file_user_can_read( $id, self::$other_id ) );
	}
	...
	public function test_upload_file_type_serializes_size_mime_kind() {
		...
		$file = desktop_mode_resolve_file( 'upload', (string) $id );
		$this->assertInstanceOf( 'Desktop_Mode_Upload_File', $file );
	...
	public function test_upload_file_type_missing_row() {
		$file = desktop_mode_resolve_file( 'upload', '999999' );
	...
	public function test_owner_lock_blocks_non_owner_move_even_with_write_share() {
		$id        = $this->make_stored_file( self::$owner_id );
		$folder_id = desktop_mode_files_create_folder( self::$owner_id, array( 'name' => 'Team' ) );
		desktop_mode_files_place( self::$owner_id, 0, 'folder', (string) $folder_id );
		$placement_id = desktop_mode_files_place( self::$owner_id, (int) $folder_id, 'upload', (string) $id );

		$share_id = desktop_mode_folder_share_invite( (int) $folder_id, self::$owner_id, 'user', (string) self::$other_id, 'write' );
		desktop_mode_folder_share_accept( $share_id, self::$other_id );

		// Writer CAN move a normal placement in the folder, but not
		// the upload.
		$result = desktop_mode_files_move( $placement_id, self::$other_id, array( 'x' => 5 ) );
		$this->assertWPError( $result );
		$this->assertSame( 'desktop_mode_files_upload_owner_locked', $result->get_error_code() );

		// The owner still can.
		$this->assertTrue( desktop_mode_files_move( $placement_id, self::$owner_id, array( 'x' => 5 ) ) );
	}
	...
	public function test_owner_lock_blocks_non_owner_remove_and_trash_gate() {
		...
		$result = desktop_mode_files_remove( $placement_id, self::$other_id );
		$this->assertWPError( $result );
		$this->assertSame( 'desktop_mode_files_upload_owner_locked', $result->get_error_code() );

		$row = desktop_mode_files_get_placement( $placement_id );
		$this->assertFalse( desktop_mode_files_user_can_trash_placement( self::$other_id, $row ) );
		$this->assertTrue( desktop_mode_files_user_can_trash_placement( self::$owner_id, $row ) );
	}
	...
	public function test_purge_of_owners_last_placement_deletes_bytes_and_cascades() {
		...
		$this->assertTrue( desktop_mode_files_remove( $owner_placement, self::$owner_id ) );
	...
	public function test_recipient_placement_removal_keeps_bytes() {
		$id  = $this->make_stored_file( self::$owner_id );
		$row = desktop_mode_stored_files_get( $id );
		desktop_mode_files_place( self::$owner_id, 0, 'upload', (string) $id );
		...
		$this->assertNotNull( desktop_mode_stored_files_get( $id ) );
		$this->assertFileExists( desktop_mode_stored_file_path( $row ) );
	}
	...
	public function test_reconcile_removes_placementless_rows_past_grace() {
		...
		$wpdb->update(
			$tables['stored_files'],
			array( 'created_at_ms' => desktop_mode_files_now_ms() - ( 2 * DAY_IN_SECONDS * 1000 ) ),
			array( 'id' => $id )
		);

		desktop_mode_stored_files_reconcile();

		$this->assertNull( desktop_mode_stored_files_get( $id ) );
		$this->assertFileDoesNotExist( desktop_mode_stored_file_path( $row ) );
	}

	public function test_reconcile_removes_rowless_bytes_past_grace() {
		$dir  = desktop_mode_stored_files_ensure_dir( self::$owner_id );
