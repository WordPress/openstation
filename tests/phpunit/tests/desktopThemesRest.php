...
class Tests_OpenStation_DesktopThemesRest extends WP_UnitTestCase {
	...
	public function set_up() {
		...
		// Both routes sit behind `open_station_rest_require_enabled()`
	...
	public function tear_down() {
		...
		remove_all_filters( 'open_station_desktop_theme_upload_capability' );
		global $wp_rest_server;
		$wp_rest_server = null;
		parent::tear_down();
	}
	...
	/**
	 * @covers ::open_station_desktop_themes_rest_permission
	 */
	 ...
	 * @covers ::open_station_desktop_themes_rest_permission
	 */
	public function test_admin_without_open_station_is_403() {
		delete_user_meta( self::$admin_id, 'desktop_mode_mode' );
		$response = $this->server->dispatch( $this->upload_request( $this->make_zip() ) );
		$this->assertSame( 403, $response->get_status() );
	}
	...
	/**
	 * @covers ::open_station_desktop_theme_upload_capability
	 */
	public function test_upload_capability_is_filterable() {
		add_filter( 'open_station_desktop_theme_upload_capability', static function () {
			return 'edit_posts';
		} );
		wp_set_current_user( self::$editor_id );
		$response = $this->server->dispatch( $this->upload_request( $this->make_zip() ) );
	...
	/**
	 * @covers ::open_station_rest_upload_desktop_theme
	 */
	...
	/**
	 * @covers ::open_station_rest_delete_desktop_theme
	 */
	...
	public function test_non_admin_cannot_delete() {
		...
		$this->assertSame( 403, $response->get_status() );
		$this->assertArrayHasKey( 'acme-neon', open_station_desktop_themes_index() );
	}
	 ...
	 *
	 * @covers ::open_station_register_desktop_themes_rest_routes
	 */
