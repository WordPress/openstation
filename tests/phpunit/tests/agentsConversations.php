...
class Tests_OpenStation_AgentsConversations extends WP_UnitTestCase {
	...
	private function create_conversation( array $messages = array() ) {
		...
		$response = openstation_agents_rest_conversations_create(
			$this->request(
				'POST',
	...
	/**
	 * @covers ::openstation_agent_conversations_register_routes
	 */
	...
	/**
	 * @covers ::openstation_agents_rest_conversations_create
	 */
	public function test_create_derives_title_and_owner() {
		...
		$this->assertSame( OPENSTATION_AGENT_CHAT_POST_TYPE, $post->post_type );
	 ...
	 *
	 * @covers ::openstation_agent_conversation_preview
	 */
	public function test_preview_reads_the_tail_of_the_last_message() {
		$short = openstation_agent_conversation_preview(
			array(
				array(
					'role' => 'user',
		...
		$long = openstation_agent_conversation_preview(
			array(
				array(
					'role' => 'agent',
					'text' => $tail,
		...
		$this->assertSame(
			OPENSTATION_AGENT_CONVERSATION_PREVIEW_CAP + 1,
			mb_strlen( $long )
		);

		$this->assertSame( '', openstation_agent_conversation_preview( array() ) );
	}
	...
	public function test_preview_prefers_the_attachment_title() {
		$preview = openstation_agent_conversation_preview(
			array(
				array(
					'role'       => 'user',
	 ...
	 *
	 * @covers ::openstation_agent_conversation_sanitize_attachment
	 * @covers ::openstation_agent_conversation_prepare
	 */
	public function test_attachments_round_trip_and_are_validated() {
		...
		$this->assertSame(
			'#9',
			openstation_agent_conversation_sanitize_attachment(
				array(
					'kind' => 'user',
					'id'   => 9,
				)
		...
		$this->assertNull(
			openstation_agent_conversation_sanitize_attachment( 'not-an-array' )
		);
	}
	...
	public function test_create_rejects_non_agent_target() {
		...
		$this->assertSame( 'openstation_agent_not_found', $response->get_error_code() );
	}

	/**
	 * @covers ::openstation_agent_conversation_sanitize_messages
	 */
	public function test_sanitizer_filters_roles_and_drops_tool_output() {
		$clean = openstation_agent_conversation_sanitize_messages(
			array(
				array(
					'role'      => 'agent',
					'text'      => 'Done.',
	...
	public function test_sanitizer_caps_message_count() {
		$many = array();
		for ( $i = 0; $i < OPENSTATION_AGENT_CONVERSATION_MESSAGE_CAP + 25; $i++ ) {
			$many[] = array(
				'role' => 'user',
				'text' => "row {$i}",
			);
		}
		$clean = openstation_agent_conversation_sanitize_messages( $many );
		$this->assertCount( OPENSTATION_AGENT_CONVERSATION_MESSAGE_CAP, $clean );
		// Newest rows survive.
		$this->assertSame( 'row ' . ( OPENSTATION_AGENT_CONVERSATION_MESSAGE_CAP + 24 ), end( $clean )['text'] );
	}

	/**
	 * @covers ::openstation_agents_rest_conversations_list
	 */
	public function test_list_is_scoped_to_the_caller() {
		$this->create_conversation();

		$mine = openstation_agents_rest_conversations_list()->get_data();
		...
		$theirs = openstation_agents_rest_conversations_list()->get_data();
		$this->assertSame( array(), $theirs );
	}

	/**
	 * @covers ::openstation_agents_rest_conversations_get
	 * @covers ::openstation_agents_rest_conversations_update
	...
	public function test_foreign_conversations_read_as_missing() {
		...
		$get = openstation_agents_rest_conversations_get(
		...
		$update = openstation_agents_rest_conversations_update(
			$this->request(
				'PUT',
		...
		$del = openstation_agents_rest_conversations_delete(
	...
	public function test_update_replaces_messages_and_retitles() {
		...
		$response = openstation_agents_rest_conversations_update(
			$this->request(
				'PUT',
				"/agents/conversations/{$data['id']}",
				array(
		...
		$reread = openstation_agents_rest_conversations_get(
			$this->request( 'GET', "/agents/conversations/{$data['id']}", array( 'id' => $data['id'] ) )
		)->get_data();
	...
	public function test_delete_removes_the_row() {
		$data = $this->create_conversation();
		$response = openstation_agents_rest_conversations_delete(
			$this->request( 'DELETE', "/agents/conversations/{$data['id']}", array( 'id' => $data['id'] ) )
		);
		$this->assertNotWPError( $response );
	...
	/**
	 * @covers ::openstation_agent_conversations_prune
	 */
	public function test_creating_past_the_cap_prunes_the_oldest() {
		$tighten = static function () {
			return 2;
		};
		add_filter( 'openstation_agent_conversation_cap', $tighten );
		...
		try {
			...
			remove_filter( 'openstation_agent_conversation_cap', $tighten );
		}
	}
	...
	public function test_history_replay_cap_is_fifty() {
		$this->assertSame( 50, OPENSTATION_AGENT_HISTORY_TURN_CAP );
		...
		$clean = openstation_agent_runner_sanitize_history( $many );
