 ...
 */
class Tests_DesktopMode_FilesStore extends WP_UnitTestCase {
	...
	public function test_place_inserts_row_and_fires_action() {
		$fired = 0;
		add_action( 'desktop_mode_file_placed', function () use ( &$fired ) { $fired++; } );

		$id = desktop_mode_files_place( self::$admin_id, 0, 'post', (string) self::$post_id, array(
			'x' => 100,
			'y' => 200,
		) );
		$this->assertIsInt( $id );
	...
	public function test_place_with_unknown_type_returns_error() {
		$result = desktop_mode_files_place( self::$admin_id, 0, 'never-registered', '1' );
		$this->assertWPError( $result );
		$this->assertSame( 'desktop_mode_files_unknown_type', $result->get_error_code() );
	}
	...
	public function test_place_can_be_blocked_by_filter() {
		add_filter( 'desktop_mode_files_can_place', '__return_false' );
		$result = desktop_mode_files_place( self::$admin_id, 0, 'post', (string) self::$post_id );
		$this->assertWPError( $result );
		$this->assertSame( 'desktop_mode_files_forbidden', $result->get_error_code() );
		remove_filter( 'desktop_mode_files_can_place', '__return_false' );
	}
	...
	public function test_move_updates_row_and_fires_action() {
		$id    = desktop_mode_files_place( self::$admin_id, 0, 'post', (string) self::$post_id );
		$fired = array();
		add_action( 'desktop_mode_file_moved', function ( $pid, $next, $prev ) use ( &$fired ) {
			$fired[] = array( 'pid' => $pid, 'next_x' => $next['x'], 'prev_x' => $prev['x'] );
		}, 10, 3 );

		$ok = desktop_mode_files_move( $id, self::$admin_id, array( 'x' => 999 ) );
		$this->assertTrue( $ok );
		$this->assertSame( 999, desktop_mode_files_get_placement( $id )['x'] );
	...
	public function test_move_rejects_non_owner() {
		$id = desktop_mode_files_place( self::$admin_id, 0, 'post', (string) self::$post_id );
		$result = desktop_mode_files_move( $id, self::$other_id, array( 'x' => 1 ) );
		$this->assertWPError( $result );
		$this->assertSame( 'desktop_mode_files_forbidden', $result->get_error_code() );
	}
	 ...
	 *
	 * @covers ::desktop_mode_files_remove
	 */
	public function test_remove_does_not_delete_underlying_entity() {
		$post_id = self::factory()->post->create( array( 'post_status' => 'publish' ) );
		$pid     = desktop_mode_files_place( self::$admin_id, 0, 'post', (string) $post_id );

		$ok = desktop_mode_files_remove( $pid, self::$admin_id );
		$this->assertTrue( $ok );

		// Placement row is gone…
		$this->assertNull( desktop_mode_files_get_placement( $pid ) );
	...
	public function test_folder_delete_does_not_delete_referenced_entities() {
		...
		desktop_mode_files_place( self::$admin_id, $folder, 'post', (string) $post_id );

		$ok = desktop_mode_files_delete_folder( $folder, self::$admin_id );
		$this->assertTrue( $ok );
	...
	public function test_remove_deletes_row_and_writes_tombstone() {
		...
		$ok = desktop_mode_files_remove( $id, self::$admin_id );
		$this->assertTrue( $ok );
		$this->assertNull( desktop_mode_files_get_placement( $id ) );
	...
	public function test_list_returns_only_users_placements_in_folder() {
		desktop_mode_files_place( self::$admin_id, 0, 'post', (string) self::$post_id );
		desktop_mode_files_place( self::$other_id, 0, 'post', (string) self::$post_id );
	...
	/**
	 * @covers ::desktop_mode_files_create_folder
	 */
	public function test_folder_create_round_trips() {
		$id = desktop_mode_files_create_folder( self::$admin_id, array(
			'name' => 'Projects',
		) );
		$this->assertIsInt( $id );
	...
	public function test_folder_rejects_invalid_share_mode() {
		$result = desktop_mode_files_create_folder( self::$admin_id, array(
			'name'       => 'X',
			'share_mode' => 'planet',
		) );
		$this->assertWPError( $result );
		$this->assertSame( 'desktop_mode_files_invalid_share_mode', $result->get_error_code() );
	}
	...
	public function test_folder_update_fires_shared_action_when_share_changes() {
		$id    = desktop_mode_files_create_folder( self::$admin_id, array( 'name' => 'X' ) );
		$fired = 0;
		add_action( 'desktop_mode_folder_shared', function () use ( &$fired ) { $fired++; } );

		$ok = desktop_mode_files_update_folder( $id, self::$admin_id, array(
			'share_mode' => 'all',
		) );
		$this->assertTrue( $ok );
		$this->assertSame( 1, $fired );
	...
	public function test_folder_delete_cascades_placements_and_tombstones() {
		global $wpdb;
		$folder = desktop_mode_files_create_folder( self::$admin_id, array( 'name' => 'X' ) );
		$pid    = desktop_mode_files_place( self::$admin_id, $folder, 'post', (string) self::$post_id );

		$ok = desktop_mode_files_delete_folder( $folder, self::$admin_id );
		$this->assertTrue( $ok );
		$this->assertNull( desktop_mode_files_get_placement( $pid ) );
		$this->assertNull( desktop_mode_files_get_folder( $folder ) );
	...
	public function test_orphan_folder_gets_auto_placed_at_root() {
		$folder_id = desktop_mode_files_create_folder( self::$admin_id, array( 'name' => 'Orphan' ) );
		// Pre-condition: no placements anywhere.
		$this->assertSame( array(), desktop_mode_files_get_for_user_folder( self::$admin_id, 0 ) );

		$placed = desktop_mode_files_auto_place_orphan_folders( self::$admin_id );
	...
	/**
	 * @covers ::desktop_mode_files_auto_place_orphan_folders
	 */
	public function test_auto_place_is_idempotent() {
		desktop_mode_files_create_folder( self::$admin_id, array( 'name' => 'Once' ) );
		$first  = desktop_mode_files_auto_place_orphan_folders( self::$admin_id );
		$second = desktop_mode_files_auto_place_orphan_folders( self::$admin_id );
	...
	public function test_auto_place_skips_folder_with_existing_placement() {
		$folder_id = desktop_mode_files_create_folder( self::$admin_id, array( 'name' => 'Already placed' ) );
		// Place it inside another folder explicitly.
		$parent = desktop_mode_files_create_folder( self::$admin_id, array( 'name' => 'Parent' ) );
		desktop_mode_files_place( self::$admin_id, $parent, 'folder', (string) $folder_id );
	...
	public function test_get_visible_folders_returns_owned() {
		desktop_mode_files_create_folder( self::$admin_id, array( 'name' => 'A' ) );
		desktop_mode_files_create_folder( self::$other_id, array( 'name' => 'B' ) );
		$rows = desktop_mode_files_get_visible_folders( self::$admin_id );
		$this->assertCount( 1, $rows );
