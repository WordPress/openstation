...
describe( 'MioController', () => {
	...
	test( 'the dock toggle gates the companion, launcher and chat without chat enabling it', async () => {
		const { MioController } = await load();
		stubMount();
		vi.stubGlobal( 'ResizeObserver', class { observe() {} disconnect() {} } );
		const host = shell();
		const win = document.createElement( 'div' ); win.id = 'wp-window-a';
		const body = document.createElement( 'div' ); body.className = 'os-window__body';
		win.append( body ); host.append( win );
		const persist = vi.fn();
		const destroy = vi.fn();
		window.openStationMountMioChat = vi.fn( () => ( { destroy } ) );
		const controller = new MioController( { shell: host, bundleUrl: 'https://example.test/mio.js', enabled: false, focusedWindow: () => 'a', persist } );
		const lease = controller.api().registerWindow( 'a', { host: body, title: 'A', documents: [], abilities: () => [], prompt: () => 'A' } );
		const frame = win.querySelector<HTMLElement>( '.os-mio-residence' )!;
		...
		await lease.openChat();
		expect( window.openStationMountMioChat ).toHaveBeenCalledOnce();
		await controller.api().toggle();
		expect( destroy ).toHaveBeenCalledOnce();
		expect( frame.hidden ).toBe( true );
		expect( controller.api().getWindowId() ).toBeNull();
		await lease.openChat();
		expect( persist.mock.calls ).toEqual( [ [ true ], [ false ] ] );
		await controller.api().toggle();
		expect( frame.hidden ).toBe( false );
		expect( button.hidden ).toBe( false );
		lease.dispose(); controller.api().disable();
		delete window.openStationMountMioChat;
		vi.unstubAllGlobals();
	} );
	test( 'first chat and automatic boot share one pending renderer mount', async () => {
		const { MioController } = await load();
		const mount = stubMount();
		let finish!: () => void;
		loadVendorScript.mockImplementation( () => new Promise<void>( resolve => { finish = () => { mount.install(); resolve(); }; } ) );
		const host = shell();
		const win = document.createElement( 'div' ); win.id = 'wp-window-a';
		const body = document.createElement( 'div' ); body.className = 'os-window__body';
		win.append( body ); host.append( win );
		vi.stubGlobal( 'ResizeObserver', class { observe() {} disconnect() {} } );
		const controller = new MioController( { shell: host, bundleUrl: 'https://example.test/mio.js', enabled: true, focusedWindow: () => 'a', persist: vi.fn() } );
		controller.boot();
		const lease = controller.api().registerWindow( 'a', { host: body, title: 'A', documents: [], abilities: () => [], prompt: () => 'A' } );
	...
	test( 'settings synchronize the master without recursively persisting, while dock toggles save', async () => {
		const { MioController } = await load();
		const mount = stubMount();
		vi.stubGlobal( 'ResizeObserver', class { observe() {} disconnect() {} } );
		const host = shell(); const win = document.createElement( 'div' ); win.id = 'wp-window-gated';
		const body = document.createElement( 'div' ); body.className = 'os-window__body'; win.append( body ); host.append( win );
		const persist = vi.fn();
		const controller = new MioController( { shell: host, bundleUrl: 'https://example.test/mio.js', enabled: false, persist, focusedWindow: () => 'gated' } );
		const lease = controller.api().registerWindow( 'gated', { host: body, title: 'Gated', prompt: () => 'Gated', documents: [], abilities: () => [] } );
		controller.boot();
		expect( mount.calls ).toHaveLength( 0 );
		controller.syncEnabled( true );
		await vi.waitFor( () => expect( controller.api().getWindowId() ).toBe( 'gated' ) );
		await vi.waitFor( () => expect( mount.calls ).toHaveLength( 1 ) );
		controller.syncEnabled( false );
		expect( controller.api().isEnabled() ).toBe( false );
		await vi.waitFor( () => expect( host.querySelector<HTMLElement>( '.os-mio-residence' )!.hidden ).toBe( true ) );
		expect( persist ).not.toHaveBeenCalled();
		await controller.api().toggle();
