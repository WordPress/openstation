...
describe( 'presence', () => {
	...
	test( 'heartbeat-send adds wp_desktop_presence_active flag via the bus', async () => {
		const handlers = installFakeJQuery();
		bootHeartbeatBus();
		bootPresenceProbe();
		const data: Record< string, unknown > = {};
		handlers[ 'heartbeat-send' ]?.( {}, data );
		expect( data.wp_desktop_presence_active ).toBe( true );
		expect( typeof data.wp_desktop_user_active ).toBe( 'boolean' );
	} );
	...
	test( 'markActive bumps the lastInputMs so next tick reports active=true', async () => {
		...
		const data: Record< string, unknown > = {};
		handlers[ 'heartbeat-send' ]?.( {}, data );
		expect( data.wp_desktop_user_active ).toBe( true );
	} );
