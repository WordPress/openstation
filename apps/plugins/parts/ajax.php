<?php
/**
 * Plugins app — admin-ajax actions.
 *
 * Part of the `desktop-mode-plugins` app: required by `plugins.os.php`,
 * plain `.php` on purpose — only `*.os.php` files are app entries to
 * the framework loader. Anything that needs an admin-only class lives
 * here, NOT on REST and NOT in an app action (a dispatch is a REST
 * request too).
 ...
 *
 * The Featured tab's handler (`wp_ajax_openstation_plugins_featured`)
 * and its curated slug list are `parts/featured.php`.
 *
 ...
 *
 * Activate / deactivate / delete are the app's own server actions,
 * which run Core's REST controller in-process (`PUT/DELETE
 * /wp/v2/plugins/{plugin}`, `wp-includes/`). No handler needed here.
 *
 * @package OpenStation
 */
