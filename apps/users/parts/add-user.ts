...
import { copyQuietly, generateStrongPassword, toast } from './profile-client';
import type { OsFormElement, ProfileConfig, UsersState } from './types';
...
export function addUserForm( cfg: ProfileConfig ): TemplateResult {
...
function generatePassword( e: Event ): void {
	...
	copyQuietly( pwd );
	toast( __( 'Generated password copied to clipboard.' ), 'success' );
}
 ...
 * a fresh form. Returns the state's `created` counter it painted for.
 */
export function syncAddUserForm( root: HTMLElement, state: UsersState, paintedFor: number ): number {
	const form = root.querySelector< OsFormElement >( '[data-os-users-add-form]' );
	if ( ! form ) {
		return paintedFor;
	}
	if ( state.created !== paintedFor ) {
		form.reset();
		form.clearErrors();
