 ...
 *
 *   - {@see open_station_code_editor_workspace_root()}      — canonical
 *                                                              absolute
 *                                                              workspace
 *                                                              root.
 *   - {@see open_station_code_editor_resolve_path()}        — turns an
 *                                                              untrusted
 *                                                              relative
 *                                                              path into
 *                                                              a canonical
 *                                                              absolute
 *                                                              one.
 *   - {@see open_station_code_editor_extension_allowlist()} — the file-
 *                                                              extension
 *                                                              allowlist.
 *
 * **All filesystem entry points MUST go through
 * {@see open_station_code_editor_resolve_path()}.** Concatenating user
 ...
 */
const OPEN_STATION_CODE_EDITOR_DEFAULT_EXTENSIONS = array(
	'php',
	'js',
	'jsx',
	'ts',
 ...
 * @return bool
 */
function open_station_code_editor_file_edit_allowed() {
 ...
 * @return bool
 */
function open_station_code_editor_user_can_use() {
	$can = open_station_code_editor_file_edit_allowed() && current_user_can( 'edit_plugins' );
	 ...
	 */
	return (bool) apply_filters( 'open_station_code_editor_user_can_use', $can );
}
 ...
 */
function open_station_code_editor_workspace_root() {
	 ...
	 */
	$root = (string) apply_filters( 'open_station_code_editor_workspace_root', $default );
 ...
 * @return string[]
 */
function open_station_code_editor_extension_allowlist() {
	/**
	...
	$exts = (array) apply_filters(
		'open_station_code_editor_extension_allowlist',
		OPEN_STATION_CODE_EDITOR_DEFAULT_EXTENSIONS
	);
 ...
 * @return bool
 */
function open_station_code_editor_extension_allowed( $absolute_path ) {
	...
	if ( '' === $ext ) {
		...
		return in_array( ltrim( $base, '.' ), open_station_code_editor_extension_allowlist(), true );
	}
	return in_array( $ext, open_station_code_editor_extension_allowlist(), true );
}
 ...
 */
function open_station_code_editor_resolve_path( $rel_path ) {
	$root = open_station_code_editor_workspace_root();
	if ( '' === $root ) {
		return new WP_Error(
			'open_station_code_editor_no_workspace',
	...
	if ( preg_match( '/[\\x00-\\x1f]/', $rel_path ) ) {
		return new WP_Error(
			'open_station_code_editor_path_invalid',
	...
	if ( false === $resolved ) {
		return new WP_Error(
			'open_station_code_editor_path_not_found',
	...
	if (
		...
		return new WP_Error(
			'open_station_code_editor_path_outside_workspace',
			__( 'Path resolves outside the workspace root.', 'desktop-mode-code-editor' ),
			array( 'status' => 403 )
		);
	}

	if ( ! open_station_code_editor_extension_allowed( $resolved_norm ) ) {
		return new WP_Error(
			'open_station_code_editor_extension_denied',
 ...
 *   WP_Error 'open_station_code_editor_write_invalid_path'           — empty or
 *           non-string path supplied (500).
 *   WP_Error 'open_station_code_editor_write_target_missing'         — target file
 *           does not exist (404); the editor cannot create new files.
 *   WP_Error 'open_station_code_editor_conflict'                     — caller's
 ...
 *   WP_Error 'open_station_code_editor_filesystem_unavailable'       — host needs
 *           FTP/SSH credentials we don't yet collect.
 *   WP_Error 'open_station_code_editor_write_failed'                 — generic.
 *
 * @param string $absolute_path  Result of {@see open_station_code_editor_resolve_path()}.
 ...
 */
function open_station_code_editor_write_file( $absolute_path, $content, $expected_mtime = 0 ) {
	if ( ! is_string( $absolute_path ) || '' === $absolute_path ) {
		return new WP_Error(
			'open_station_code_editor_write_invalid_path',
	...
	if ( ! is_file( $absolute_path ) ) {
		return new WP_Error(
			'open_station_code_editor_write_target_missing',
	...
	if ( $expected_mtime > 0 && $current_mtime !== $expected_mtime ) {
		...
		return new WP_Error(
			'open_station_code_editor_conflict',
	...
	$context = array(
		'path'  => open_station_code_editor_path_to_relative( $absolute_path ),
		'mtime' => $current_mtime,
	 ...
	 */
	$filtered = apply_filters( 'open_station_code_editor_save_content', $content, $absolute_path, $context );
	 ...
	 * @param array  $context
	 */
	do_action( 'open_station_code_editor_before_save', $absolute_path, $content, $context );

	$fs = open_station_code_editor_get_filesystem();
	if ( is_wp_error( $fs ) ) {
		return $fs;
	}
	...
	if ( ! $fs->put_contents( $absolute_path, $content, FS_CHMOD_FILE ) ) {
		return new WP_Error(
			'open_station_code_editor_write_failed',
	 ...
	 * @param array  $context
	 */
	do_action( 'open_station_code_editor_after_save', $absolute_path, $content, $context );
 ...
 */
function open_station_code_editor_get_filesystem() {
	...
	add_filter( 'filesystem_method', 'open_station_code_editor_force_direct_filesystem', 999 );
	$ok = WP_Filesystem();
	remove_filter( 'filesystem_method', 'open_station_code_editor_force_direct_filesystem', 999 );

	if ( ! $ok || ! ( $wp_filesystem instanceof WP_Filesystem_Base ) ) {
		return new WP_Error(
			'open_station_code_editor_filesystem_unavailable',
...
/**
 * @internal Filter callback for {@see open_station_code_editor_get_filesystem()} — pin to direct.
 */
function open_station_code_editor_force_direct_filesystem() {
	return 'direct';
}
 ...
 * @return string
 */
function open_station_code_editor_path_to_relative( $absolute_path ) {
	$root = open_station_code_editor_workspace_root();
	if ( '' === $root ) {
		return '';
	}
