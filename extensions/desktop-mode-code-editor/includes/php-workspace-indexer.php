 ...
 * `open_station_code_editor_php_index_extra_symbols` filter seam, so
...
const OPEN_STATION_CODE_EDITOR_WORKSPACE_INDEX_VERSION = 2;

/** Transient key for the workspace index. */
const OPEN_STATION_CODE_EDITOR_WORKSPACE_INDEX_KEY = 'desktop_mode_code_editor_workspace_index';

/** TTL — long, but not forever. Stale entries get refreshed on demand. */
const OPEN_STATION_CODE_EDITOR_WORKSPACE_INDEX_TTL = 30 * DAY_IN_SECONDS;

/**
 * Directory + file-name patterns the workspace walker skips.
 */
const OPEN_STATION_CODE_EDITOR_WORKSPACE_DEFAULT_SKIP_DIRS = array(
	'uploads',
	'cache',
	'languages',
	'vendor',
 ...
 */
function open_station_code_editor_get_workspace_index() {
	$cached = get_transient( OPEN_STATION_CODE_EDITOR_WORKSPACE_INDEX_KEY );
	if (
		is_array( $cached ) &&
		isset( $cached['version'], $cached['files'] ) &&
		(int) $cached['version'] === OPEN_STATION_CODE_EDITOR_WORKSPACE_INDEX_VERSION
	) {
		return $cached;
	}
	return array(
		'version' => OPEN_STATION_CODE_EDITOR_WORKSPACE_INDEX_VERSION,
		'files'   => array(),
	);
}

/** Persist the index back to its transient. */
function open_station_code_editor_save_workspace_index( array $index ) {
	$index['version'] = OPEN_STATION_CODE_EDITOR_WORKSPACE_INDEX_VERSION;
	set_transient(
		OPEN_STATION_CODE_EDITOR_WORKSPACE_INDEX_KEY,
		$index,
		OPEN_STATION_CODE_EDITOR_WORKSPACE_INDEX_TTL
	);
}

/** Drop the cache; next read rebuilds. */
function open_station_code_editor_flush_workspace_index() {
	delete_transient( OPEN_STATION_CODE_EDITOR_WORKSPACE_INDEX_KEY );
}
 ...
 */
function open_station_code_editor_iter_workspace_php_files() {
	...
	$skip_dirs = (array) apply_filters(
		'open_station_code_editor_workspace_index_skip_dirs',
		OPEN_STATION_CODE_EDITOR_WORKSPACE_DEFAULT_SKIP_DIRS
	);
	 ...
	 * @param string $regex
	 */
	$skip_re = (string) apply_filters( 'open_station_code_editor_workspace_index_skip_filename_re', '' );
 ...
 */
function open_station_code_editor_scan_workspace_file( $absolute_path ) {
	...
	$rel_path = open_station_code_editor_path_to_relative( $absolute_path );
	$symbols  = array();
	$count    = count( $tokens );
	...
	for ( $i = 0; $i < $count; $i++ ) {
		...
		if ( is_array( $tok ) && T_NAMESPACE === $tok[0] ) {
			$ns               = open_station_code_editor_collect_namespace_name( $tokens, $i + 1 );
			$last_doc_comment = '';
			continue;
		}
		...
		if ( is_array( $tok ) && T_STRING === $tok[0] ) {
			$kind = open_station_code_editor_hook_kind_for_function_name( $tok[1] );
			if ( null !== $kind ) {
				$paren = open_station_code_editor_next_significant_token( $tokens, $i + 1 );
				if ( null !== $paren && '(' === $tokens[ $paren ] ) {
					$arg = open_station_code_editor_next_significant_token( $tokens, $paren + 1 );
					if (
						null !== $arg &&
						is_array( $tokens[ $arg ] ) &&
 ...
 * @internal
 */
function open_station_code_editor_collect_namespace_name( array $tokens, $start ) {
	$count = count( $tokens );
	$parts = array();
 ...
 */
function open_station_code_editor_refresh_workspace_index( $file_budget = 200 ) {
	$index = open_station_code_editor_get_workspace_index();
	$files = is_array( $index['files'] ) ? $index['files'] : array();

	$seen    = array();
	$scanned = 0;

	foreach ( open_station_code_editor_iter_workspace_php_files() as $abs ) {
		$rel          = open_station_code_editor_path_to_relative( $abs );
		$seen[ $rel ] = true;
		...
		$symbols       = open_station_code_editor_scan_workspace_file( $abs );
		$files[ $rel ] = array(
			'mtime'   => $mtime,
			'symbols' => $symbols,
		);
	...
	$index['files'] = $files;
	open_station_code_editor_save_workspace_index( $index );
	return $index;
}
 ...
 */
function open_station_code_editor_refresh_workspace_file( $absolute_path ) {
	$rel = open_station_code_editor_path_to_relative( $absolute_path );
	if ( '' === $rel ) {
		return;
	}
	$index = open_station_code_editor_get_workspace_index();
	...
	if ( ! is_file( $absolute_path ) ) {
		...
		$files[ $rel ] = array(
			...
			'symbols' => open_station_code_editor_scan_workspace_file( $absolute_path ),
		);
	}
 ...
 */
function open_station_code_editor_workspace_index_on_save( $abs ) {
	open_station_code_editor_refresh_workspace_file( $abs );
}
add_action( 'open_station_code_editor_after_save', 'open_station_code_editor_workspace_index_on_save', 10, 1 );
 ...
 * @return array
 */
function open_station_code_editor_workspace_extend_symbols( $pool ) {
	$index = open_station_code_editor_get_workspace_index();

	if ( empty( $index['files'] ) ) {
		$index = open_station_code_editor_refresh_workspace_index();
	}
...
add_filter( 'open_station_code_editor_php_index_extra_symbols', 'open_station_code_editor_workspace_extend_symbols', 10 );
 ...
 * @return array|null
 */
function open_station_code_editor_get_workspace_symbol( $name ) {
	$index = open_station_code_editor_get_workspace_index();
	$name  = (string) $name;
	foreach ( $index['files'] as $entry ) {
