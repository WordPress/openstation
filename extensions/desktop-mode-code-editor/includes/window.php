 ...
 *
 * Window id (`wpdc-editor`), DOM selectors (`data-wpdc-editor-*`),
 * CSS classes (`wpdc-editor*`), and the JS-side config global
 * (`window.wpDesktopCodeEditorConfig`) intentionally retain their
 ...
 * @return string
 */
function desktop_mode_code_editor_monaco_vendor_url() {
	return untrailingslashit(
		DESKTOP_MODE_CODE_EDITOR_URL . 'assets/vendor/monaco-editor/min/vs'
	);
}
...
function desktop_mode_code_editor_register_assets() {
	...
	wp_register_style(
		...
		DESKTOP_MODE_CODE_EDITOR_URL . 'assets/css/code-editor.css',
		...
		file_exists( $css_path ) ? (string) filemtime( $css_path ) : DESKTOP_MODE_CODE_EDITOR_VERSION
	);

	$bundle_url = add_query_arg(
		array( 'action' => 'desktop_mode_code_editor_bundle' ),
	...
	wp_register_script(
		...
		$bundle_url,
		array( 'wp-i18n' ),
		DESKTOP_MODE_CODE_EDITOR_VERSION,
		true
	);
	wp_set_script_translations(
		'desktop-mode-code-editor',
		'desktop-mode-code-editor',
		DESKTOP_MODE_CODE_EDITOR_DIR . 'languages'
	);
}
 ...
 *
 * Hooked on `wp_ajax_desktop_mode_code_editor_bundle`. Outputs:
 *
 *   1. `window.wpDesktopCodeEditorConfig = {...};` — Monaco vendor URL
 *      + REST URLs + nonce.
...
function desktop_mode_code_editor_serve_bundle() {
	...
	$base = trailingslashit( rest_url( DESKTOP_MODE_CODE_EDITOR_REST_NAMESPACE ) );

	$config = array(
		'monacoVendorUrl' => desktop_mode_code_editor_monaco_vendor_url(),
		'pluginUrl'       => untrailingslashit( DESKTOP_MODE_CODE_EDITOR_URL ),
	...
	echo 'window.wpDesktopCodeEditorConfig = ' . wp_json_encode( $config ) . ';' . "\n";

	$suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';
	$bundle = DESKTOP_MODE_CODE_EDITOR_DIR . 'assets/js/code-editor' . $suffix . '.js';
	if ( ! file_exists( $bundle ) ) {
		$bundle = DESKTOP_MODE_CODE_EDITOR_DIR . 'assets/js/code-editor.js';
	}
	if ( file_exists( $bundle ) ) {
...
function desktop_mode_code_editor_render_template() {
	 ...
	 *
	 * Keep the `data-wpdc-editor-monaco` hook (or rename via the
...
function desktop_mode_code_editor_register_window() {
	...
	$window_args = array(
		...
		'template'     => 'desktop_mode_code_editor_render_template',
	 ...
	 */
	$window_args = (array) apply_filters( 'desktop_mode_code_editor_window_args', $window_args );

	$registered = desktop_mode_register_window( 'wpdc-editor', $window_args );
	...
	desktop_mode_register_icon( 'wpdc-editor', $icon_args );
}

/**
 * Enqueue the editor stylesheet for any desktop-mode admin page.
 *
...
function desktop_mode_code_editor_maybe_init_ui() {
	...
	add_action( 'init', 'desktop_mode_code_editor_register_assets', 20 );
	add_action( 'init', 'desktop_mode_code_editor_register_window', 20 );
	add_action( 'admin_enqueue_scripts', 'desktop_mode_code_editor_enqueue_style', 30 );
}
...
// `desktop_mode_code_editor_user_can_use()` is the actual auth gate
// inside the handler.
add_action( 'wp_ajax_desktop_mode_code_editor_bundle', 'desktop_mode_code_editor_serve_bundle' );
add_action( 'plugins_loaded', 'desktop_mode_code_editor_maybe_init_ui', 20 );
