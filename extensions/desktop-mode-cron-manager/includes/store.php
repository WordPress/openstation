...
function openstation_cron_manager_save_custom_schedule( $slug, $interval, $display = '' ) {
	...
	if ( '' === $slug ) {
		return new WP_Error(
			'openstation_cron_invalid_schedule_slug',
	...
	if ( $interval <= 0 ) {
		return new WP_Error(
			'openstation_cron_invalid_interval',
	...
	if ( isset( $existing[ $slug ] ) && ! isset( $custom[ $slug ] ) ) {
		return new WP_Error(
			'openstation_cron_schedule_exists',
	...
	update_option( OPENSTATION_CRON_MANAGER_CUSTOM_SCHEDULES_OPTION, $custom, false );
...
function openstation_cron_manager_register_custom_schedules( $schedules ) {
	...
	foreach ( openstation_cron_manager_get_custom_schedules() as $slug => $entry ) {
...
function openstation_cron_manager_normalize_hook( $hook ) {
	...
	if ( strlen( $hook ) > 191 ) {
		return new WP_Error(
			'openstation_cron_hook_too_long',
...
function openstation_cron_manager_is_json_safe( $value ) {
	...
	foreach ( $value as $k => $v ) {
		...
		if ( ! openstation_cron_manager_is_json_safe( $v ) ) {
			return false;
		}
	}
	return true;
...
function openstation_cron_manager_normalize_args( $args ) {
	...
	if ( ! is_array( $args ) ) {
		return new WP_Error(
			'openstation_cron_invalid_args',
			__( 'Cron args must be a JSON array or object.', 'desktop-mode-cron-manager' ),
			array( 'status' => 400 )
		);
	}
	if ( ! openstation_cron_manager_is_json_safe( $args ) ) {
		return new WP_Error(
			'openstation_cron_unsupported_args',
...
function openstation_cron_manager_format_event( $timestamp, $hook, $args_hash, $event ) {
	...
	$args_editable = openstation_cron_manager_is_json_safe( $args );
	...
	return array(
		'id'              => openstation_cron_manager_event_id( $timestamp, $hook, $args_hash ),
		'identity'        => array(
		...
		'callbackCount'   => openstation_cron_manager_count_hook_callbacks( $hook ),
		'args'            => $args,
		'argsJson'        => $args_json,
		'argsEditable'    => $args_editable,
		'argsSummary'     => openstation_cron_manager_args_summary( $args, $args_editable ),
	);
}
	...
	foreach ( $crons as $timestamp => $hooks ) {
		...
		foreach ( $hooks as $hook => $events ) {
			...
			foreach ( $events as $args_hash => $event ) {
				...
				$out[] = openstation_cron_manager_format_event(
					(int) $timestamp,
					(string) $hook,
					(string) $args_hash,
					$event
...
function openstation_cron_manager_find_event( $identity ) {
	if ( ! is_array( $identity ) ) {
		return new WP_Error(
			'openstation_cron_missing_identity',
	...
	if ( $timestamp <= 0 || '' === $hook || '' === $args_hash ) {
		return new WP_Error(
			'openstation_cron_invalid_identity',
	...
	if (
		...
		return new WP_Error(
			'openstation_cron_event_not_found',
...
function openstation_cron_manager_normalize_event_payload( $payload, $fallback_args = null ) {
	if ( ! is_array( $payload ) ) {
		return new WP_Error(
			'openstation_cron_invalid_payload',
			__( 'Cron event payload must be an object.', 'desktop-mode-cron-manager' ),
			array( 'status' => 400 )
		);
	}

	$hook = openstation_cron_manager_normalize_hook( isset( $payload['hook'] ) ? $payload['hook'] : '' );
	if ( is_wp_error( $hook ) ) {
		return $hook;
	}
	...
	if ( $timestamp <= 0 ) {
		return new WP_Error(
			'openstation_cron_invalid_timestamp',
	...
	if ( array_key_exists( 'args', $payload ) ) {
		$args = openstation_cron_manager_normalize_args( $payload['args'] );
	...
	$custom = isset( $payload['customSchedule'] ) && is_array( $payload['customSchedule'] )
		...
		: null;
	if ( $custom ) {
		$saved = openstation_cron_manager_save_custom_schedule(
	...
	if ( '' !== $schedule ) {
		...
		if ( ! isset( $schedules[ $schedule ] ) ) {
			return new WP_Error(
				'openstation_cron_invalid_schedule',
...
function openstation_cron_manager_schedule_normalized_event( $event ) {
	...
	if ( ! $result ) {
		return new WP_Error(
			'openstation_cron_schedule_failed',
...
function openstation_cron_manager_create_event( $payload ) {
	...
	$scheduled = openstation_cron_manager_schedule_normalized_event( $event );
...
function openstation_cron_manager_delete_event( $identity ) {
	...
	if ( ! $result ) {
		return new WP_Error(
			'openstation_cron_unschedule_failed',
...
function openstation_cron_manager_update_event( $identity, $payload ) {
	...
	$next = openstation_cron_manager_normalize_event_payload( $payload, (array) $found['args'] );
	if ( is_wp_error( $next ) ) {
		return $next;
	}
	...
	$scheduled = openstation_cron_manager_schedule_normalized_event( $next );
	if ( is_wp_error( $scheduled ) ) {
		openstation_cron_manager_schedule_normalized_event( $old );
		return $scheduled;
	}
