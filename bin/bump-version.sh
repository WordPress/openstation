...
# The in-tree `@openstation/types` package mirrors the plugin version
# (its README documents this), so keep it in lockstep.
npm version "$new" --no-git-tag-version --allow-same-version --prefix packages/openstation-types > /dev/null
