...
export class OsMultiselect extends Component {
	...
	appendItems(
		...
		const existing = new Set(
			Array.from( this.querySelectorAll( ':scope > os-option' ) ).map(
	...
	private _openPopover(): void {
		...
		const onPopoverScroll = (): void => {
			...
			if ( sh - ( st + ch ) < 64 ) {
				this.emit( 'os-multiselect-load-more', {} );
			}
		};
	...
	private _closePopover(): void {
		...
		if ( this._popover ) {
			...
			this.requestUpdate();
			this.emit( 'os-multiselect-close', {} );
		}
	}
	...
	private _refreshPopover(): void {
		...
		if ( options.length === 0 ) {
			...
			empty.className = 'os-multiselect__empty';
		...
		if ( selected.size > 0 ) {
			...
			clear.type = 'button';
			clear.className = 'os-multiselect__clear';
		...
		for ( const opt of options ) {
			...
			row.className = 'os-multiselect__option';
		...
		if ( this._loadingMore ) {
			...
			spinner.className = 'os-multiselect__spinner';
...
defineComponent( 'os-multiselect', OsMultiselect );
