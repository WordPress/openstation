...
import { textareaStyles } from './os-textarea.styles';

export class OsTextarea extends Component {
	static props = [
		'label',
		'value',
		'placeholder',
	...
	static help = {
		...
		props: [
			...
			{ name: 'max-rows', type: 'integer (string)', default: '8' },
			{
				...
				description: 'Enter fires os-submit; Shift+Enter inserts a newline.',
			},
		],
		events: [
			{ name: 'os-input-change', description: 'Fires on every keystroke.', detail: '{ value: string }' },
			{ name: 'os-input-commit', description: 'Fires on blur / native change.', detail: '{ value: string }' },
			{
				name: 'os-submit',
		...
		example: html`
			<os-textarea label="Message" rows="3" auto-grow max-rows="8" submit-on-enter></os-textarea>
		`,
	} as const;
	...
	protected render() {
		...
		return html`
			${ label
				? html`<label class="os-textarea__label" for=${ fieldId }>${ label }</label>`
				: html`` }
			<textarea
				id=${ fieldId }
				part="textarea"
	...
	private _onInput( e: Event ): void {
		...
		this.emit( 'os-input-change', { value: ta.value } );
	...
	private _onChange( e: Event ): void {
		const ta = e.target as HTMLTextAreaElement;
		this.emit( 'os-input-commit', { value: ta.value } );
	}
	...
	private _onKeyDown( e: KeyboardEvent ): void {
		...
		if ( e.key === 'Enter' && ! e.shiftKey && ! e.altKey && ! e.metaKey && ! e.ctrlKey ) {
			e.preventDefault();
			const ta = e.target as HTMLTextAreaElement;
			this.emit( 'os-submit', { value: ta.value } );
		}
	}
...
defineComponent( 'os-textarea', OsTextarea );
