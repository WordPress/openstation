...
const sampleColumns: OsTableColumn< User >[] = [
...
describe( '<os-table>', () => {
	...
	test( 'sticky-columns adds is-sticky to header + body cells in the band', async () => {
		host.innerHTML = `<os-table sticky-columns="2"></os-table>`;
		await tick();
		const table = host.querySelector( 'os-table' ) as OsTable< User >;
		table.columns = [
			{ key: 'a', label: 'A' },
	...
	test( 'subTable: prepends an expander column and toggling reveals a nested os-table', async () => {
		host.innerHTML = `<os-table></os-table>`;
		await tick();
		const table = host.querySelector( 'os-table' ) as OsTable< User >;
		...
		table.addEventListener( 'os-table-expand-change', ( e: Event ) => {
			events.push( ( e as CustomEvent ).detail );
		} );
		...
		expect( sub!.querySelector( 'os-table' ) ).not.toBeNull();
	} );
	...
	test( 'selectable=multi: prepends a checkbox column with a select-all in the header', async () => {
		host.innerHTML = `<os-table selectable="multi"></os-table>`;
		await tick();
		const table = host.querySelector( 'os-table' ) as OsTable< User >;
		table.columns = [ { key: 'name', label: 'Name' } ];
		table.data = sampleData;
	...
	test( 'selectable=single: enforces at-most-one selected', async () => {
		host.innerHTML = `<os-table selectable="single"></os-table>`;
		await tick();
		const table = host.querySelector( 'os-table' ) as OsTable< User >;
		table.columns = [ { key: 'name', label: 'Name' } ];
		table.data = sampleData;
	...
	test( 'loading: paints skeleton rows in place of body content', async () => {
		host.innerHTML = `<os-table loading loading-rows="3"></os-table>`;
		await tick();
		const table = host.querySelector( 'os-table' ) as OsTable< User >;
