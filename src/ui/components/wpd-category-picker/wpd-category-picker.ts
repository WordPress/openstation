/**
 * `<wpd-category-picker>` — hierarchical multi-select for taxonomy
 ...
 *
 * Purely presentational + event-driven, like `<wpd-tag-input>`. The
 ...
 *
 *   - `wpd-categories-change` `{ value }` — user toggled a row in
...
import { styles } from './wpd-category-picker.styles';
 ...
 */
export interface WpdCategoryItem {
	id: number;
	name: string;
	parent: number;
}
 ...
 * Posts → Categories UX.
 */
function _isUncategorized( item: WpdCategoryItem ): boolean {
...
export class WpdCategoryPicker extends Component {
	static props = [
		'placeholder',
		'add-label',
		'disabled',
	...
	private _items: WpdCategoryItem[] = [];
	...
	get items(): WpdCategoryItem[] {
		return this._items;
	}
	set items( next: readonly WpdCategoryItem[] | null | undefined ) {
	...
	private get _searchInput(): HTMLInputElement | null {
		return this.shadowRoot?.querySelector< HTMLInputElement >( '.wpd-cat__search' ) ?? null;
	}
	...
	private _renderChipRow(
		...
		// One `<wpd-crumb-chain>` per "leaf selection" — a selected
	...
	private _buildChains(
	...
	): Array< { id: number; segments: WpdCrumbSegment[] } > {
		const byId = new Map< number, WpdCategoryItem >();
		...
		const chains: Array< { id: number; segments: WpdCrumbSegment[] } > = [];
		for ( const leaf of chainLeaves ) {
			...
			const path: WpdCategoryItem[] = [];
			let cursor: WpdCategoryItem | undefined = leaf;
			let safety = 16;
			while ( cursor && safety-- > 0 ) {
	...
	private _renderChain(
		chain: { id: number; segments: WpdCrumbSegment[] },
		readonly: boolean,
		disabled: boolean,
	) {
		...
		// declaratively in the wpd-html template engine — fall back
		// to imperative property assignment.
		const el = document.createElement( 'wpd-crumb-chain' ) as WpdCrumbChain;
		...
		return html`
			...
			>
				<div class="wpd-cat__create-wrap">
					...
					/>
					${ pending
						? html`<span class="wpd-cat__create-spinner" aria-hidden="true"></span>`
						: html`<button
								type="button"
	...
	private _selectedItemsInOrder(): WpdCategoryItem[] {
		const byId = new Map< number, WpdCategoryItem >();
		for ( const item of this._items ) {
			byId.set( item.id, item );
		}
		...
		const real: WpdCategoryItem[] = [];
		const uncatItems: WpdCategoryItem[] = [];
	...
	private _scrollFocusedIntoView(): void {
		queueMicrotask( () => {
			const tree = this.shadowRoot?.querySelector< HTMLElement >( '.wpd-cat__tree' );
			if ( ! tree ) {
				return;
			}
