...
describe( '<wpd-range-field>', () => {
	...
	test( 'an integer step reads out as an integer', async () => {
		host.innerHTML = `<wpd-range-field min="0" max="360" step="1" value="45"></wpd-range-field>`;
		await tick();
		expect( readout().textContent ).toBe( '45' );
	} );

	test( 'a fractional step reads out to the step’s precision', async () => {
		host.innerHTML = `<wpd-range-field min="0" max="3" step="0.05" value="1.4"></wpd-range-field>`;
		await tick();
		expect( readout().textContent ).toBe( '1.40' );
	} );

	test( 'decimals overrides what the step implies', async () => {
		host.innerHTML = `<wpd-range-field min="0" max="360" step="1" decimals="2" value="45"></wpd-range-field>`;
		await tick();
	...
	test( 'the readout box is sized from the range, not the value', async () => {
		...
		host.innerHTML = `<wpd-range-field min="0" max="360" step="0.5" suffix="°" value="5"></wpd-range-field>`;
		await tick();
		const field = host.querySelector( 'wpd-range-field' )!;
	...
	function readout(): HTMLElement {
		return host
			.querySelector( 'wpd-range-field' )!
			.shadowRoot!.querySelector( '.wpd-range-field__value' ) as HTMLElement;
	}
} );
