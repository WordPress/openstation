/**
 * `<wpd-user-search>` — debounced autocomplete picker over the
 * `/desktop-mode/v1/files/users/search` endpoint. Renders a search
 * field; emits `wpd-user-pick { user }` when the user picks a row.
 *
 * Optional attributes:
 *
 ...
 *     `desktopModeConfig.filesUsersSearchUrl`).
 *
...
import { userSearchStyles } from './wpd-user-search.styles';
...
export class WpdUserSearch extends Component {
