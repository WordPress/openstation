/**
 * `<wpd-user-search>` tests.
 */
import { afterEach, beforeEach, describe, expect, test, vi } from 'vitest';

async function load() {
	return await import( './wpd-user-search' );
}
...
describe( 'wpd-user-search', () => {
	...
	test( 'renders the input with placeholder', async () => {
		const el = document.createElement( 'wpd-user-search' );
	...
	test( 'emits wpd-user-pick when a result is clicked', async () => {
		const fetchSpy = vi.fn( async () =>
			new Response(
				JSON.stringify( {
		...
		const el = document.createElement( 'wpd-user-search' );
		const picks: unknown[] = [];
		el.addEventListener( 'wpd-user-pick', ( e ) => picks.push( ( e as CustomEvent ).detail ) );
		document.body.appendChild( el );
		await new Promise( ( r ) => queueMicrotask( () => r( null ) ) );
