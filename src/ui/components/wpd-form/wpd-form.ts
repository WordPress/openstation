/**
 * `<wpd-form>` — declarative, robust, container-query-driven form.
 *
 ...
 * `wpd-form-submit` event whose `detail.values` is a
 ...
 * above it — so `<wpd-form>` adapts to its window's width
 ...
 * cancellable `wpd-form-submit` event is suppressed. Hosts that
 * need richer validation can listen for `wpd-form-input` (every
 * keystroke), call `setFieldInvalid(name)`, or short-circuit
 * inside their `wpd-form-submit` handler.
 *
...
import { wpdFormStyles } from './wpd-form.styles';
...
export class WpdForm extends Component {
	static props = [
		'submit-label',
		'reset-label',
		'error',
	...
	connectedCallback(): void {
		...
		// them as `wpd-form-input`. The host doesn't have to know
		// every field's event name (`wpd-input-change`,
		// `wpd-checkbox-change`, native `input`) — it gets one bus.
		...
		// Pressing Enter inside any descendant `<wpd-text-field>`
		// fires `wpd-submit` — treat it as a form submit so keyboard
	...
	private _captureInitialValues(): void {
		...
		for ( const field of fields ) {
			...
			const isCheckbox =
				field.tagName === 'WPD-CHECKBOX' ||
				field.tagName === 'WPD-CHECKBOX-LABEL' ||
				( field.tagName === 'INPUT' &&
	...
	private _namedFields(): FieldElement[] {
		// Walk the LIGHT DOM — slotted descendants. `<wpd-*>` web
	...
	private _onAnyFieldInput( e: Event ): void {
		...
		// event — components like `<wpd-text-field>` still need it
