...
async function load() {
	return await import( './wpd-modal' );
}

function mount( attrs: Record< string, string > = {}, body: string = '' ): HTMLElement {
	const el = document.createElement( 'wpd-modal' );
...
describe( 'wpd-modal', () => {
	...
	test( 'Escape key cancels and fires wpd-modal-cancel', async () => {
		const el = mount( { open: '', title: 'Hi' } );
		const events: Event[] = [];
		el.addEventListener( 'wpd-modal-cancel', ( e ) => events.push( e ) );
	...
	test( 'wpd-modal-cancel is cancelable — preventDefault keeps the modal open', () => {
		const el = mount( { open: '', title: 'Hi' } );
		el.addEventListener( 'wpd-modal-cancel', ( e ) => e.preventDefault() );
		el.dispatchEvent( new KeyboardEvent( 'keydown', { key: 'Escape', bubbles: true } ) );
		expect( el.hasAttribute( 'open' ) ).toBe( true );
	} );
