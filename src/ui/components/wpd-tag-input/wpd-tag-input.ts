/**
 * `<wpd-tag-input>` — multi-tag picker with autocomplete and free-form
 * creation.
 *
 ...
 *
 *   - `wpd-tag-suggest` `{ query }` — user typed in the input. The
 *     consumer should fetch suggestions and assign them back via
 *     `el.suggestions = [...]`.
 *   - `wpd-tag-add` `{ tag, isNew }` — user picked a suggestion or,
 ...
 *     background.
 *   - `wpd-tag-remove` `{ tag }` — user clicked × on a chip. Same
 *     contract: consumer mutates `value`, runs REST.
 *   - `wpd-tag-open` / `wpd-tag-close` — lifecycle of the inline
 ...
 *
 * ```js
 * const picker = document.querySelector( 'wpd-tag-input' );
 * picker.value = [ { id: 1, label: 'WordPress' } ];
 * picker.creatable = true;
 *
 * picker.addEventListener( 'wpd-tag-suggest', async ( e ) => {
...
import { styles } from './wpd-tag-input.styles';
// Side-effect import — registers `<wpd-chip>` so callers don't need
// to remember to import it separately.
import '../wpd-chip/wpd-chip';
...
export class WpdTagInput extends Component {
	static props = [
		'label',
		'placeholder',
		'add-label',
	...
	static help = {
		title: 'Tag input',
		summary:
			'Multi-tag picker with autocomplete and free-form creation. Purely presentational — emits wpd-tag-suggest / wpd-tag-add / wpd-tag-remove and lets the consumer drive REST + optimistic UI.',
		status: 'experimental',
		since: '0.8.0',
		props: [
			{
			...
			},
			{
				...
				description:
					'Allow Enter on a non-matching query to emit `wpd-tag-add` with `isNew: true`. Off by default — opt in for taxonomies the user is allowed to extend.',
			},
			{
				name: 'removable',
				type: 'boolean attribute',
				description:
					'Show × on every chip and emit `wpd-tag-remove` on click. On by default; switch off for read-only views.',
			},
			{
				name: 'disabled',
				type: 'boolean attribute',
			...
			},
			{
				...
				default: '0',
				description:
					'Minimum query length before `wpd-tag-suggest` fires. Set to 1 or 2 for taxonomies with thousands of terms.',
			},
			{
				name: 'open',
				type: 'boolean attribute',
	...
	private _value: WpdTagItem[] = [];
	private _suggestions: WpdTagItem[] = [];
	...
	private get _input(): HTMLInputElement | null {
		...
		return root ? root.querySelector< HTMLInputElement >( '.wpd-tag-input__input' ) : null;
	}

	// --- Public properties (JS-only) -------------------------------------

	get value(): WpdTagItem[] {
		return this._value;
	}
	set value( next: readonly WpdTagItem[] | null | undefined ) {
	...
	get suggestions(): WpdTagItem[] {
		return this._suggestions;
	}
	set suggestions( next: readonly WpdTagItem[] | null | undefined ) {
				...
				${ tags.map( ( tag, idx ) => {
					...
					return html`
						<wpd-chip
							...
							data-idx=${ String( idx ) }
							@wpd-chip-dismiss=${ ( e: Event ) => this._onChipDismiss( e, tag ) }
			...
			<span class="wpd-tag-input__editor">
				<input
					...
					aria-activedescendant=${ this._highlight >= 0
						? `wpd-tag-suggestion-${ this._highlight }`
						: '' }
	...
	private _onChipDismiss( e: Event, tag: WpdTagItem ): void {
		e.stopPropagation();
	...
	private _addSuggestion( tag: WpdTagItem, isNew: boolean ): void {
