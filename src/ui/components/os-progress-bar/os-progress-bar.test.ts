...
describe( '<os-progress-bar>', () => {
	...
	test( 'renders a progressbar track with default ARIA values', async () => {
		host.innerHTML = `<os-progress-bar value="0"></os-progress-bar>`;
		await tick();
		const track = host
	...
	test( 'fill width matches value/max in determinate mode', async () => {
		host.innerHTML = `<os-progress-bar value="42"></os-progress-bar>`;
		await tick();
		const fill = host
			.querySelector( 'os-progress-bar' )!
	...
	test( 'value over max is clamped', async () => {
		host.innerHTML = `<os-progress-bar value="999" max="200"></os-progress-bar>`;
		await tick();
		const fill = host
			.querySelector( 'os-progress-bar' )!
			.shadowRoot!.querySelector( '.fill' ) as HTMLElement;
	...
	test( 'indeterminate drops aria-valuenow/max and clears inline width', async () => {
		host.innerHTML = `<os-progress-bar value="50" indeterminate></os-progress-bar>`;
		await tick();
	...
	test( 'label populates the header text and aria-label on the track', async () => {
		host.innerHTML = `<os-progress-bar value="10" label="hero.jpg"></os-progress-bar>`;
		await tick();
		const bar = host.querySelector( 'os-progress-bar' )!;
	...
	test( 'show-percent renders the integer percent next to the label', async () => {
		host.innerHTML = `<os-progress-bar value="33" label="x" show-percent></os-progress-bar>`;
		await tick();
		const percent = host
			.querySelector( 'os-progress-bar' )!
	...
	test( 'changing value live re-paints the fill width', async () => {
		host.innerHTML = `<os-progress-bar value="10"></os-progress-bar>`;
		await tick();
		const bar = host.querySelector( 'os-progress-bar' )!;
		const fill = bar.shadowRoot!.querySelector( '.fill' ) as HTMLElement;
	...
	test( 'indeterminate drops the host aria-valuenow / aria-valuemax', async () => {
		host.innerHTML = `<os-progress-bar value="60" indeterminate></os-progress-bar>`;
		await tick();
		const bar = host.querySelector( 'os-progress-bar' )!;
		expect( bar.getAttribute( 'role' ) ).toBe( 'progressbar' );
	...
	test( 'author-provided aria-label on the host wins over `label`', async () => {
		host.innerHTML = `<os-progress-bar value="10" label="Saving" aria-label="Custom"></os-progress-bar>`;
		await tick();
		const bar = host.querySelector( 'os-progress-bar' )!;
	...
	test( 'max <= 0 falls back to indeterminate behavior', async () => {
		host.innerHTML = `<os-progress-bar value="5" max="0"></os-progress-bar>`;
		await tick();
		const track = host
			.querySelector( 'os-progress-bar' )!
			.shadowRoot!.querySelector( '.track' )!;
