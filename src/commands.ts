...
// Words too common to help disambiguate an intent → command match.
const INTENT_STOP_WORDS = new Set( [
	'the', 'a', 'an', 'to', 'my', 'me', 'for', 'of', 'on', 'in', 'and',
	'please', 'open', 'go', 'goto', 'show', 'take', 'let', 'i', 'want',
	'can', 'you', 'this', 'that', 'new',
] );

/**
 * Best-effort match of a natural-language intent to a registered command,
 * by token overlap against each command's label + description. This is what
 * the AI assistant's `run_command` tool uses: the model passes the user's
 * intent, we pick the closest command. Returns null when nothing overlaps.
 *
 * @since 0.9.5
 */
export function matchCommandByIntent( intent: string ): DesktopCommand | null {
	...
	let terms = words.filter( ( w ) => ! INTENT_STOP_WORDS.has( w ) );
	if ( terms.length === 0 ) {
		terms = words;
	}

	const needle = intent.trim().toLowerCase();
	let best: DesktopCommand | null = null;
	let bestScore = 0;
	for ( const c of listCommands() ) {
		...
		// Strong boost when the label literally contains the whole intent.
...
/**
 * Resolve a natural-language intent to a command and run it. Used by the AI
 * assistant's `run_command` tool. Returns the command's result, or an
 * `{ error }` envelope the assistant relays when nothing matched.
 *
 * @since 0.9.5
 */
export async function runCommandByIntent(
	intent: string,
	ctx: CommandContext,
