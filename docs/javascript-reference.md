...
2. For server-side tools, register a WordPress [ability](https://developer.wordpress.org/) with `wp_register_ability()` and add it to the Copilot via the [`desktop_mode_ai_abilities`](./hooks-reference.md#desktop_mode_ai_abilities--stable-since-094) filter. Its `permission_callback` gates execution and input/output are schema-validated by Core.
