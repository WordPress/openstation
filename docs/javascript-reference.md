...
> **Core-update notice.** When a core update's branch has release art the shell shows `<wpd-release-card>` — the release's album sleeve with a CSS-drawn vinyl sliding out, its accent color sampled from the sleeve. This applies to any update in that branch, including a minor (which reuses its major's art). The persistent toast is the fallback when no art is available. The wording follows the server descriptor: crossing into a new major shows the branch version + codename ("WordPress 7.0 "Armstrong" is available"); a same-branch minor shows the exact version with no codename ("WordPress 7.0.1 is available"). Driven by `config.coreUpdate` (`{ version, name, branch, url, release }`) and the `desktop_mode_core_update_notice` / `desktop_mode_core_update_release` PHP filters — see the [Hooks Reference](hooks-reference.md).
