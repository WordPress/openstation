...
> **Core-update notice.** The server reports only *that* a core update is pending (`config.coreUpdate` = `{ version, available, branch, url, crossing }`); the shell resolves the release **art + codename** client-side from the wordpress.org/news announcement (cached in `localStorage`), waits for the art to load, then shows the release card once — the album sleeve with a CSS-drawn vinyl sliding out, accent sampled from the sleeve. Because it waits for art, there's no placeholder flash. When no art can be resolved (unknown release / offline) it falls back to a persistent toast. Wording: crossing into a new major shows the branch version + codename; a same-branch minor shows the exact version. Suppress the whole thing with the `desktop_mode_core_update_notice` PHP filter — see the [Hooks Reference](hooks-reference.md).
