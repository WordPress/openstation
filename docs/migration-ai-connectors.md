...
- The **OpenStation Settings → AI** tab is gone. The per-user **AI assistant** toggle (opt-in, off by default; enable-able only once a provider is configured in Settings → Connectors) now lives in **OpenStation Settings → Features**, alongside **Score new comments with AI**. There is no provider or model picker — provider + model selection is delegated entirely to the Core AI Client.
