...
**Dock placement.** Unified and Spatial sit on the edge named by the `dockPlacement` preference (`bottom` — the default — `left`, or `right`), reflected on each rail as `data-os-dock-placement`. Two layouts ignore it, and `primaryOrientation()` in the dispatcher is the one place that decides so: Classic, whose side bar already owns the left edge (honouring the pick would stack both rails on one side), and OpenStation, whose rail skin is drawn for a horizontal rail and scoped to the bottom placement. OpenStation Preferences paints the Dock position control only for the layouts that read it. The pick is remembered while either of the other two is worn, and applies again the moment the user returns to Unified or Spatial. Moving the dock is a full rebuild (placement reaches a renderer through `mount()`), and fires `os-layout-changed` for the same reason a layout switch does.
