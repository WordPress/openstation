...
Two rules keep this correct when **several windows share one bundle** (Posts, Pages, Users and Profile all ride `os-posts-window`): the synthesized `openStationWindowConfig[ id ]` assignments are grouped **by script handle**, so every entry naming the handle carries the whole set (own window first) and whichever entry fetches the bundle delivers its siblings' configs too — necessary because a shared bundle can serve one window from inside another (the Users window mounts the Profile form, which reads the user-edit config). And on the client, the script *tag* dedupes by URL but each entry's inline data is replayed on that entry's own first open regardless — so the second window onto an already-loaded bundle still gets its data injected before its render callback runs.
