...
- **Agents (Planned)** — a dedicated section inside the *My WordPress* native window, alongside Posts / Pages / Users / Media. Each agent is conceptually a synthetic user with a role, a system prompt, a subset of WordPress 6.9 Abilities API tools, and one or more triggers (drag-and-drop onto the agent tile, double-click chat, listen-to-hook, REST endpoint, listen-to-another-agent). The first PR ships a **UX mock**: four hard-coded agents (Remove BG, Optimize SEO, Send to mail list, Moderate Comments), a read-only Define / Tools / Triggers right pane, and a "+ Create agent" button stubbed to a *Coming soon* dialog — see `src/my-wordpress/agents-mock.ts` and `src/my-wordpress/agents-renderer.ts`. The renderer rides on the existing `kind` extension point (`wp.desktop.myWordpress.registerEntityKind( 'agents', … )`), so the backend follow-up PRs (real persistence, login-block for synthetic users, Abilities-API → AI-tool bridge, chat runtime, trigger plumbing) slot in behind this UI without re-shooting any screens.
