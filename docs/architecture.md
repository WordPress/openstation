...
There were two more, `spatial` and `openstation`, and both are gone: their dispatcher branches, the `dock-core:*` wallpaper-icon synthesis Spatial owned, and the rail skin OpenStation brought. A stored value of either fails validation in `parse()` (`src/settings/state.ts`) and in `openstation_sanitize_os_settings()`, landing on the default, which is the migration for anyone who had picked one. The constellation was written for OpenStation and outlived it.
