...
**The way out is not on the dock.** Exit OpenStation lives in the [tray](javascript-reference.md#the-tray), as its trailing section: it leaves the desktop rather than opening something closeable, and with the admin bar hidden by default it is the only route back to classic admin. The dock is a list of things you open and close, and the one control that does neither kept needing to be drawn as an exception to every rule the rail had — a different silhouette, its own gap, an inverted hover. In the tray it is simply not a launcher, so none of that is needed. `SystemDockItem.locked` survives as a public flag for plugins; nothing built-in sets it.
