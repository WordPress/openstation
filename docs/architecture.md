...
1. `/wp-admin/...` loads → the shell renders in place. The portal forward on `admin_init` fires **only** when `/openstation/` would land the user somewhere else: a network-admin or otherwise non-allowlisted path, or a URL carrying a query arg the portal would strip. For an ordinary admin URL the portal would resolve `?target=` straight back to the page already being served, so the two-bootstrap round trip is skipped and the request renders where it is (override with `openstation_skip_redundant_portal_forward`). When the forward does fire it bounces through `/openstation/?target=<original-url>`.
