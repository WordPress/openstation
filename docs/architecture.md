...
The browser posts to the site and the site forwards to the collector (`includes/feedback/`): ad blockers drop third-party telemetry hosts, the collector then sees the server's IP rather than the person's, and the payload is assembled in PHP where a host can filter it or turn the feature off. The forward is synchronous, three seconds at most, and best-effort — the plugin is about to be deactivated, so no cron callback of ours would ever run. Nothing is stored on the site. The payload has no site id and no URL hash; `readme.txt` lists every field, and the PHPUnit suite pins that list.
