...
- `viewTransition` / `windowTransition` / `viewTransitionDuration` follow the `windowReveal` pattern (same charset, same clamping, same "unknown id resolves to none" stance; both default `'none'` — opt-in), and drive the shell's use of the browser's **View Transitions API** (`src/view-transitions/`). Where a reveal uncovers one window's content, a transition animates a *state change*. **Two settings, one registry, partitioned by a def's `scope`**: `'root'` transitions play on whole-screen changes (`switchDesktop()`, and `OsSettings.apply()` — so every wallpaper / accent / desktop-theme / dock-placement / layout change animates), `'element'` transitions play on window lifecycle (`WindowManager.createWindowAnimated()` for every open, `Window.close()` / `minimize()` / `restore()` / `toggleMaximize()`). Two rather than one because a cube rotation is right for "the screen changed" and absurd for "a window opened", where it would freeze and rotate the whole desk to animate one corner. Each wired surface keeps its original un-animated code path literally unchanged behind a `hasActiveViewTransition( scope )` guard, so every existing behaviour still has an execution path that never touches the module. The **launcher morph** is what makes an open read as caused: `src/view-transitions/launcher.ts` infers the pressed element from the last `pointerdown` (rather than threading a `sourceElement` through every caller — which would need every FUTURE launcher, including a plugin's, to know to pass it), and the player gives it and the new window one shared `view-transition-name` so the browser interpolates the box from icon to window. The subject of a window transition additionally carries `view-transition-class: os-vt-morph`, distinguishing it from the `os-vt-card` every open window wears. Session restore is gated out via the PHP-set `os-area--booting` class, which would otherwise play one transition per restored window at boot. Architecturally the layer is **CSS with a JS trigger, not a JS animation system**: `src/view-transitions/play.ts` starts the transition with a set of view-transition *types* (`os-vt-<id>` plus the def's own extras plus caller context — `os-vt-desktop`, `os-vt-window`, `os-vt-forward`/`-backward`, and the universal `os-vt-on`) and publishes `--os-vt-duration` / `--os-vt-easing` / `--os-vt-x` / `--os-vt-y` on the document element; `assets/css/view-transitions.css` matches those types with `:active-view-transition-type()` and owns every keyframe. A plugin's transition is therefore a registry entry plus a stylesheet, with no animation code and no teardown. Degradation is four-tiered and probed independently: missing `document.startViewTransition` → no animation, mutation still runs; missing view-transition *types* → the id is mirrored onto `<html data-os-vt="…">` and the stylesheet's parallel `:is()` branch matches (context types are lost); missing `element.startViewTransition()` → an `'element'` def runs at the root with identical types; missing `document.activeViewTransition` → an in-flight transition is not skipped explicitly and the browser handles it a frame later. `prefers-reduced-motion` skips the animation outright rather than shortening it. The brand palette is scoped to `body.os-active` and the pseudo-element tree inherits from the *root* element, so the player copies the resolved `--os-mesh-holo` / `--os-ui-accent-dim` / `--os-ui-surface` up to `--os-vt-mesh` / `--os-vt-accent` / `--os-vt-surface` for the run — keeping `variables.css` the sole owner of the palette and declaring nothing new at `:root` for a chromeless document to inherit. Separately and independently, `chromeless.css` carries `@view-transition { navigation: auto; types: os-page-nav }`, which hands *page-to-page navigation inside a window* to the browser as a cross-document view transition; that one lives in the framed document, is not driven by this setting, and is skipped rather than broken when a navigation leaves the shell. Registration is JS-only for now — no `serverViewTransitionScripts` payload entry, so a plugin activated mid-session needs an F5 (the same known gap as reveals and palettes).
