...
| Generic reactive registry + server-sync + REST client primitives. The REST clients whose failures reach the user (notes, desktop files, the app runtime, the agents window, the Drafts and data widgets) throw `RestError` from `api-client.ts` (status, `WP_Error` code, data, the server's message), and `src/core/rest-failure.ts` turns one into the sentence a toast or notice shows; a client that still throws a plain `Error` lands on the surface's generic line: the server's own message for a refusal, a line of its own for offline, an expired session or an unreadable reply, the surface's generic line plus the status for a 5xx. | `src/core/{reactive-registry,server-sync,api-client,rest-failure}.ts` | Stable |
