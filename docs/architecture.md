...
1. `admin_init` — the portal redirect sends a plain admin GET into the desktop: straight to the shell screen (`admin.php?page=openstation`, `includes/shell-screen.php`) with the URL as its `target` when the portal would only hand that URL back, through `/openstation/?target=…` otherwise. A URL carrying the frozen `desktop_mode_portal` flag is an alias and goes to the screen too. The screen itself, windows, solo boots and classic-flagged requests pass through. So does a sub-resource fetch: an `<img>`, a script or an XHR whose URL is an admin page (Jetpack's admin-bar sparkline is `admin.php?page=stats&noheader&proxy&chart=…`) carries a `Sec-Fetch-Mode` other than `navigate`, and gets the bytes it asked for rather than the shell.
2. `admin_body_class` — the `os-active` or `os-chromeless` class is appended so CSS and JS can key off it.
3. `admin_enqueue_scripts` — CSS and JS are registered on a per-mode basis (shell assets on the shell screen, chromeless overrides in iframes). `openstation_is_shell_request()` is the one predicate for "this request paints the shell".
4. `in_admin_header @ 5` — on the shell screen only, the shell markup is injected right after the admin bar (`<div id="os-shell">`). No other screen's scripts, render callback or HTML reach the shell document.
...
1. `/wp-admin/...` loads → `admin_init` redirects into the desktop. For an ordinary admin URL the portal would resolve `?target=` straight back to the page being served, so the request goes straight to the shell screen, `admin.php?page=openstation&target=<original-url>&intent=1`, in one hop (route through `/openstation/` anyway with `openstation_skip_redundant_portal_forward`). It bounces through `/openstation/?target=<original-url>` **only** when the portal might land the user somewhere else: a network-admin or otherwise non-allowlisted path, or a URL carrying a query arg the portal would strip. A URL tagged with the frozen `desktop_mode_portal=1` flag — an old bookmark, a hand-built link — is an alias for the desktop and goes to the screen with that URL as its target.
2. `/openstation/` forwards to the shell screen: bare when there is no `target`, so the screen resolves the entry itself (the saved session's focused window, the default-window preference, the Dashboard); with `target=<admin path>&intent=1` when the redirect resolved from a user-supplied `target` (admin-bar link, bookmark, etc.). The portal answered to `/desktop-mode/` before the rebrand and still does, so bookmarks of that address keep working — it is an alias, not a second canonical URL: `openstation_portal_url()` always emits `/openstation/`. The screen re-validates `target` on read; `currentPage` is the result, and the booleans `fromPortal` / `fromPortalIntent` ride along in the shell config so the boot flow can distinguish "the screen picked this page" from "the user actually asked for it." Validation refuses two URLs that resolve but must not become a window: the shell screen itself (it would boot a second desktop in an iframe) and a `page`-less `admin.php` (core's bootstrap with nothing to dispatch to — 200 with an empty body). Either falls back to the entry resolver.
3. The shell screen renders the shell and nothing else: OpenStation's own assets plus Core's every-admin-page baseline, no host screen's scripts, render callback or hidden HTML (`openstation_shell_dequeue_handles` lets a site name globally-enqueued handles it never wants there). The entry window opens `currentPage` (Station Home through the Dashboard URL remap when the user has opted in via `stationHomeEnabled`, the classic Dashboard iframe otherwise; the user's saved focused window, the default-window preference, or the `target` URL take precedence), titled after its dock entry until the iframe reports its own title.
4. The shell's Vite-built TypeScript bundle (`desktop.js` in dev, `desktop.min.js` in prod) initializes:
   ...
   - Strips the consumed `target` / `intent` args from the address bar (`shellUrlWithoutBootArgs()` in `src/shell-url.ts`, one `history.replaceState()`), leaving the canonical `admin.php?page=openstation`. They are a one-shot boot instruction, already read server-side into `currentPage` / `fromPortalIntent`; left in the URL, every reload re-opens that page on top of the restored session. The URL is otherwise left alone — normalising it to `/openstation/` was tried and reverted, since that route costs a redirect, and an address-bar flash, on every reload.
...
**The work area.** A side dock is a flex sibling of `#os-area`, so the area is already narrower for it; the bottom dock is a pill that floats *over* the area's lower band. Every surface that places content used to reserve that band on its own guess (an 80px padding, a 100px canvas margin, nothing at all), so `src/work-area/index.ts` now measures the area and every `.os-dock` in the shell body, works out which bands the chrome overlays, and publishes one rectangle: as `--os-work-area-inset-*` custom properties on `#os-shell` for stylesheets, as `wp.os.workArea` for scripts, and as the `os-work-area-changed` event once per actual change. Every default placement reads it — window open and restore, cascade and tile, the widget clamp, the icon grid, sticky notes, the graphs' Fit — and maximize and snap use the same safe area, reflowing on dock size, placement and behavior changes. Fullscreen and deliberate manual placement still use the whole desktop. Only chrome that floats over the area claims a band; the admin bar needs no case (the shell already starts below it — below its *measured* bottom edge, `--os-admin-bar-height` from `src/admin-bar-height.ts`, rather than Core's 32px token, so a host that makes the bar taller or pushes it down cannot paint it over the title bars), and a rail set to the **Dynamic** behavior (Preferences → Appearance → Desktop layout, Dock behavior and, in Split, Sidebar behavior; `data-os-dock-behavior` on the rail, `src/dock-behavior.ts` for the edge reveal) folds into a thin indicator line at its edge and claims nothing either. Details in [`javascript-reference.md`](./javascript-reference.md#workarea--experimental).
...
The shell's own **Station Home** is the opt-in replacement for the ordinary `index.php` Dashboard destination (`stationHomeEnabled` in OS Settings → Features, default off — the classic Dashboard iframe is what a user gets until they opt in). It is an [App Framework](./app-framework.md) app (`apps/station-home/`, window id `desktop-mode-dashboard`) painted entirely on the server — no bundle, no REST routes of its own; the body is re-rendered through the app dispatch route on every interaction and morphed in place. It keeps the existing URL as its entry contract through `registerNativeUrlRemap()` (the matcher lives in `src/open-targets/station-home-url.ts`), while `desktop_mode_classic=1` deliberately falls through to a classic iframe window. Plugins extend the home surface through the structured PHP card registry (`openstation_register_station_home_card()`), not raw HTML: registry metadata always reaches the per-user picker, but a card's callback runs only when that user has enabled it. Explicit on/off choices live in `openstation_station_home_card_preferences`, written by `openstation_station_home_set_card_preference()` — which is what the app's Customize switches call. **OpenStation Preferences** (wallpaper / accent / layout / themes / effects / navigation / features) is an App Framework app too (`apps/os-settings/`, window id `desktop-mode-os-settings`, see [`migration-os-settings-app.md`](./migration-os-settings-app.md)) — the reference for an app that edits *shell* state: the settings store stays in the shell bundle (it applies before the first paint), and the app's client view reads and writes it through the public `wp.os.getOsSettings()` / `updateOsSettings()` API. Lifecycle hooks — `os.native-window.before-render` (filter), `after-render`, `before-close` — let a plugin decorate or wrap another plugin's render output.
...
Those registries live in a `createSharedStore` (`desktop-mode/window-channels`) because the module compiles into both the main bundle and the lazy `window-system` one — `createWindowElement()` marks a window loading from the latter, while `native-windows.ts`' synthetic-iframe readiness signal and `connection/index.ts`' subscriber registration run in the former. On module-level state the two halves kept separate books and every pairing failed silently in one direction: a window whose loading mark and ready signal landed in different copies never fired `WINDOW_CONTENT_LOADED` and sat under its loading overlay for good.

`src/connection/index.ts` straddles the same seam and shares its registries the same way (`desktop-mode/connection`). The bridge runs in the shell, but `Window.send()` reaches in for `getSyntheticIframe()` to decide whether a window's payload goes out by `postMessage` or in-process — and for an `iframeContent` native window the answer lives in a registry the shell wrote. Split across two copies, `send()` resolved no target and routed to `dispatchToNative()`, where a shell-synthesised body has no subscriber to receive it.

`tests/vitest/bundle-shared-state.test.ts` checks each key is present in both built bundles; `window-channels-cross-bundle.test.ts` and `connection-cross-bundle.test.ts` drive the seams themselves, built with the `loadTwoBundleCopies()` helper in `tests/vitest/helpers/bundle-seam.ts`.
...
A window that was grid-snapped also carries a **`gridSpan`** — `{ anchor, cursor, cols, rows }`, its placement in cells. On restore the cells outrank the saved pixels: a span is a fraction of the desk, the pixels are from whatever display the session was written on. `openstation_sanitize_session_grid_span()` accepts a span only when every value is an integer, the grid is within 1×1–24×24, and both cells fit it; anything else is dropped and the window restores on its pixels, as one written before grid snap does.
...
**What a harvested command gets when it runs.** A `navigate`-classified command never executes its callback at all — the shell opens the extracted URL as a window (via the native-URL remap registry first). An `action`-classified one is dispatched by calling the callback the store handed us, with the arguments WordPress documents: `callback( { close } )`, where `close` is the palette's real close, so a command that dismisses the overlay before doing its work behaves the same here as in Core's palette. **A callback that throws is a command failure**, not a silent no-op: it propagates to the palette, which renders `Command /<slug> failed: <message>` and fires `HOOKS.COMMAND_ERROR`. That matters more than it sounds — a plugin's command can list, highlight and accept Enter and still throw on its first statement (a helper its own script never loaded, say, because only the palette family is replayed into the shell), and swallowing that made the command indistinguishable from one that ran and did nothing.

The classifier remains the only defence against a callback navigating the shell out of the window model, and it is textual: it reads the callback's own source. **A handler that navigates through a helper — `callback: ( a ) => goTo( url, a.close )` — names no sink, classifies as an action, and will navigate for real when it runs.** There is no runtime net under that, and there cannot be: `location` is `[LegacyUnforgeable]`, so `Object.defineProperty( document | window, 'location', … )` throws `TypeError: Cannot redefine property: location` and any sandbox built on it is inert. Plugin authors who want a command to open a desktop window instead of unloading the shell should either write the destination as a string literal in the callback (which the classifier extracts) or register through `wp.os.registerCommand()` and call `ctx.openInWindow()`.
...
- **Ask AI** — natural-language questions routed through `/ai/search` (read-only [Abilities](hooks-reference.md) + content search). It **suggests** (answers, entity cards, `admin_links` the user clicks) and never auto-runs a command. Answers only when a provider is configured *and* the toggle is on; until then the mode shows a prompt to set it up, which opens Preferences → Features (or, on a site without WordPress's AI APIs, says AI isn't available).

A **mode switch** at the end of the input row flips between them (replacing the `/` shortcut) and is always shown, so the assistant looks the same whether or not AI is set up. Each mode keeps its own input draft, and the last AI answer is re-shown when returning to Ask AI. The **OpenStation Preferences → Features → "AI assistant"** toggle (`ai.enabled`, off by default, provider-gated) enables Ask AI and makes it the default mode on open (off → Commands). The overlay reads AI support, provider status and the toggle live via `AiAssistantConfig.isAiSupported()` / `isAiAvailable()` / `isOverrideEnabled()`, so connecting a provider or flipping the toggle takes effect on the next open without a reload.

Besides ⌘K and Core's admin-bar icon, the overlay has a dock tile: **Site assistant** (`os-site-assistant`), the first tile of the shell's trailing cluster, wearing the brand's copilot sparkle. It is placeable, so Preferences → Navigation can move or hide it.
...
**OpenStation Network** — opt-in site-wide, **off by default**: the `network` extended option (OpenStation Preferences → Features → Extended options; filter `openstation_network_enabled`) gates the whole module in `includes/network/bootstrap.php` on `plugins_loaded` — while off, none of the network PHP loads (no keypair, no routes, no registry, no hop token) and the Network window stays out of the app registry; a multisite keeps the site switcher it has on its own. See [network.md](./network.md).
...
**Mobile — the phone layer** — the shell's third experience. `src/mode/` resolves `wp.os.mode` (`desktop | tablet | mobile`) from the viewport (`matchMedia`, so only a crossing costs anything) and the per-user `mobileLayout` preference, stamps it on `<html data-os-mode>` — the one selector every mode-aware sheet keys on — and fires `os.mode.changed` / `os-mode-changed`. A few bytes of inline script PHP prints in `<head>` (`openstation_print_mode_stamp()`, `includes/mobile.php`) write the same attribute from the same rule before the body parses, so a phone's first paint is already the phone layer. When the mode is `mobile`, the main bundle fetches `mobile[.min].js` (`src/mobile/loader.ts`, the window-system pattern) and mounts the layer over the SAME window manager: one `os.window.geometry` filter (`src/mobile/constraints.ts`, main bundle) forces every window `maximized` while keeping the displaced geometry; the window's own title bar hides under a shell-level top bar; "home" is `minimizeAll()` and the layer's state is derived from the manager, never stored; the home grid is `wp.os.getNav()` folded together; the tab bar's pins are the `mobileTabs` setting or the `openstation_mobile_tab_bar` default; the switcher is `manager.getAll()` as swipeable cards plus the *recents*. The session diet is the performance decision: a phone boot restores only the focused session window (one iframe, not a desktop's worth), parks the rest as recents, and a new `os.session.snapshot` filter — run last in `WindowManager.snapshot()` — writes the displaced geometry and the parked windows back, so a desktop reload after a phone visit finds exactly what it left. Wallpapers are suspended and widgets are not hydrated while the layer is up. `tablet` is reported and renders the desktop for now. See [mobile.md](./mobile.md).
...
- **Mobile, next** — pull-to-refresh (needs the chromeless bridge to report overscroll from inside the iframe), and a device pass on real phones.
- **Tablet hybrid** — split view, slide-over overlay, horizontal bottom dock, optional OpenStation toggle for large tablets — on the `wp.os.mode` primitive the phone layer shipped.
...
The per-user `mioEnabled` preference (default `false`, synchronized with its `mioApiEnabled` alias in the existing `desktop_mode_os_settings` user-meta blob) is shared by the dock and Features → MIO API. The independent `mioShowOnWallpaper` preference (default `true`) hides or shows the desktop mascot without disabling window integrations. The shell’s MIO residency coordinator accepts explicit per-window leases, follows focus, and reparents the one canvas through a cancellable scale transition. Resident physics use only the window body boundaries. Idle residents are invisible until chat or a caller-authored control callout needs the mascot. Callout requests, target resolvers and dismissal ids live in the lease and are erased on disposal. Caller-owned dynamic prompts, Markdown documents and private abilities form an in-memory session per lease. A sequential client tool loop calls authenticated `POST desktop-mode/v1/mio/turn`; the server uses the configured AI Client to generate intentions and never dispatches those writes. The client revalidates against the live window allowlist and invokes ordinary app/REST actions. Per-turn validation budgets permit bounded corrections before writes. Effect metadata separates reads from confirmed or uncertain writes; a bounded, memory-only operation ledger can reconcile through an app-owned read-only status endpoint. UTF-8 request budgets are checked before transport, with caller-owned history compaction for large documents. No WordPress Abilities registration, new option, user-meta key, database table or persistent backscroll is introduced. Ask MIO requires the AI opt-in and a compatible connector on both client and server; callouts do not need AI. The chat mounts from the lazy Mio bundle; Preferences bundles its own Markdown help. See [Window-scoped MIO](./mio-window-assistant.md).

MIO response controls are app-owned read/navigation callbacks attached once to settled messages using confirmed operation metadata. A bounded lease-local registry holds executable descriptors; conversations carry only opaque message/action IDs, and provider history strips those references. Chat close aborts work, message eviction releases callbacks, and lease disposal clears the registry. See [the response-button contract](mio-window-assistant.md#assistant-response-buttons). No server transport or persistence layer is added.
