...
### App windows — `.osx.php` (Experimental)
...
- **Definition.** `OpenStation\App::define( $id )` — title, icon, size, title-bar buttons, ⋯ rows, per-window chrome, a typed `state( $defaults )`, `action()` handlers, and a `view()` that paints `<os-*>` markup. An `.osx.php` file `return`s one. `App::manifest()` is the whole window as data.
