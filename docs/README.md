...
13. **[Desktop Themes](./desktop-themes.md)** — *Experimental (since 0.9.7).* Whole-OS reskins uploaded as a ZIP of `theme.json` plus images: every design token, the title-bar / dock / desktop textures, the window frame and corners, and a complete iconset down to the window control glyphs. No author CSS or JS ever executes — PHP validates the manifest and compiles the stylesheet. Read before authoring a theme, or before touching the texture tokens in `variables.css`. See also [examples/register-desktop-theme.md](./examples/register-desktop-theme.md).
