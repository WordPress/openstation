...
16. **[Migration — activity channels move to `os/` (1.0.0)](./migration-activity-channels.md)** — the eleven framework-published activity channels drop the pre-rebrand `desktop-mode/` prefix. No alias ships: a subscriber left on an old slug stops firing silently. Read if you subscribe to or filter any built-in channel.
