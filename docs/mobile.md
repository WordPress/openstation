...
**The top bar** is the app's identity — icon and title — and two round controls: minimize, which sends the app to the background with its window kept for the switcher, and ×, which closes it. Both morph the app back into its tile, and both act in the same frame: × minimizes first and lets the close handshake (an iframe page may be asked about unsaved changes) finish behind the transition, so the screen never waits on it. A flick down on the bar is minimize too. There is no Back and no menu: going home is also the system's job on a phone (the tab bar's Home, the edge swipe, the hardware Back), and reload is the pull-to-refresh that is still to come. The window's own title bar, its controls and its plugin-registered buttons are not shown.

## The session on a phone

Restoring a desktop's worth of windows on a phone would mean a desktop's worth of iframes. A phone boot restores **one** window — the session's focused one — and parks the rest as *recents*: cold cards in the switcher that open on tap.

The desktop is not degraded by this. `WindowManager.snapshot()` runs the `os.session.snapshot` filter last, and the phone layer uses it to hand the desktop its own numbers back: a window the phone forced full-screen is written with the geometry and state it had before, and the parked recents are folded back into every save until they are opened. A desktop reload after a phone visit finds exactly what it left.
