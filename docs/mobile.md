...
**The home screen** is a grid of tiles from `wp.os.getNav()` — the rails, the sidebar and the wallpaper icons folded together, deduplicated, in the user's own order, minus the entries that are on a rail only because their window is open (those belong to the switcher). A search field above it filters by title; Enter opens the first match. Badges are the dock's badges. The grid always scrolls by at least one pixel: a phone only rubber-bands a surface that can scroll, so a home screen that fits its screen would otherwise be a dead surface under the thumb. The overflow is structural (`.os-mobile-home__content`), and the bounce is the platform's own.
...
- **Trash**'s toolbar takes three rows — the type filter scrolling sideways under the search, the selection actions sharing a row, Empty Trash keeping the last row's end.
- **WP Explorer**'s preview pane becomes a sheet along the bottom, at most 45% tall, present only while something is selected. On a phone (the mode stamp), and under any coarse primary pointer, **one tap opens**: a row or tile opens its editor (a person, their footprint), a folder tile opens the folder. The Finder semantics — click selects, double click opens — stay everywhere a pointer can double click.
...
- **Posts, Pages and Users** offer the status filter as an `<os-select>` in place of the pill bar (same data attribute, same `os-pick`; `src/posts-window/status-control.ts`), the toolbar gives the filter and the search a row each, and the table shows the title and the date (users: name, role, actions). The narrowing runs after the user's hidden-column list and the columns filter, so a phone's rule hides nothing on the desk.
- **Plugins** does the same for its table (name, status, actions) and drops the sticky name column, which on a phone had nothing to scroll under it.
