...
**Its tabs are the Posts menu, in both directions.** All posts, Add Post, Categories and Tags — the same rows the dock's Posts submenu lists, in the same order — so the remap claims `post-new.php` and `edit-tags.php?taxonomy=category|post_tag` alongside `edit.php` and passes the tab each one stands for as the window's open-time `{ tab }` param (the URL → tab mapping is `src/open-targets/posts-tabs.ts`; the app reads it on `mount` and on `reopen`). Add Post shows the editor embedded in the panel through [`wp.os.embedAdminPage()`](./javascript-reference.md#wposembedadminpage-host-url-opts---stable), re-mounted on every visit so it opens blank the way `post-new.php` does in a classic window. Two consequences worth knowing: those URLs route to the window **wherever** they are clicked, the admin bar's "+ New" included, the same way the Users opt-in claims every `user-edit.php`; and an embedded editor is not an iframe window, so it has no close-time unsaved-changes prompt. Pages works the same minus the taxonomies, and adds the reverse case: **Page atlas** is a tab wp-admin has no screen for, and the dock offers it as a row anyway, because the submenu is built from the window's declared tabs rather than from `$submenu`. That is `App::menu()`, and it is how every window that replaces a menu keeps the two lists equal — present only while the opt-in is on, which is why flipping a Beta toggle spends a menu refresh.
