...
|---|---|---|
| `wp.os.openOsSettings( opts? )` | `void` | Portable opener for the shell's OS Settings window — same window the dock tile opens. Avoids the Classic-layout gotcha where the OS Settings tile lives on a different rail than your custom renderer. Pass `{ tabId }` (e.g. `'ai'`, `'features'`) to deep-link to a specific tab. |
| `wp.os.listSystemTiles()` | `Array<{ id, title, icon, affinity, placeable }>` | Enumerate every JS-registered system tile (OS Settings, Mio toggle, plugin native-window launchers). Compose your own launcher palette without scraping the DOM. |
