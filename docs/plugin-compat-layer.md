...
5. Every menu URL those rules classify resolves through `openstation_menu_admin_url()`, never through `self_admin_url()`. That function's filter receives the path, and on the default WordPress.com interface wpcomsh uses it to answer `plugin-install.php` with the Calypso installer. Resolved through it, the original in rule 4 reads as off-site too and is dropped along with its replacement, leaving Plugins with no Add Plugin row.
