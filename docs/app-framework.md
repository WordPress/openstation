...
Eleven of OpenStation's own windows are apps — the three reference apps below, Trash and WP Explorer (`apps/trash/`, `apps/my-wordpress/`), and the six list windows ported together (see [`migration-list-apps.md`](./migration-list-apps.md)): **Posts** and **Pages** (`apps/posts/`, `apps/pages/` — the list-window shape: a `data()` over `openstation_app_rest_page()`, paging / filtering / sorting as state, the table an `os-preserve`d element driven from `updated()`, and two PixiJS canvases mounted lazily into preserved hosts; Pages reuses the Posts parts), **Users** and **User Edit** (`apps/users/`, `apps/user-edit/` — the singleton that retargets through `reopen` on its `userId` param, and a custom element shared by two bundles), **Plugins** (`apps/plugins/` — in-process Core REST for the mutations, admin-ajax for what needs `wp-admin`, the landing tab a param) and **Comments** (`apps/comments/` — a conversation view whose scope is its `post` param). **Code Blue** — `apps/code-blue/` — is the error-log reader, rebuilt from a PHP module plus a 1,726-line TypeScript bundle into an `.os.php` (window, actions, data) and an `.os.ts` (the body, with range / search / sort / legend / expand running locally): same features, under half the lines, every filter instant. **Station Home** — `apps/station-home/` — is the native Dashboard, and the one to read for the *server* view: an `.os.php`, a snapshot model and a body painted in PHP, no client script at all — Refresh is the built-in, the Customize picker is one state key, a switch is an action, a restore is the `show` lifecycle. **OpenStation Preferences** — `apps/os-settings/` — is the one to read for an app that *edits shell state*: the settings are not its state (they are applied before the first paint and written by more than one window), so its client view reads them through `wp.os.getOsSettings()` and writes them through `wp.os.updateOsSettings()`, the same public API a third-party settings tab uses; its declared state is the page, and its server surface is the handful of writes that are site truth rather than a preference. Read one of them after this page.
...
| `prefetch()` | Compute `data()` once at registration and ship it with the window config, so the client view paints from the declared state **the moment the window opens** instead of behind a spinner for the length of the `mount` round trip (a WordPress request — and the first click on a spinner is a click lost). `mount` still runs and refreshes state and data. A window opened with params (a deep link) waits for `mount` as before, since the state those params produce is the server's to derive. Opt-in: the cost is paid on every shell page load for every user who may open the app, so it is right for a `data()` that is capability checks and options (Preferences) and wrong for one that runs queries (WP Explorer) — those declare a `placeholder` in the client view instead (see [The client view](#the-client-view--osts)): the frame paints at once, the rows land with `mount`. |
| `menu( $slug, $tabs, $gate = null )` | The admin menu this window answers for, and the tabs that ARE that menu's pages — an ordered `id => label` map (or `id => array( 'label' => …, 'page' => … )` naming the wp-admin submenu slug a tab replaces), or a callable returning one when capabilities decide the list. While `$gate` says this window is the one in charge (the per-user opt-in), **the dock's submenu for `$slug` becomes these tabs**: same labels, same order, the first as the tile's own label, each row tagged `os_tab=<id>` so picking it opens the window on that tab. `tab` becomes declared state and the runtime writes it on `mount` and `reopen`; the list reaches the client view as `menuTabs` in the config extra, which the view should render its strip from so the two cannot drift. A row for a page no tab names is kept and follows the window's tabs, so a plugin's screen under that menu stays reachable; a named page is also claimed by the shell wherever it is clicked, so it opens the window on that tab rather than the classic screen. With the gate off the dock keeps wp-admin's own submenu, since that is what those rows open. The framework declares `reopen` for you, because the client dispatches a lifecycle action only when the app declared one and that is how an open window retargets. See [`hooks-reference.md`](hooks-reference.md#openstation_dock_item--stable). |
...
Three action names are built in: **`mount`** (the first render), **`set`** (a bound control changed; nothing to run, just re-render), and **`refresh`** (recompute `data()` and re-render — the action to point a Refresh button at with no handler declared; declaring one still works and wins, for the app that also resets something on the way). Six more are **lifecycle** names the runtime dispatches only when you declared a handler: **`resize`** (`$args['width']`/`['height']`, debounced), **`show`** / **`hide`** (restore / minimize), **`focus`** / **`blur`** — on *transitions*: a window opens focused, so `focus` fires when the user comes back to it after leaving, not on the open and not on every click inside it (the shell reports a focus request on each pointerdown; the runtime gates those into transitions, because a handler here is a round trip) — and **`reopen`**: the window was asked to open while already open (`wp.os.openWindow( id, { params } )` on a live singleton, a deep link landing on a window that exists). The shell writes the new params onto the window first and the runtime adopts them on every session, so the handler reads `$os->params` (also echoed as `$args['params']`) and retargets — the User Edit app re-mounts on the new `userId` this way, the Comments app re-scopes to a new `post`, the Plugins app switches to the requested `tab`. Without a `reopen` handler the new params are still adopted for every later dispatch; only the immediate reaction is yours to declare.
...
| `config( array \| callable )` | Extra values for the client, readable as `wp.os.getWindowConfig( id ).extra` (and `ctx.extra`). A callable (`function (): array`) is resolved when the manifest is built, for the acting user — the shape for capability flags, the viewer's id, a filtered option — instead of once when the definition file loads. |
...
It ships the **list-window layout** too, so a list app declares no chrome of its own: a root `.os-app-list` (a column, a container for the width queries), `.os-app-list__tabs` / `__panel` for an in-body tab strip, `__toolbar` with its `__toolbar-left` / `__toolbar-right` (the selection's actions, honouring `hidden`) / `__toolbar-trailing` (pushed to the end) clusters, `__status` and `__search` for the two controls the narrow rules re-flow, `__count`, `__body` (the flex-1 host that gives `<os-table>` its height and takes the sideways overflow), `__empty` / `__empty-hint`, and `__pager` / `__pager-meta` / `__pager-nav` / `__pager-perpage`. Under 600px of window, and at any width under the shell's phone mode (`html[data-os-mode="mobile"]`), the toolbar gives the status control and the search a row each, the insets shrink to what a 360px screen can spare, the body goes edge to edge for the card list, and the pager wraps; a `__toolbar-right` that also wears `__bulk--footer` becomes the selection's bar along the bottom of the panel, clearing the home indicator, with its buttons sharing the width. Posts, Pages and Users wear exactly this; the same classes are open to any list a plugin ships. And `.os-app__menu-section` styles the label of a checkbox section [`mountMenuCheckboxes()`](#the-client-view--osts) appends to the window's ⋯ menu.

For app bodies with persistent actions, responsive grids or resizable panes, see [app layout recipes](./examples/app-layouts.md). These containers work in both server and client views; they do not alter the app state or persistence contract.
...
**The first paint.** A client view used to paint only once `mount` had answered — a whole WordPress request behind the shell's overlay, and a click on that overlay is a click lost. Two ways out, one per kind of `data()`:

- **`App::prefetch()`** (PHP) ships the real `data()` with the window config, computed at registration. Right for a `data()` that is capability checks and options (Preferences); wrong for one that runs queries, since every shell boot would pay them.
- **`placeholder( state )`** (the `.os.ts`) is the client's own stand-in — an empty list, zero counts, whatever lets the view paint its frame — used until `mount` answers:

```ts
export default defineApp< State, Data >( 'hello', {
	// The frame at once; the rows with `mount`. `ctx.loading` is true in between.
	placeholder: ( state ) => ( { list: { items: [], total: 0, pages: 0, page: state.page, perPage: state.perPage } } ),
	view: ( ctx ) => html`…<os-table ?loading=${ ctx.loading }>…</os-table>…`,
} );
```

The runtime paints from the declared state and the placeholder the moment the window opens (the shell drops its overlay; the window's busy mark shows while `mount` is in flight), then applies the `mount` answer as it would any response — one `mounted()`, no second mount. A window opened **with params** (a deep link) still waits for `mount`: the state those params produce is the server's to derive, and painting the defaults first would show the wrong page for a beat. Posts, Pages, Users, Plugins and Comments all declare one; the table windows hand `ctx.loading` (or the root's `aria-busy`) to `<os-table loading>`, Comments paints ghost rows in its rail.

And the `mount` itself can start before the click. With **Prewarm windows on hover** on, a sustained mouse hover over the window's dock tile calls [`wp.os.prewarmWindow( id )`](./javascript-reference.md#wposprewarmwindow-id---experimental): the bundles come into the tab and the runtime sends the window's first `mount` — the declared state, no params — holding the answer for ~30 s. The open takes it instead of fetching, so the placeholder's skeleton is on screen for a frame rather than a request. Nothing to declare: every app with a client view gets it, and a deep link (params) still fetches.
...
- **`statusControl( { segments, value, bind, action, label, phone? } )`** — the status filter across a list's toolbar: an `<os-segmented>` pill bar on a desk, an `<os-select>` on a phone (six pills in 360px wrap into two ragged rows), both wearing `.os-app-list__status`, both bound the same way — `os-bind` writes the state key, `os-action` is the app's "reset to page 1 and re-query" action. `phone` defaults to the shell's mode stamp: a desktop window pulled narrow keeps the pills, a phone gets the picker even in landscape.
- **`pager( { page, pages, perPage, summary, labels, pageAction?, perPageBind?, perPageAction?, perPageOptions? } )`** — the footer pager: the app's own `summary` text ("Page 2 of 5 · 90 posts"), Previous / Next dispatching `pageAction` (default `page`) with `os-arg-page`, disabled at the edges, and a per-page select bound to `perPageBind` (default `perPage`) that dispatches `perPageAction` (default `filter`), since the page count just changed.
- **`createListTableSync< Row >()`** — keeping a preserved `<os-table>` in step with the view: `sync( { table, rows, listKey, fingerprint, columns, wire?, rowId?, onSelection? } )` from `updated()` puts the card list on a phone (and rebuilds the columns only when that answer changes), runs the one-time wiring, clears the selection when the QUERY changes (`listKey`), and assigns `data` only when the rows changed (`fingerprint`) while pruning the selection to what is still visible. `invalidateColumns()` / `invalidateData()` force the next pass. Keep it in `ctx.ui`.
- **`mountMenuCheckboxes( root, { section, items, isChecked, onToggle, prefix } )`** — a checkbox section in the window's ⋯ menu ("Show columns"): a label plus one `menuitemcheckbox` row per item, painted from `isChecked`, answering a click through `onToggle`. The menu belongs to the window chrome outside the mount root, so this one is imperative: call it from `mounted()`, keep the handle (`refresh()` after an external change, `dispose()` in the teardown). Idempotent per `prefix`; `null` when the window has no menu or there is nothing to toggle.

Server-side, a list app's `data()` reads the collections WordPress already knows how to serve instead of re-implementing a query per window: **`openstation_app_rest( $method, $route, $query, $body )`** runs a REST request in-process — the same controller, the same permission checks, every `register_rest_field()` a plugin added, `_fields` applied and `_embed` expanded — and returns `array( ok, status, data, total, pages, error, code )`; **`openstation_app_rest_page( $route, $query )`** wraps it into the `Os::page()` envelope (plus an `error` key, '' on success) that `createPagedList()` and the pager understand. This is what keeps the row a browser fetched and the row an app renders byte-identical, and what keeps the filters plugin authors already hook (`rest_post_query`, the REST fields, the `openstation_*_window_query_args` projections) working: the request IS a REST request, minus the HTTP round trip. Both live in `includes/framework/wordpress.php` — they are the WordPress host's, not the core's — and `Tests_OpenStation_AppFrameworkRest` pins them.
...
- on open, applies the declared appearance, mounts one **session per mount root** (the body, plus one per tab panel), dispatches `mount` for each, and delegates every trigger event on the roots; with a client view (`window.openStationApps[ id ]`, published by the app's `.os.ts` bundle), the main body is rendered from `( state, data )` and `local` actions / `os-bind` writes re-render without a request — and with prefetched data (`App::prefetch()`) or a client `placeholder` it is rendered at once, before `mount` answers, so the window never shows the overlay;
...
apps/posts/, apps/pages/        the Posts and Pages list windows (Pages imports the Posts parts); the Categories mind map and Tags cloud under apps/posts/parts/
apps/users/, apps/user-edit/    the Users list and the User Edit singleton; <os-user-profile> under apps/users/parts/, shared by both bundles
apps/plugins/, apps/comments/   the Plugins window (installed / browse / featured, admin-ajax parts) and the Comments conversation view
src/app-runtime/list-ui.ts      statusControl(), pager(), mountMenuCheckboxes() — the list-window furniture
```
