...
The games framework is **opt-in and off by default** — an admin enables it site-wide in OS Settings → Features → Extended options (or via the `desktop_mode_games_enabled` filter). While it's off, none of the games module loads — `desktop_mode_register_game()` is undefined, exactly as if Desktop Mode weren't active. The `function_exists()` guard in the recipe below covers both cases; saved scores and play time survive a disable/re-enable round trip untouched.
...
add_action( 'init', function () {
    if ( ! function_exists( 'desktop_mode_register_game' ) ) {
        return; // Desktop Mode not active.
    }
    ...
    desktop_mode_register_game( 'my-plugin-tap', array(
...
add_filter( 'desktop_mode_game_score_pre_save', function ( $pre, $game, $user_id, $score ) {
...
add_filter( 'desktop_mode_games_can_challenge', function ( $allowed, $challenger_id, $recipient_id ) {
