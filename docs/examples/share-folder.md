...
add_action( 'init', function () {
    // Imagine we just created a folder via open_station_files_create_folder
    // and want to grant the team Editor role read access programmatically.
    if ( ! function_exists( 'open_station_folder_share_invite' ) ) {
        return;
    }
...
```php
add_action( 'open_station_files_share_accepted', function ( $share_id, $row, $user_id ) {
...
add_action( 'open_station_files_share_revoked', function ( $share_id, $row, $actor_id ) {
...
```php
add_filter( 'open_station_files_share_can_manage', function ( $can, $folder_id, $user_id, $folder ) {
    if ( $can ) {
        return $can;
    }
...
```php
add_filter( 'open_station_files_share_eligible_roles', function ( $roles ) {
...
```php
add_filter( 'open_station_files_share_user_query_args', function ( $args, $req_params ) {
...
`open_station_files_can_delete_folder` runs after the ownership
...
add_filter(
    ...
    function ( $can, $folder_id, $user_id, $folder ) {
        ...
        $shares = (array) open_station_files_get_folder_shares( $folder_id );
...
`open_station_files_after_delete_folder_cascade` fires once per
...
Per-share `open_station_files_share_revoked` actions fire during
