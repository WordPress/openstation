...
add_filter(
    'desktop_mode_drafts_ai_instructions',
...
```php
add_filter(
    'desktop_mode_drafts_ai_content_limit',
    function ( $limit, $post ) {
...
`desktop_mode_drafts_ai_suggestions` runs last, after tag-stripping and
truncation — drop, reorder or append entries without re-sanitizing.

```php
add_filter(
    'desktop_mode_drafts_ai_suggestions',
...
- With no provider, an authorized caller gets `503
  desktop_mode_ai_unavailable` and the 💡 button never renders —
