...
| Type | Direction | Carries | Purpose |
|---|---|---|---|
| `os-iframe-other-admin-link` | iframe → parent | `{ url }` | Posted for a same-origin click that leaves this ADMIN — another site's `wp-admin`, or the network admin, detected by the bridge's `adminScope()` rule. Deliberately its own message rather than a flag on `os-iframe-admin-link`: the same-admin path consults remaps and dock entries that mean nothing in another admin. The shell opens the URL in that admin's own Space (see [multisite.md](./multisite.md#site-spaces)); the flag `openstation_chromeless` is stripped before posting. Modifier and middle clicks never produce it — the interceptor yields those to the browser. |
