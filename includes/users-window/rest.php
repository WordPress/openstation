 ...
 *          the filtered `open_station_users_window_assignable_roles()`
 ...
 */
function open_station_users_window_register_rest_routes() {
	register_rest_route(
		'desktop-mode/v1',
		'/users/bulk-role',
		array(
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => 'open_station_users_window_rest_bulk_role',
	...
	register_rest_route(
		...
		array(
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => 'open_station_users_window_rest_send_password_reset',
			'permission_callback' => static function () {
	...
	register_rest_route(
		...
		array(
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => 'open_station_users_window_rest_resend_welcome',
			'permission_callback' => static function () {
				return current_user_can( 'edit_users' );
			},
			'args'                => array(
	...
	register_rest_route(
		...
		array(
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => 'open_station_users_window_rest_create',
			'permission_callback' => static function () {
	...
	register_rest_route(
		...
		array(
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => 'open_station_users_window_rest_bulk_delete',
			'permission_callback' => static function () {
				return is_multisite()
...
add_action( 'rest_api_init', 'open_station_users_window_register_rest_routes' );
 ...
 */
function open_station_users_window_rest_bulk_role( $req ) {
	$ids  = array_values(
		array_filter(
	...
	$assignable = open_station_users_window_assignable_roles( $viewer_id );
	if ( ! in_array( $role, $assignable, true ) ) {
		return new WP_Error(
			'open_station_users_role_forbidden',
 ...
 * @return WP_REST_Response|WP_Error
 */
function open_station_users_window_rest_send_password_reset( $req ) {
 ...
 * @return WP_REST_Response|WP_Error
 */
function open_station_users_window_rest_resend_welcome( $req ) {
	$id   = (int) $req->get_param( 'id' );
	$user = $id > 0 ? get_userdata( $id ) : null;
	if ( ! $user instanceof WP_User ) {
		return new WP_Error(
 ...
 * @return WP_REST_Response|WP_Error
 */
function open_station_users_window_rest_bulk_delete( $req ) {
	$ids       = array_values(
		array_filter(
			array_map( 'intval', (array) $req->get_param( 'ids' ) ),
			static function ( $id ) {
 ...
 * @return WP_REST_Response|WP_Error
 */
function open_station_users_window_rest_create( $req ) {
	...
	if ( '' === $username ) {
		return new WP_Error(
			'open_station_users_username_required',
	...
	if ( ! validate_username( $username ) ) {
		return new WP_Error(
			'open_station_users_username_invalid',
	...
	if ( '' === $email || ! is_email( $email ) ) {
		return new WP_Error(
			'open_station_users_email_invalid',
	...
	if ( username_exists( $username ) ) {
		return new WP_Error(
			'open_station_users_username_exists',
	...
	if ( email_exists( $email ) ) {
		return new WP_Error(
			'open_station_users_email_exists',
	...
	$assignable = open_station_users_window_assignable_roles( (int) get_current_user_id() );
	// `open_station_users_window_assignable_roles` is gated on
	...
	if ( '' !== $locale ) {
		$locale_slugs = array_keys( open_station_users_window_locales_map() );
	 ...
	 */
	do_action(
		'open_station_users_window_user_created',
		(int) $user_id,
