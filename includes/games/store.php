 ...
 * `open_station_game_score_pre_save` filter for plugins that want
 * stricter validation.
 *
 ...
 */
function open_station_games_sanitize_score_meta( $meta ) {
	if ( ! is_array( $meta ) ) {
		return array();
	}
	$out = array();
 ...
 */
function open_station_games_save_score( $game, $user_id, $score, $meta = array() ) {
	global $wpdb;

	$game    = sanitize_key( (string) $game );
	$user_id = (int) $user_id;
	$score   = max( 0, (int) $score );
	$meta    = open_station_games_sanitize_score_meta( $meta );
	 ...
	 */
	$pre = apply_filters( 'open_station_game_score_pre_save', null, $game, $user_id, $score, $meta );
	if ( is_wp_error( $pre ) ) {
		return $pre;
	}
	...
	$ok     = $wpdb->insert(
		...
		array(
			...
			'created_at_ms' => open_station_games_now_ms(),
		),
		array( '%s', '%d', '%d', '%s', '%d' )
	);
	if ( false === $ok ) {
		return new WP_Error(
			'open_station_score_save_failed',
	 ...
	 */
	do_action( 'open_station_game_score_saved', $id, $game, $user_id, $score, $meta );
 ...
 */
function open_station_games_get_scores( $game, $args = array() ) {
	global $wpdb;
	...
	return array(
		'rows'  => array_map( 'open_station_games_shape_score', (array) $rows ),
		'total' => $total,
	);
}
 ...
 * @return array
 */
function open_station_games_shape_score( $row ) {
 ...
 */
function open_station_games_create_challenge( $game, $challenger_id, $recipient_id, $score_to_beat, $score_meta = array() ) {
	global $wpdb;
	...
	if ( $recipient_id <= 0 || ! get_userdata( $recipient_id ) ) {
		return new WP_Error(
			'open_station_invalid_recipient',
	...
	$ok     = $wpdb->insert(
		...
		array(
			...
			'score_meta'    => wp_json_encode( open_station_games_sanitize_score_meta( $score_meta ) ),
			'state'         => 'pending',
			'created_at_ms' => $now,
			'updated_at_ms' => $now,
		),
	...
	if ( false === $ok ) {
		return new WP_Error(
			'open_station_challenge_create_failed',
	 ...
	 */
	do_action( 'open_station_game_challenge_created', $id, open_station_games_get_challenge( $id ) );
 ...
 */
function open_station_games_get_challenge( $id ) {
	global $wpdb;
	$tables = open_station_games_table_names();
	$row    = $wpdb->get_row(
 ...
 */
function open_station_games_set_challenge_state( $id, $state ) {
	global $wpdb;

	if ( ! in_array( $state, array( 'accepted', 'declined' ), true ) ) {
		return new WP_Error(
			'open_station_invalid_challenge_state',
	...
	if ( 'pending' !== $row['state'] ) {
		return new WP_Error(
			'open_station_challenge_state_conflict',
	...
	$now    = max( open_station_games_now_ms(), (int) $row['updated_at_ms'] + 1 );
	$tables = open_station_games_table_names();
	$wpdb->update(
		$tables['challenges'],
		array(
	...
	if ( 'accepted' === $state ) {
		 ...
		 */
		do_action( 'open_station_game_challenge_accepted', (int) $id, $row );
	} else {
		/**
		 ...
		 * @param array $row The (pre-transition) challenge row.
		 */
		do_action( 'open_station_game_challenge_declined', (int) $id, $row );
	}
 ...
 */
function open_station_games_complete_challenge( $id, $score, $meta = array() ) {
	global $wpdb;
	...
	$score_id = open_station_games_save_score( $row['game'], (int) $row['recipient_id'], $score, $meta );
	...
	$updated = open_station_games_get_challenge( $id );
	 ...
	 */
	do_action( 'open_station_game_challenge_completed', (int) $id, $result, $updated );
 ...
 */
function open_station_games_get_challenges_for_user( $user_id, $since_ms = 0, $cap = 50 ) {
	global $wpdb;
	$tables = open_station_games_table_names();
	$rows   = $wpdb->get_results(
		$wpdb->prepare(
 ...
 * @return array
 */
function open_station_games_shape_challenge( $row ) {
