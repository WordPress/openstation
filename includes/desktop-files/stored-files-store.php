 ...
 * {@see open_station_stored_files_handle_unplaced()}).
 *
 * @package OpenStation
 */
 ...
 * {@see open_station_stored_files_ensure_dir()}.
 *
 * @param int $user_id Optional. Owner whose subdirectory to return.
 * @return string
 */
function open_station_stored_files_dir( $user_id = 0 ) {
	 ...
	 */
	$base = (string) apply_filters( 'open_station_stored_files_base_dir', $base );
	if ( (int) $user_id > 0 ) {
 ...
 */
function open_station_stored_files_ensure_dir( $user_id ) {
	$user_id = (int) $user_id;
	if ( $user_id <= 0 ) {
		return new WP_Error( 'open_station_stored_files_invalid_user', __( 'A user id is required.', 'desktop-mode' ), array( 'status' => 400 ) );
	}
	$base = open_station_stored_files_dir();
	$dir  = open_station_stored_files_dir( $user_id );
	if ( ! wp_mkdir_p( $dir ) ) {
		return new WP_Error( 'open_station_stored_files_mkdir_failed', __( 'Could not create the storage directory.', 'desktop-mode' ), array( 'status' => 500 ) );
	}
 ...
 * @return bool
 */
function open_station_stored_files_valid_disk_name( $disk_name ) {
 ...
 * @return string|null
 */
function open_station_stored_file_path( $row ) {
	if ( ! is_array( $row ) || empty( $row['owner_id'] ) || empty( $row['disk_name'] ) ) {
		return null;
	}
	if ( ! open_station_stored_files_valid_disk_name( $row['disk_name'] ) ) {
		return null;
	}
	$base = open_station_stored_files_dir();
	$path = open_station_stored_files_dir( (int) $row['owner_id'] ) . '/' . $row['disk_name'];
 ...
 */
function open_station_stored_files_create( $owner_id, $args ) {
	global $wpdb;
	...
	if ( ! open_station_stored_files_valid_disk_name( $args['disk_name'] ) ) {
		return new WP_Error( 'open_station_stored_files_bad_disk_name', __( 'Invalid storage name.', 'desktop-mode' ), array( 'status' => 400 ) );
	}
	...
	if ( false === $ok ) {
		return new WP_Error( 'open_station_stored_files_insert_failed', __( 'Failed to record the uploaded file.', 'desktop-mode' ), array( 'status' => 500 ) );
	}
	 ...
	 * @param int $owner_id Owner.
	 */
	do_action( 'open_station_stored_file_created', $id, $owner_id );
 ...
 * @return array|null
 */
function open_station_stored_files_get( $file_id ) {
	global $wpdb;
	$file_id = (int) $file_id;
	if ( $file_id <= 0 ) {
		return null;
	...
	return open_station_stored_files_normalize_row( $row );
}
 ...
 * @return array
 */
function open_station_stored_files_normalize_row( $row ) {
	return array(
		'id'            => (int) $row['id'],
 ...
 */
function open_station_stored_files_rename( $file_id, $name ) {
	global $wpdb;
	...
	if ( '' === $name ) {
		return new WP_Error( 'open_station_stored_files_bad_name', __( 'A file name is required.', 'desktop-mode' ), array( 'status' => 400 ) );
	}
	$tables = open_station_files_table_names();
	$now    = open_station_files_now_ms();
	$wpdb->update(
	 ...
	 */
	do_action( 'open_station_stored_file_renamed', (int) $row['id'], $name, (string) $row['display_name'] );
 ...
 * @return bool
 */
function open_station_stored_files_gate_trash( $can, $user_id, $row ) {
...
add_filter( 'open_station_files_user_can_trash_placement', 'open_station_stored_files_gate_trash', 20, 3 );

/**
 * Delete a stored file: bytes first, then the row. Does NOT touch
 * placements — callers that need the full cascade go through
 * {@see open_station_stored_files_purge()}.
 *
 * @param int $file_id Row id.
 * @return true|WP_Error
 */
function open_station_stored_files_delete( $file_id ) {
	global $wpdb;
	$row = open_station_stored_files_get( $file_id );
	if ( ! $row ) {
		return new WP_Error( 'open_station_stored_files_not_found', __( 'Stored file not found.', 'desktop-mode' ), array( 'status' => 404 ) );
	 ...
	 */
	do_action( 'open_station_stored_file_deleted', (int) $row['id'], $row );
 ...
 * @return true|WP_Error
 */
function open_station_stored_files_purge( $file_id ) {
	global $wpdb;
	$file_id = (int) $file_id;
	$row     = open_station_stored_files_get( $file_id );
	if ( ! $row ) {
	...
	return open_station_stored_files_delete( $file_id );
}
 ...
 * @return int
 */
function open_station_stored_files_total_bytes( $owner_id ) {
	global $wpdb;
	$tables = open_station_files_table_names();
 ...
 * @return int
 */
function open_station_stored_files_user_quota_bytes( $user_id ) {
	/**
	 ...
	 */
	return max( 0, (int) apply_filters( 'open_station_stored_files_user_quota_bytes', 0, (int) $user_id ) );
}
 ...
 * @return string
 */
function open_station_stored_files_upload_capability() {
	/**
	 * Filters the capability required to upload desktop files.
	 *
	 * @param string $capability Default 'upload_files'.
	 */
	return (string) apply_filters( 'open_station_stored_files_upload_capability', 'upload_files' );
}
 ...
 * @return bool
 */
function open_station_stored_file_user_can_read( $file_id, $user_id ) {
	global $wpdb;
	$file_id = (int) $file_id;
	$user_id = (int) $user_id;
	...
	if ( function_exists( 'open_station_stored_file_share_state' ) ) {
		if ( 'accepted' === open_station_stored_file_share_state( $file_id, $user_id ) ) {
			return true;
		}
	}
	...
	if ( function_exists( 'open_station_folder_share_user_capability' ) ) {
		foreach ( (array) $parents as $parent_id ) {
			if ( 'none' !== open_station_folder_share_user_capability( (int) $parent_id, $user_id ) ) {
				return true;
			}
		}
	}
	 ...
	 */
	return (bool) apply_filters( 'open_station_stored_file_can_read', false, $file_id, $user_id, $row );
}
 ...
 */
function open_station_stored_files_handle_unplaced( $placement_id, $row ) {
	global $wpdb;
	if ( ! is_array( $row ) || 'upload' !== (string) ( $row['file_type'] ?? '' ) ) {
		return;
	}
	...
	open_station_stored_files_purge( $file_id );
}
add_action( 'open_station_file_unplaced', 'open_station_stored_files_handle_unplaced', 10, 2 );
 ...
 */
function open_station_stored_files_reconcile() {
	global $wpdb;
	$tables = open_station_files_table_names();
	$grace  = DAY_IN_SECONDS;

	// a) Placement-less rows past grace.
	$cutoff_ms = open_station_files_now_ms() - ( $grace * 1000 );
	...
	foreach ( (array) $orphans as $orphan_id ) {
		open_station_stored_files_delete( (int) $orphan_id );
	}
	...
	foreach ( (array) $user_dirs as $user_dir ) {
		...
		foreach ( (array) $entries as $entry ) {
			...
			if ( ! open_station_stored_files_valid_disk_name( $name ) ) {
...
add_action( 'desktop_mode_files_daily_prune', 'open_station_stored_files_reconcile' );
 ...
 */
function open_station_stored_files_handle_deleted_user( $user_id ) {
	global $wpdb;
	$user_id = (int) $user_id;
	if ( $user_id <= 0 ) {
		return;
	...
	foreach ( (array) $ids as $file_id ) {
		open_station_stored_files_purge( (int) $file_id );
	}
	$dir = open_station_stored_files_dir( $user_id );
	if ( is_dir( $dir ) ) {
		$index = $dir . '/index.php';
...
add_action( 'deleted_user', 'open_station_stored_files_handle_deleted_user' );
