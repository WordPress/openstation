...
function openstation_stored_file_share_invite( $file_id, $actor_id, $recipient_user_id ) {
	...
	if ( ! openstation_stored_files_share_can_manage( $file_id, $actor_id ) ) {
		return new WP_Error( 'openstation_files_forbidden', __( 'You cannot manage shares for this file.', 'desktop-mode' ), array( 'status' => 403 ) );
	}
	if ( $uid <= 0 ) {
	...
	if ( $uid === (int) $file['owner_id'] ) {
		return new WP_Error( 'openstation_files_share_owner', __( 'You cannot share with the file owner.', 'desktop-mode' ), array( 'status' => 400 ) );
	}
...
function openstation_stored_file_share_deny( $share_id, $user_id ) {
	...
	if ( $was_accepted ) {
		openstation_files_trash_upload_for_user( (int) $row['folder_id'], $user_id );
	}
...
function openstation_stored_file_share_leave( $file_id, $user_id ) {
	...
	$file    = openstation_stored_files_get( $file_id );
	if ( ! $file ) {
		return new WP_Error( 'openstation_files_not_found', __( 'File not found.', 'desktop-mode' ), array( 'status' => 404 ) );
	}
	if ( (int) $file['owner_id'] === $user_id ) {
		return new WP_Error( 'openstation_files_owner_cannot_leave', __( 'Owners cannot leave their own file.', 'desktop-mode' ), array( 'status' => 400 ) );
	}
	...
	if ( ! $row ) {
		return new WP_Error( 'openstation_files_not_member', __( 'You do not have access to this file.', 'desktop-mode' ), array( 'status' => 404 ) );
	}
	$normalized = openstation_files_normalize_share_row( $row );
	$wpdb->update(
		$tables['shares'],
		array(
			'state'         => 'denied',
...
function openstation_stored_file_share_revoke( $share_id, $actor_id ) {
	...
	if ( ! openstation_stored_files_share_can_manage( (int) $row['folder_id'], $actor_id ) ) {
		return new WP_Error( 'openstation_files_forbidden', __( 'You cannot manage shares for this file.', 'desktop-mode' ), array( 'status' => 403 ) );
	}
...
function openstation_files_register_file_share_rest_routes() {
	...
	register_rest_route( $ns, '/files/uploads/(?P<id>\d+)/shares', array(
		array(
			...
			'callback'            => 'openstation_files_rest_list_file_shares',
		),
		array(
			'methods'             => WP_REST_Server::CREATABLE,
			'permission_callback' => 'openstation_files_rest_share_permission',
			'callback'            => 'openstation_files_rest_create_file_share',
			'args'                => array(
...
function openstation_files_rest_list_file_shares( WP_REST_Request $req ) {
	...
	if ( ! openstation_stored_files_share_can_manage( $file_id, $user_id ) ) {
		return openstation_files_download_not_found();
	}
	$out = array();
	foreach ( openstation_stored_files_get_file_shares( $file_id ) as $row ) {
		$out[] = openstation_files_shape_file_share( $row );
	}
...
function openstation_files_rest_create_file_share( WP_REST_Request $req ) {
	...
	if ( null !== $capability && 'read' !== (string) $capability ) {
		return new WP_Error(
			'openstation_files_invalid_capability',
			__( 'Uploaded files can only be shared read-only.', 'desktop-mode' ),
			array( 'status' => 400 )
		);
	}
	$id = openstation_stored_file_share_invite(
		(int) $req['id'],
		get_current_user_id(),
...
function openstation_files_rest_delete_file_share( WP_REST_Request $req ) {
	...
	$ok = openstation_stored_file_share_revoke( (int) $row['id'], get_current_user_id() );
	if ( is_wp_error( $ok ) ) {
		return $ok;
	}
...
function openstation_files_rest_accept_file_share( WP_REST_Request $req ) {
	...
	$next = openstation_stored_file_share_accept( (int) $row['id'], get_current_user_id() );
	if ( is_wp_error( $next ) ) {
		return $next;
	}
...
function openstation_files_rest_deny_file_share( WP_REST_Request $req ) {
	...
	$next = openstation_stored_file_share_deny( (int) $row['id'], get_current_user_id() );
	if ( is_wp_error( $next ) ) {
		return $next;
	}
	return rest_ensure_response( openstation_files_shape_file_share( $next ) );
...
function openstation_files_file_share_inject_shell_config( $config ) {
	...
	$pending = isset( $config['serverPendingShares'] ) && is_array( $config['serverPendingShares'] )
		...
		: array();
	foreach ( openstation_files_get_pending_file_shares_for_user( $user_id, 0 ) as $row ) {
		$pending[] = openstation_files_shape_file_share( $row );
	}
	$config['serverPendingShares'] = $pending;
	return $config;
}
add_filter( 'openstation_shell_config', 'openstation_files_file_share_inject_shell_config', 21 );
