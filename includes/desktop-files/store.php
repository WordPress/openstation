 ...
 * Every read goes through `open_station_files_query_args` so
 ...
 * `open_station_files_can_place` (filter) and the file's
 * `Open_Station_File::can_read()` before writing.
 *
 ...
 * open_station_files_write_tombstone() for the invariant.
 *
 * @package OpenStation
 */
 ...
 */
function open_station_files_place( $user_id, $parent_id, $type, $ref, $args = array() ) {
	global $wpdb;
	...
	$entry = open_station_get_file_type( $type );
	if ( ! $entry ) {
		return new WP_Error( 'open_station_files_unknown_type', __( 'Unknown file type.', 'desktop-mode' ), array( 'status' => 400 ) );
	}
	 ...
	 * @param string $ref     Entity reference.
	 */
	$file = open_station_resolve_file( $type, $ref );
	$can  = $file ? $file->can_read( $user_id ) : false;
	$can  = (bool) apply_filters( 'open_station_files_can_place', $can, $user_id, $type, $ref );
	if ( ! $can ) {
		return new WP_Error( 'open_station_files_forbidden', __( 'You are not allowed to place this file.', 'desktop-mode' ), array( 'status' => 403 ) );
	}
	...
	// always allowed.
	if ( (int) $parent_id > 0 ) {
		$target_folder = open_station_files_get_folder( (int) $parent_id );
		if ( $target_folder && (int) $target_folder['owner_id'] !== $user_id ) {
			$cap = function_exists( 'open_station_folder_share_user_capability' )
				? open_station_folder_share_user_capability( (int) $parent_id, $user_id )
				: 'none';
			if ( 'write' !== $cap ) {
				return new WP_Error(
					'open_station_files_no_write_in_shared_folder',
	...
	if ( false === $ok ) {
		...
		if ( ! $existing ) {
			return new WP_Error( 'open_station_files_insert_failed', __( 'Failed to write placement.', 'desktop-mode' ), array( 'status' => 500 ) );
		}

		$existing_id = (int) $existing['id'];

		if ( ! empty( $existing['trashed_at_ms'] ) ) {
			$restore = open_station_files_restore_placement( $user_id, $existing_id );
		...
		open_station_files_clear_tombstones_for( 'placement', $existing_id );

		$move = open_station_files_move(
			$existing_id,
			$user_id,
			array(
				'parent_id'  => max( 0, $parent_id ),
	 ...
	 */
	do_action( 'open_station_file_placed', $id, $row );
 ...
 */
function open_station_files_move( $placement_id, $user_id, $changes = array() ) {
	global $wpdb;
	...
	$upload_lock = open_station_files_upload_owner_lock( $row, $user_id );
	...
	if ( (int) $row['parent_id'] > 0 ) {
		$source_folder = open_station_files_get_folder( (int) $row['parent_id'] );
		if ( $source_folder ) {
			$is_folder_owner = (int) $source_folder['owner_id'] === $user_id;
			$source_cap      = function_exists( 'open_station_folder_share_user_capability' )
				? open_station_folder_share_user_capability( (int) $row['parent_id'], $user_id )
				: 'none';
		...
		return new WP_Error( 'open_station_files_forbidden', __( 'You cannot edit this placement.', 'desktop-mode' ), array( 'status' => 403 ) );
	}
	if ( isset( $changes['parent_id'] ) ) {
		$target_parent = max( 0, (int) $changes['parent_id'] );
		if ( $target_parent > 0 ) {
			$target = open_station_files_get_folder( $target_parent );
			if ( $target && (int) $target['owner_id'] !== $user_id ) {
				$cap = function_exists( 'open_station_folder_share_user_capability' )
					? open_station_folder_share_user_capability( $target_parent, $user_id )
					: 'none';
				if ( 'write' !== $cap ) {
					return new WP_Error(
						'open_station_files_no_write_in_shared_folder',
	...
	$set['updated_at_ms'] = open_station_files_now_ms();
	$fmt[]                = '%d';
	...
	if ( false === $ok ) {
		return new WP_Error( 'open_station_files_update_failed', __( 'Failed to update placement.', 'desktop-mode' ), array( 'status' => 500 ) );
	}

	$next = open_station_files_get_placement( $placement_id );
	 ...
	 */
	do_action( 'open_station_file_moved', $placement_id, $next, $row );
 ...
 * @return true|WP_Error
 */
function open_station_files_remove( $placement_id, $user_id ) {
	global $wpdb;
	...
	if ( ! $allowed ) {
		return new WP_Error( 'open_station_files_forbidden', __( 'You cannot remove this placement.', 'desktop-mode' ), array( 'status' => 403 ) );
	}

	$tables = open_station_files_table_names();
	$ok     = $wpdb->delete( $tables['placements'], array( 'id' => $placement_id ), array( '%d' ) );
	if ( false === $ok ) {
		return new WP_Error( 'open_station_files_delete_failed', __( 'Failed to remove placement.', 'desktop-mode' ), array( 'status' => 500 ) );
	}

	open_station_files_write_tombstone( 'placement', $placement_id );
	 ...
	 */
	do_action( 'open_station_file_unplaced', $placement_id, $row );
 ...
 * @return true|WP_Error
 */
function open_station_files_upload_owner_lock( $row, $user_id ) {
	if ( ! is_array( $row ) || 'upload' !== (string) ( $row['file_type'] ?? '' ) ) {
		return true;
	}
	if ( ! function_exists( 'open_station_stored_files_get' ) ) {
		return true;
	}
	$stored = open_station_stored_files_get( (int) $row['file_ref'] );
	if ( ! $stored ) {
		return true;
	}
	...
	return new WP_Error(
		'open_station_files_upload_owner_locked',
 ...
 * @return array|null
 */
function open_station_files_get_placement( $placement_id ) {
	global $wpdb;
	$tables = open_station_files_table_names();
	$row    = $wpdb->get_row(
	...
	return open_station_files_normalize_placement_row( $row );
}

/**
 * List placements for a user under a given folder (0 = desktop
 * root). Honors the `open_station_files_query_args` filter and
 ...
 * @return array[]
 */
function open_station_files_get_for_user_folder( $user_id, $parent_id = 0 ) {
	global $wpdb;
	$user_id   = (int) $user_id;
	...
	if ( $parent_id > 0 ) {
		...
		if ( (int) $folder['owner_id'] !== $user_id ) {
			$cap = function_exists( 'open_station_folder_share_user_capability' )
				? open_station_folder_share_user_capability( $parent_id, $user_id )
				: 'none';
			if ( 'none' === $cap ) {
				return array();
			}
	 ...
	 */
	$args = (array) apply_filters( 'open_station_files_query_args', $args, $user_id, $parent_id );
	...
	$out = array();
	foreach ( $rows as $row ) {
		$normalized = open_station_files_normalize_placement_row( $row );
		$file       = open_station_resolve_file( $normalized['file_type'], $normalized['file_ref'] );
		if ( empty( $args['share_view'] ) ) {
 ...
 *
 *   2. Plugin shortcuts (`open_station_register_icon()`) the
 ...
 */
function open_station_files_auto_place_orphans( $user_id ) {
	global $wpdb;
	$user_id = (int) $user_id;
	if ( $user_id <= 0 ) {
		return 0;
	...
	$shortcut_ids   = array();
	$registry       = function_exists( 'open_station_desktop_icon_registry' )
		? open_station_desktop_icon_registry()
		: array();
	if ( is_array( $registry ) ) {
		// Run through the same `open_station_icons` filter the
		// build-payload path uses so plugins (and tests) can inject
		// virtual entries.
		$registry = (array) apply_filters( 'open_station_icons', $registry );
	}
	...
	$emit_at = function ( $type, $ref, $col, $row ) use ( $user_id, &$occupied, &$placed ) {
		...
		$result = open_station_files_place(
			$user_id,
			0,
			$type,
			(string) $ref,
 ...
 *
 * @deprecated Use {@see open_station_files_auto_place_orphans}.
 *
 * @param int $user_id Viewer.
 * @return int
 */
function open_station_files_auto_place_orphan_folders( $user_id ) {
	return open_station_files_auto_place_orphans( $user_id );
}
 ...
 * @return array
 */
function open_station_files_normalize_placement_row( $row ) {
	...
	return array(
		...
		// need the actor (e.g. `open_station_files_check_if_match`)
 ...
 * `open_station_files_compute_heartbeat_delta`. A tombstone on a
 ...
 *
 * Pair every revival path (`open_station_files_restore_placement`,
 * `open_station_files_restore_folder`, and the duplicate-key
 * revival branch in `open_station_files_place`) with
 * {@see open_station_files_clear_tombstones_for} so a row coming
 ...
 */
function open_station_files_write_tombstone( $kind, $ref ) {
	global $wpdb;
	$tables = open_station_files_table_names();
	$wpdb->insert(
		$tables['tombstones'],
		array(
			'kind'          => (string) $kind,
			'ref_id'        => (int) $ref,
			'removed_at_ms' => open_station_files_now_ms(),
		),
		array( '%s', '%d', '%d' )
	);
}
 ...
 */
function open_station_files_clear_tombstones_for( $kind, $ref_id ) {
	global $wpdb;
	$ref_id = (int) $ref_id;
	if ( $ref_id <= 0 ) {
		return;
 ...
 */
function open_station_files_prune_tombstones() {
	global $wpdb;
	$tables = open_station_files_table_names();
	$cutoff = open_station_files_now_ms() - ( 7 * DAY_IN_SECONDS * 1000 );
	$wpdb->query( $wpdb->prepare( "DELETE FROM {$tables['tombstones']} WHERE removed_at_ms < %d", $cutoff ) );
}
add_action( 'desktop_mode_files_daily_prune', 'open_station_files_prune_tombstones' );
 ...
 */
function open_station_files_schedule_prune() {
...
add_action( 'init', 'open_station_files_schedule_prune' );
 ...
 */
function open_station_files_would_create_folder_cycle( $user_id, $moving_folder_id, $target_parent_id ) {
