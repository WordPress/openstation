 ...
 * Register the route.
 */
function open_station_my_wordpress_register_media_usage_route() {
	register_rest_route(
		'desktop-mode/v1',
		'/media-usage/(?P<id>\d+)',
		array(
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => 'open_station_my_wordpress_media_usage_callback',
...
add_action( 'rest_api_init', 'open_station_my_wordpress_register_media_usage_route' );
 ...
 * @return int
 */
function open_station_my_wordpress_media_usage_ttl( $attachment_id ) {
	/**
	 ...
	 */
	return (int) apply_filters( 'open_station_my_wordpress_media_usage_cache_ttl', 300, $attachment_id );
}
 ...
 * @return string[]
 */
function open_station_my_wordpress_media_usage_cache_buckets() {
 ...
 * @return string
 */
function open_station_my_wordpress_media_usage_current_bucket() {
 ...
 * @return string
 */
function open_station_my_wordpress_media_usage_cache_key( $attachment_id, $bucket = null ) {
	if ( null === $bucket ) {
		$bucket = open_station_my_wordpress_media_usage_current_bucket();
	}
 ...
 */
function open_station_my_wordpress_media_usage_callback( $request ) {
	$attachment_id = (int) $request->get_param( 'id' );
	$attachment    = get_post( $attachment_id );
	if ( ! $attachment || 'attachment' !== $attachment->post_type ) {
		return new WP_Error(
			'open_station_media_not_found',
			__( 'Attachment not found.', 'desktop-mode' ),
			array( 'status' => 404 )
		);
	}

	$cache_key    = open_station_my_wordpress_media_usage_cache_key( $attachment_id );
	$rows_by_post = get_transient( $cache_key );
	if ( ! is_array( $rows_by_post ) ) {
		$rows_by_post = open_station_my_wordpress_media_usage_collect( $attachment );
		set_transient(
			$cache_key,
			$rows_by_post,
			open_station_my_wordpress_media_usage_ttl( $attachment_id )
		);
	}
	 ...
	 */
	$payload = open_station_my_wordpress_media_usage_build( $attachment, $rows_by_post );
	 ...
	 */
	return apply_filters( 'open_station_my_wordpress_media_usage', $payload, $attachment_id );
}
 ...
 * `open_station_my_wordpress_media_usage_build()` and runs on every
 ...
 */
function open_station_my_wordpress_media_usage_collect( $attachment ) {
	global $wpdb;
 ...
 * @return array
 */
function open_station_my_wordpress_media_usage_build( $attachment, $rows_by_post = null ) {
	$attachment_id = (int) $attachment->ID;
	$file_url      = (string) wp_get_attachment_url( $attachment_id );
	$file_basename = '' !== $file_url ? wp_basename( $file_url ) : '';
 ...
 */
$GLOBALS['open_station_media_usage_pre_save_refs'] = array();
 ...
 * any ids appended via the `open_station_my_wordpress_attached_media`
 ...
 */
function open_station_my_wordpress_media_usage_extract_refs( $post ) {
	$ids = array();
	$obj = is_object( $post ) ? $post : get_post( (int) $post );
	if ( ! $obj || ! isset( $obj->ID ) ) {
		return $ids;
	}
	if ( ! function_exists( 'open_station_my_wordpress_post_attached_media' ) ) {
	...
	foreach ( open_station_my_wordpress_post_attached_media( (int) $obj->ID ) as $id ) {
		$id = (int) $id;
		if ( $id > 0 ) {
			$ids[ $id ] = true;
		}
 ...
 */
function open_station_my_wordpress_media_usage_snapshot_pre_save( $post_id ) {
	$post_id = (int) $post_id;
	if ( $post_id <= 0 ) {
		return;
	}
	$GLOBALS['open_station_media_usage_pre_save_refs'][ $post_id ] =
		open_station_my_wordpress_media_usage_extract_refs( $post_id );
}
add_action( 'pre_post_update', 'open_station_my_wordpress_media_usage_snapshot_pre_save' );
 ...
 */
function open_station_my_wordpress_media_usage_bust_for_post( $post_id ) {
	$post_id = (int) $post_id;
	if ( $post_id <= 0 ) {
		return;
	}

	$ids = open_station_my_wordpress_media_usage_extract_refs( $post_id );

	if ( isset( $GLOBALS['open_station_media_usage_pre_save_refs'][ $post_id ] ) ) {
		$ids += $GLOBALS['open_station_media_usage_pre_save_refs'][ $post_id ];
		unset( $GLOBALS['open_station_media_usage_pre_save_refs'][ $post_id ] );
	}

	foreach ( array_keys( $ids ) as $attachment_id ) {
		foreach ( open_station_my_wordpress_media_usage_cache_buckets() as $bucket ) {
			delete_transient(
				open_station_my_wordpress_media_usage_cache_key( (int) $attachment_id, $bucket )
			);
		}
	}
}
...
// arg, the post id).
add_action( 'before_delete_post', 'open_station_my_wordpress_media_usage_bust_for_post' );
 ...
 */
function open_station_my_wordpress_media_usage_bust_for_attachment( $post_id ) {
	$post_id = (int) $post_id;
	if ( $post_id <= 0 ) {
		return;
	}
	foreach ( open_station_my_wordpress_media_usage_cache_buckets() as $bucket ) {
		delete_transient(
			open_station_my_wordpress_media_usage_cache_key( $post_id, $bucket )
		);
	}
}
add_action( 'delete_attachment', 'open_station_my_wordpress_media_usage_bust_for_attachment' );
