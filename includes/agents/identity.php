 ...
 *
 * Those blocks, and `open_station_agent_is_agent()` itself, live in
 ...
 * @return string
 */
function open_station_agent_resolve_unique_login( $slug ) {
	$base    = 'agent-' . $slug;
	$login   = $base;
	$counter = 1;
 ...
 * @return string
 */
function open_station_agent_synthetic_email( $slug ) {
 ...
 * `open_station_agent_create()` orchestrator in store.php — call that,
 ...
 */
function open_station_agent_create_user( $args ) {
	...
	if ( '' === $slug ) {
		return new WP_Error(
			'open_station_agent_invalid_slug',
	...
	$user_id = wp_insert_user(
		array(
			'user_login'           => open_station_agent_resolve_unique_login( $slug ),
			'user_email'           => open_station_agent_synthetic_email( $slug ),
	...
	update_user_meta( $user_id, OPEN_STATION_AGENT_USER_MARKER_META, '1' );
 ...
 */
function open_station_agent_delete( $user_id, $reassign = null ) {
	if ( ! open_station_agent_is_agent( $user_id ) ) {
		return new WP_Error(
			'open_station_agent_not_an_agent',
	...
	if ( ! $deleted ) {
		return new WP_Error(
			'open_station_agent_delete_failed',
	 ...
	 */
	do_action( 'open_station_agent_deleted', (int) $user_id, get_current_user_id() );
 ...
 * @return string
 */
function open_station_agent_avatar_url() {
	return OPEN_STATION_URL . 'assets/images/agent-avatar.svg';
}
 ...
 * @return array
 */
function open_station_agent_avatar( $args, $id_or_email ) {
	$user_id = 0;
	...
	if ( $user_id > 0 && open_station_agent_is_agent( $user_id ) ) {
		$args['url']          = open_station_agent_avatar_url();
		$args['found_avatar'] = true;
	}
	return $args;
}
add_filter( 'pre_get_avatar_data', 'open_station_agent_avatar', 10, 2 );
 ...
 * @return string[]
 */
function open_station_agent_users_columns( $columns ) {
	$columns['open_station_agent_type'] = __( 'Type', 'desktop-mode' );
	return $columns;
}
add_filter( 'manage_users_columns', 'open_station_agent_users_columns' );
 ...
 * @return string
 */
function open_station_agent_users_custom_column( $output, $column_name, $user_id ) {
	if ( 'open_station_agent_type' !== $column_name ) {
		return $output;
	}
	if ( open_station_agent_is_agent( $user_id ) ) {
...
add_filter( 'manage_users_custom_column', 'open_station_agent_users_custom_column', 10, 3 );
