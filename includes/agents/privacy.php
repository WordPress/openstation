 ...
 * @return array
 */
function open_station_agents_register_personal_data_exporter( $exporters ) {
	$exporters['os-agents'] = array(
		'exporter_friendly_name' => __( 'OpenStation agents', 'desktop-mode' ),
		'callback'               => 'open_station_agents_personal_data_exporter',
	);
	return $exporters;
}
add_filter( 'wp_privacy_personal_data_exporters', 'open_station_agents_register_personal_data_exporter' );
 ...
 * @return array
 */
function open_station_agents_personal_data_exporter( $email_address, $page = 1 ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	...
	$rows = array(
		...
		array(
			...
			'value' => open_station_agent_get_description( (int) $user->ID ),
		),
		array(
			'name'  => __( 'Instructions (system prompt)', 'desktop-mode' ),
			'value' => open_station_agent_get_instructions( (int) $user->ID ),
		),
	);
	...
	$rate_limit = open_station_agent_get_rate_limit( (int) $user->ID );
	if ( $rate_limit > 0 ) {
		$rows[] = array(
 ...
 * @return array
 */
function open_station_agents_register_personal_data_eraser( $erasers ) {
	$erasers['os-agents'] = array(
		'eraser_friendly_name' => __( 'OpenStation agents', 'desktop-mode' ),
		'callback'             => 'open_station_agents_personal_data_eraser',
	);
	return $erasers;
}
add_filter( 'wp_privacy_personal_data_erasers', 'open_station_agents_register_personal_data_eraser' );
 ...
 * @return array
 */
function open_station_agents_personal_data_eraser( $email_address, $page = 1 ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	$user = get_user_by( 'email', $email_address );
	if ( ! $user || ! open_station_agent_is_agent( $user ) ) {
		return array(
			'items_removed'  => false,
	...
	$result = open_station_agent_delete( (int) $user->ID );
