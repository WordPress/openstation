 ...
 */
function open_station_comments_window_bulk_action_map() {
	return array(
 ...
 * Register all routes.
 */
function open_station_comments_window_register_rest_routes() {
	register_rest_route(
		'desktop-mode/v1',
		'/comments/bulk',
		array(
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => 'open_station_comments_window_rest_bulk',
			...
			'args'                => array(
				...
				'action' => array(
					'required' => true,
					'type'     => 'string',
					'enum'     => array_keys( open_station_comments_window_bulk_action_map() ),
				),
			),
		)
	);
	...
	register_rest_route(
		...
		array(
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => 'open_station_comments_window_rest_reply',
			'permission_callback' => static function () {
	...
	register_rest_route(
		...
		array(
			...
			'callback'            => 'open_station_comments_window_rest_insights',
			'permission_callback' => static function () {
				return current_user_can( 'moderate_comments' );
			},
			'args'                => array(
	...
	register_rest_route(
		...
		array(
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => 'open_station_comments_window_rest_counts',
			'permission_callback' => static function () {
				return current_user_can( 'edit_posts' );
			},
		)
...
add_action( 'rest_api_init', 'open_station_comments_window_register_rest_routes' );
 ...
 */
function open_station_comments_window_rest_bulk( WP_REST_Request $request ) {
	$ids    = array_values( array_filter( array_map( 'intval', (array) $request['ids'] ) ) );
	$action = (string) $request['action'];
	$map    = open_station_comments_window_bulk_action_map();

	if ( ! isset( $map[ $action ] ) ) {
		return new WP_Error(
			'open_station_comments_invalid_action',
	 ...
	 */
	do_action(
		'open_station_comments_window_after_bulk',
		$action,
		$processed,
		$skipped
	);
	...
	return new WP_REST_Response(
		array(
			...
			'processed' => $processed,
			'skipped'   => $skipped,
			'counts'    => open_station_comments_window_counts(),
		),
		200
	);
}
 ...
 * @return WP_REST_Response|WP_Error
 */
function open_station_comments_window_rest_reply( WP_REST_Request $request ) {
	...
	if ( ! $parent instanceof WP_Comment ) {
		return new WP_Error(
			'open_station_comments_no_parent',
	...
	if ( ! $post instanceof WP_Post || ! current_user_can( 'edit_post', $post->ID ) ) {
		return new WP_Error(
			'open_station_comments_forbidden',
	...
	if ( '' === trim( wp_strip_all_tags( $content ) ) ) {
		return new WP_Error(
			'open_station_comments_empty_reply',
	...
	if ( ! $user || ! $user->ID ) {
		return new WP_Error(
			'open_station_comments_unauthenticated',
 ...
 * @return WP_REST_Response|WP_Error
 */
function open_station_comments_window_rest_insights( WP_REST_Request $request ) {
	$email = strtolower( urldecode( (string) $request['email'] ) );
	if ( '' === $email || ! is_email( $email ) ) {
		return new WP_Error(
			'open_station_comments_invalid_email',
 ...
 */
function open_station_comments_window_rest_counts() {
	return new WP_REST_Response( open_station_comments_window_counts(), 200 );
}
 ...
 */
function open_station_comments_window_counts() {
