...
All in-tree routes register under `desktop-mode/v1`. Extensions are expected to register under `os-<extension>/v1` (declared via the `extensions/base/Open_Station_Extension_Rest` base's `namespace()` method; the base does not enforce a particular namespace — its docblock also permits `desktop-mode/v1`).
...
| `/debug` | GET | `includes/devtools.php` | `manage_options` (filterable via `open_station_debug_rest_permission`) |
...
| `/content-graph/post-types` | GET | `includes/content-graph/rest.php` | `edit_posts` (filterable via `open_station_content_graph_user_can_use`) |
| `/content-graph/nodes` | GET | `includes/content-graph/rest.php` | `edit_posts` (filterable via `open_station_content_graph_user_can_use`) |
| `/content-graph/post/{id}` | GET | `includes/content-graph/rest.php` | `edit_posts` (filterable via `open_station_content_graph_user_can_use`) |
...
| `/games/{game}/scores` | GET / POST | `includes/games/rest.php` | logged-in + OpenStation enabled + `read` (filterable via `open_station_games_rest_permission`) |
...
| `/games/challenges` | GET / POST | `includes/games/rest.php` | same games gate (create also passes `open_station_games_can_challenge`) |
...
- **Permission.** Permission callbacks use either `is_user_logged_in()` + capability checks or domain predicates. Shell-internal endpoints that only touch the caller's own per-user desktop state (`/session`, `/default-window`, `/intros`, `/os-settings`, `/pwa-state`, `/presence`) share the `open_station_rest_require_enabled()` gate (`includes/helpers.php`): logged-in **and** `open_station_is_enabled()`, returning `401` when logged out and `403` when OpenStation is off. `read` alone is deliberately not enough — every authenticated role carries it. Filtering with `open_station_*` hooks lets plugins extend or harden access.
...
If a future extension adds REST routes that don't fit any existing module, the `extensions/base/Open_Station_Extension_Rest` base class is the cheapest path. See `extensions/base/README.md`.
