 ...
 *
 * The header is sanitised with `sanitize_key()` — session ids
 * generated by the JS side (`crypto.randomUUID()` or the legacy
 * fallback) are alphanumeric + dashes, so this is a tight gate.
 *
 * @since 0.6.0
 *
 ...
 * @return bool True on success, false when storage refused (ring full
 *              and transient API unavailable, etc.).
 */
function desktop_mode_debug_publish( $session_id, $channel, $payload ) {
	 ...
	 * @param mixed  $payload    Published payload (post-filter).
	 */
