 ...
 * @return bool
 */
function open_station_desktop_theme_rmdir( $dir ) {
	$dir  = (string) $dir;
	$base = realpath( open_station_desktop_themes_dir() );
	$real = realpath( $dir );
	...
	foreach ( $items as $item ) {
		...
		if ( is_dir( $path ) && ! is_link( $path ) ) {
			open_station_desktop_theme_rmdir( $path );
		} else {
			wp_delete_file( $path );
		}
	}
 ...
 * @return bool
 */
function open_station_desktop_theme_zip_entry_ignored( $name ) {
 ...
 *                         `WP_Error` otherwise.
 */
function open_station_desktop_theme_validate_zip( $zip_path ) {
	if ( ! class_exists( 'ZipArchive' ) ) {
		return new WP_Error(
			'open_station_desktop_theme_no_zip_support',
	...
	if ( true !== $zip->open( (string) $zip_path ) ) {
		return new WP_Error(
			'open_station_desktop_theme_bad_zip',
	...
	for ( $i = 0; $i < $zip->numFiles; $i++ ) {
		...
		if ( false !== strpos( $name, "\0" ) || false !== strpos( $name, '\\' ) ) {
			$zip->close();
			return new WP_Error(
				'open_station_desktop_theme_unsafe_entry',
		...
		if ( open_station_desktop_theme_zip_entry_ignored( $name ) ) {
			continue;
		}
		// Directory entry.
		...
		if ( $counted > $caps['max_entries'] ) {
			$zip->close();
			return new WP_Error(
				'open_station_desktop_theme_too_many_entries',
		...
		if ( $size > $caps['max_file'] ) {
			$zip->close();
			return new WP_Error(
				'open_station_desktop_theme_entry_too_large',
				sprintf(
		...
		if ( $total > $caps['max_uncompressed'] ) {
			$zip->close();
			return new WP_Error(
				'open_station_desktop_theme_archive_too_large',
		...
		if ( ! isset( $extensions[ $ext ] ) ) {
			$zip->close();
			return new WP_Error(
				'open_station_desktop_theme_bad_extension',
				sprintf(
					/* translators: %s: file name inside the archive. */
	...
	if ( 1 !== count( $manifests ) ) {
		return new WP_Error(
			'open_station_desktop_theme_missing_manifest',
 ...
 */
function open_station_desktop_theme_sanitize_svg( $file ) {
	if ( ! class_exists( 'DOMDocument' ) ) {
		return new WP_Error(
			'open_station_desktop_theme_no_dom',
	...
	if ( '' === trim( $markup ) ) {
		return new WP_Error(
			'open_station_desktop_theme_bad_svg',
 ...
 */
function open_station_desktop_theme_sweep_staging( $max_age = DAY_IN_SECONDS ) {
	...
	foreach ( (array) glob( $base . '/.staging-*', GLOB_ONLYDIR ) as $dir ) {
		...
		if ( open_station_desktop_theme_rmdir( $dir ) ) {
			++$removed;
		}
	}
 ...
 */
function open_station_desktop_theme_install_from_zip( $zip_path ) {
	...
	open_station_desktop_theme_sweep_staging();

	$manifest_entry = open_station_desktop_theme_validate_zip( $zip_path );
	...
	if ( ! WP_Filesystem() ) {
		open_station_desktop_theme_rmdir( $staging );
		return new WP_Error(
			'open_station_desktop_theme_filesystem_unavailable',
	...
	if ( null === $decoded ) {
		open_station_desktop_theme_rmdir( $staging );
		return new WP_Error(
			'open_station_desktop_theme_bad_json',
	...
	$manifest = open_station_sanitize_desktop_theme_manifest(
		$decoded,
		open_station_desktop_theme_staging_asset_resolver( $root )
	);
	...
	foreach ( array_keys( $assets ) as $relative ) {
		...
		$sanitized = open_station_desktop_theme_sanitize_svg( $root . '/' . $relative );
	...
	$target = open_station_desktop_themes_dir( $slug );
	if ( is_dir( $target ) ) {
		open_station_desktop_theme_rmdir( $target );
	}
	...
	foreach ( array_keys( $assets ) as $relative ) {
		...
		if ( ! $wp_filesystem->move( $root . '/' . $relative, $destination, true ) ) {
			...
			open_station_desktop_theme_rmdir( $target );
			return new WP_Error(
				'open_station_desktop_theme_write_failed',
	 ...
	 */
	do_action( 'open_station_desktop_theme_installed', $slug, $entry );
 ...
 * @return true|WP_Error
 */
function open_station_desktop_theme_delete( $slug ) {
	$slug  = sanitize_key( (string) $slug );
	$index = open_station_desktop_themes_index();
	if ( '' === $slug || ! isset( $index[ $slug ] ) ) {
		return new WP_Error(
			'open_station_desktop_theme_not_found',
	...
	$entry = $index[ $slug ];
	$dir   = open_station_desktop_themes_dir( $slug );
	if ( is_dir( $dir ) ) {
		open_station_desktop_theme_rmdir( $dir );
	}
	unset( $index[ $slug ] );
	open_station_desktop_themes_put_index( $index );
	 ...
	 */
	do_action( 'open_station_desktop_theme_deleted', $slug, $entry );
